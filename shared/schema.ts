...
export const licencasAmbientais = pgTable("licencas_ambientais", {
  ...
  dispensaLicenciamento: boolean("dispensa_licenciamento").default(false),
  semDataValidade: boolean("sem_data_validade").default(false),
...
export const condicionantes = pgTable("condicionantes", {
  ...
  solicitadoEncerramento: boolean("solicitado_encerramento").notNull().default(false),
  dataEncerramento: date("data_encerramento"),
  arquivoEncerramento: text("arquivo_encerramento"), // object: path
...
export const campanhas = pgTable("campanhas", {
  ...
  nome: text("nome").notNull(),
  empreendimentoId: integer("empreendimento_id").references(() => empreendimentos.id).notNull(),
  projetoId: integer("projeto_id"), // optional link to a project (FK handled at app level)
  periodoInicio: date("periodo_inicio").notNull(),
  periodoFim: date("periodo_fim").notNull(),
  descricao: text("descricao"),
  observacoesMetodologicas: text("observacoes_metodologicas"),
  // planejada | em_andamento | atrasada | suspensa | cancelada | encerrada | concluida
  status: text("status").default("planejada"),
  motivoStatus: text("motivo_status"),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
  atualizadoEm: timestamp("atualizado_em").defaultNow().notNull(),
});

export const campanhaIntercorrencias = pgTable("campanha_intercorrencias", {
  id: serial("id").primaryKey(),
  campanhaId: integer("campanha_id").references(() => campanhas.id).notNull(),
  data: date("data").notNull(),
  tipo: text("tipo").notNull().default("ocorrencia"), // ocorrencia, anomalia, impedimento, observacao
  titulo: text("titulo").notNull(),
  descricao: text("descricao"),
  imagens: text("imagens").array().default([]),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
});

export type CampanhaIntercorrencia = typeof campanhaIntercorrencias.$inferSelect;
export type InsertCampanhaIntercorrencia = z.infer<typeof insertCampanhaIntercorrenciaSchema>;
export const insertCampanhaIntercorrenciaSchema = createInsertSchema(campanhaIntercorrencias).omit({ id: true, criadoEm: true });
...
export const projetos = pgTable("projetos", {
  ...
  // Operação
  periodicidade: text("periodicidade"), // diario, semanal, quinzenal, mensal, bimestral, trimestral, semestral, anual
...
export const projetoIntercorrencias = pgTable("projeto_intercorrencias", {
  id: serial("id").primaryKey(),
...
export type ProjetoIntercorrencia = typeof projetoIntercorrencias.$inferSelect;
export type InsertProjetoIntercorrencia = z.infer<typeof insertProjetoIntercorrenciaSchema>;
export const insertProjetoIntercorrenciaSchema = createInsertSchema(projetoIntercorrencias).omit({ id: true, criadoEm: true });
...
export const financeiroLancamentos = pgTable("financeiro_lancamentos", {
  ...
  campanhaId: integer("campanha_id").references(() => campanhas.id), // Campanha vinculada (opcional)
  ...
  comprovanteUrl: text("comprovante_url"), // URL do arquivo de comprovante (recibo/NF)
  notaFiscalUrl: text("nota_fiscal_url"), // URL da nota fiscal separada
  observacoes: text("observacoes"),
  unidade: text("unidade").notNull().default('salvador'), // unidade padrão
  criadoPor: integer("criado_por").references(() => users.id).notNull(),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
...
export const financeiroLancamentosRelations = relations(financeiroLancamentos, ({ one, many }) => ({
  ...
  campanha: one(campanhas, {
    fields: [financeiroLancamentos.campanhaId],
    references: [campanhas.id],
  }),
...
export const datasets = pgTable("datasets", {
  ...
  projetoVinculoId: integer("projeto_vinculo_id"), // Vínculo com projeto
  campanhaVinculoId: integer("campanha_vinculo_id"), // Vínculo com campanha
});
...
// Tabela de vinculação projeto ↔ equipamentos (many-to-many)
export const projetoEquipamentos = pgTable("projeto_equipamentos", {
  id: serial("id").primaryKey(),
  projetoId: integer("projeto_id").references(() => projetos.id).notNull(),
...
export const insertProjetoEquipamentoSchema = createInsertSchema(projetoEquipamentos).omit({ id: true, criadoEm: true });
export type InsertProjetoEquipamento = z.infer<typeof insertProjetoEquipamentoSchema>;
export type ProjetoEquipamento = typeof projetoEquipamentos.$inferSelect;

// Tabela de vinculação projeto ↔ veículos/frota (many-to-many)
export const projetoVeiculos = pgTable("projeto_veiculos", {
  id: serial("id").primaryKey(),
  projetoId: integer("projeto_id").references(() => projetos.id).notNull(),
  veiculoId: integer("veiculo_id").references(() => veiculos.id).notNull(),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
});

export const insertProjetoVeiculoSchema = createInsertSchema(projetoVeiculos).omit({ id: true, criadoEm: true });
export type InsertProjetoVeiculo = z.infer<typeof insertProjetoVeiculoSchema>;
export type ProjetoVeiculo = typeof projetoVeiculos.$inferSelect;
...
export const amostras = pgTable("amostras", {
  ...
  contatoLaboratorio: text("contato_laboratorio"), // phone / email / contact name
  dataEnvioLab: date("data_envio_lab"),
  dataPrevisaoResultado: date("data_previsao_resultado"), // prazo de entrega
  ...
  intercorrencias: text("intercorrencias"), // incidents / observations during analysis
  resultadoArquivo: text("resultado_arquivo"),
  laudoArquivo: text("laudo_arquivo"), // path to laudo PDF/DOCX
  imagensAnexos: text("imagens_anexos"), // JSON array of { name, url, type } objects
  observacoes: text("observacoes"),
...
export const recibos = pgTable("recibos", {
  ...
  observacoes: text("observacoes"),
  arquivo: text("arquivo"), // URL do recibo (PDF/imagem)
  notaFiscalUrl: text("nota_fiscal_url"), // URL da nota fiscal
  unidade: text("unidade"),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
});
...
export const campoRegistros = pgTable("campo_registros", {
  ...
  caracterizacao: text("caracterizacao"),
  malha: text("malha"),
  densidade: text("densidade"),
  estagioDesenvolvimento: text("estagio_desenvolvimento"),
  numeracaoLista: text("numeracao_lista"),
  numeroEtiquetaCampo: text("numero_etiqueta_campo"),
  ...
  // Status & Environment
  sazonalidade: text("sazonalidade"),
  ...
  // Collection + Ichthyofauna biometrics
  pesoG: text("peso_g"),
  ctMm: text("ct_mm"),
  lcMm: text("lc_mm"),
  ccMm: text("cc_mm"),
  acMm: text("ac_mm"),
  doMm: text("do_mm"),
  tipoCorpoAgua: text("tipo_corpo_agua"),
  estratoVertical: text("estrato_vertical"),
  ...
  // Location classification (e.g. jusante / montante)
  localizacao: text("localizacao"),
  // Effort data for CPUE analysis (fisheries / trapping)
  esforcoAmostral: real("esforco_amostral"),
  unidadeEsforco: text("unidade_esforco"),
  // Environmental parameters (non-fauna types: ruido, qualidade_agua, solo)
  parametros: json("parametros").$type<Record<string, number | string | null>>(),
  // System
  unidade: text("unidade").notNull().default('goiania'),
...
// Audit log de alterações em registros de campo (F5)
export const campoAuditLog = pgTable("campo_audit_log", {
  id: serial("id").primaryKey(),
  registroId: integer("registro_id").notNull(),
  userId: integer("user_id"),
  userName: text("user_name"),
  acao: text("acao").notNull(), // 'create' | 'update' | 'delete'
  camposAlterados: json("campos_alterados").$type<string[]>(),
  valorAnterior: json("valor_anterior").$type<Record<string, any>>(),
  valorNovo: json("valor_novo").$type<Record<string, any>>(),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
});
export type CampoAuditLog = typeof campoAuditLog.$inferSelect;
...
export const documentoVinculos = pgTable("documento_vinculos", {
  ...
  origemTipo: text("origem_tipo").notNull(), // 'licenca' | 'contrato' | 'minuta' | 'parecer' | 'plano' | 'ata' | 'entregavel' | 'autorizacao' | 'condicionante'
  origemId: integer("origem_id").notNull(),
  origemNome: text("origem_nome").notNull(),
  destinoTipo: text("destino_tipo").notNull(),
...
export const insertDocumentoVinculoSchema = createInsertSchema(documentoVinculos).omit({ id: true, criadoEm: true });
export type InsertDocumentoVinculo = z.infer<typeof insertDocumentoVinculoSchema>;
export type DocumentoVinculo = typeof documentoVinculos.$inferSelect;

// ═══════════════════════════════════════════════════════
// MODELOS E LAYOUTS
// ═══════════════════════════════════════════════════════
export const modelosLayouts = pgTable("modelos_layouts", {
  id: serial("id").primaryKey(),
  titulo: text("titulo").notNull(),
  tipo: text("tipo").notNull().default("relatorio"),
  // relatorio, oficio, ata, memorando, contrato, comunicado, proposta, planilha, apresentacao, outro
  descricao: text("descricao"),
  tags: text("tags"), // comma-separated
  versao: text("versao").notNull().default("1.0"),
  // Empreendimento vinculado
  empreendimentoId: integer("empreendimento_id").references(() => empreendimentos.id),
  // Paleta de cores RGB — JSON array de strings "R,G,B" (até 4 cores)
  coresRgb: text("cores_rgb"), // JSON: ["0,89,156","178,205,225","51,115,33","95,126,107"]
  // Arquivo (Object Storage)
  arquivoPath: text("arquivo_path"), // object:modelos/timestamp_name.ext
  arquivoNome: text("arquivo_nome"),
  arquivoMime: text("arquivo_mime"),
  arquivoTamanho: integer("arquivo_tamanho"),
  // Controle
  unidade: text("unidade").notNull().default("geral"),
  ativo: boolean("ativo").notNull().default(true),
  criadoPor: text("criado_por"),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
  atualizadoEm: timestamp("atualizado_em").defaultNow().notNull(),
...
export const insertModeloLayoutSchema = createInsertSchema(modelosLayouts).omit({
  id: true, criadoEm: true, atualizadoEm: true,
});
export type InsertModeloLayout = z.infer<typeof insertModeloLayoutSchema>;
export type ModeloLayout = typeof modelosLayouts.$inferSelect;

// ══════════════════════════════════════════════════════════════════════════════
// EMAIL AUTOMAÇÃO
// ══════════════════════════════════════════════════════════════════════════════
export const emailContas = pgTable("email_contas", {
  id: serial("id").primaryKey(),
  nome: text("nome").notNull(),
  ...
  ativo: boolean("ativo").notNull().default(true),
  unidade: text("unidade").notNull().default("geral"),
  ultimaSincEm: timestamp("ultima_sinc_em"),
  totalEmails: integer("total_emails").notNull().default(0),
  criadoPor: integer("criado_por"),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
});

export const insertEmailContaSchema = createInsertSchema(emailContas).omit({ id: true, criadoEm: true, ultimaSincEm: true, totalEmails: true });
export type InsertEmailConta = z.infer<typeof insertEmailContaSchema>;
export type EmailConta = typeof emailContas.$inferSelect;

export const emailsCapturados = pgTable("emails_capturados", {
  id: serial("id").primaryKey(),
  contaId: integer("conta_id"),
  messageId: text("message_id"),
  dataRecebimento: timestamp("data_recebimento"),
  remetente: text("remetente"),
  remetenteOrg: text("remetente_org"),
  destinatarios: json("destinatarios").$type<string[]>(),
  ...
  descricaoDemanda: text("descricao_demanda"),
  prioridade: text("prioridade").default("media"),
  nivelRisco: text("nivel_risco").default("baixo"),
  statusDemanda: text("status_demanda"),
  // Vínculos
  empreendimentoId: integer("empreendimento_id"),
  empreendimentoNome: text("empreendimento_nome"),
  clienteVinculado: text("cliente_vinculado"),
  contratoVinculado: text("contrato_vinculado"),
  programaAmbiental: text("programa_ambiental"),
  condicionante: text("condicionante"),
  numeroProcesso: text("numero_processo"),
  responsavelSugerido: text("responsavel_sugerido"),
  responsavelExterno: text("responsavel_externo"),
  prazoIdentificado: text("prazo_identificado"),
  dataVencimento: timestamp("data_vencimento"),
  // Anexos e evidências
  anexos: json("anexos").$type<{ nome: string; tamanho?: number; tipo?: string }[]>(),
  evidenciaCumprimento: boolean("evidencia_cumprimento").default(false),
  // Controle de qualidade IA
  nivelConfianca: real("nivel_confianca"),
  precisaValidacao: boolean("precisa_validacao").default(false),
  vinculoPendente: boolean("vinculo_pendente").default(false),
  sugestoesVinculo: json("sugestoes_vinculo").$type<{ id: number; nome: string; confianca: number }[]>(),
  observacoesIA: text("observacoes_ia"),
  // Demanda criada automaticamente
  demandaCriadaId: integer("demanda_criada_id"),
  demandaAtualizada: boolean("demanda_atualizada").default(false),
  // Validação humana
  validadoPor: integer("validado_por"),
  validadoEm: timestamp("validado_em"),
  validacaoObservacoes: text("validacao_observacoes"),
  // Controle
  unidade: text("unidade").notNull().default("geral"),
  processado: boolean("processado").notNull().default(true),
  criadoEm: timestamp("criado_em").defaultNow().notNull(),
  atualizadoEm: timestamp("atualizado_em").defaultNow().notNull(),
});

export const insertEmailCapturadoSchema = createInsertSchema(emailsCapturados).omit({ id: true, criadoEm: true, atualizadoEm: true });
export type InsertEmailCapturado = z.infer<typeof insertEmailCapturadoSchema>;
export type EmailCapturado = typeof emailsCapturados.$inferSelect;

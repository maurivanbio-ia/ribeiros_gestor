...
import { Plus, FileText, Calendar, Building, Download, Shield, AlertCircle, CheckCircle, Save, Pencil, Trash2, AlertTriangle, Loader2, Upload, RefreshCw, Archive, RefreshCcw, X, Ban, Infinity, Layers, ChevronDown, ChevronRight as ChevronRightIcon } from "lucide-react";
import { Switch } from "@/components/ui/switch";
import { Textarea } from "@/components/ui/textarea";
import { formatDate, formatDateBR, getStatusLabel, getStatusClass } from "@/lib/date-utils";
...
export function LicencasTab({ empreendimentoId }: LicencasTabProps) {
  ...
  const renovacaoForm = useForm<LicenseFormData>({
    resolver: zodResolver(licenseSchema),
    defaultValues: { numero: "", tipo: "", tipoOutorga: "", orgaoEmissor: "", dataEmissao: "", validade: "", arquivoPdf: "", dispensaLicenciamento: false, semDataValidade: false, observacoes: "", grupo: "" },
  });
  ...
  const handleEdit = (license: LicencaAmbiental) => {
    ...
    form.reset({
      ...
      dispensaLicenciamento: (license as any).dispensaLicenciamento || false,
      semDataValidade: (license as any).semDataValidade || false,
      observacoes: (license as any).observacoes || "",
      grupo: (license as any).grupo || "",
    });
  ...
  return (
    <div className="space-y-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <p className="text-xs text-muted-foreground">Use o mesmo nome para agrupar licenças de um bloco ou fase na visualização.</p>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
      ...
      {licencas.length > 0 ? (() => {
        ...
        const hasGroups = licencas.some(l => (l as any).grupo);
        const grouped: [string, LicencaAmbiental[]][] = [];
        if (hasGroups) {
          const map = new Map<string, LicencaAmbiental[]>();
          for (const l of licencas) {
            const key = (l as any).grupo || "Sem grupo";
            if (!map.has(key)) map.set(key, []);
            map.get(key)!.push(l);
          }
          // Sort: named groups first (alphabetically), then "Sem grupo" last
          const keys = [...map.keys()].sort((a, b) => {
            if (a === "Sem grupo") return 1;
            if (b === "Sem grupo") return -1;
            return a.localeCompare(b, 'pt-BR');
          });
          for (const k of keys) grouped.push([k, map.get(k)!]);
        }

        const renderLicenseCard = (license: LicencaAmbiental) => (
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center mb-2 flex-wrap gap-2">
                    ...
                    {(license as any).dispensaLicenciamento && (
                      <Badge variant="outline" className="text-amber-700 border-amber-400 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-700 dark:text-amber-300 flex items-center gap-1">
                        <Ban className="h-3 w-3" />
                        Dispensa de Licenciamento
                      </Badge>
                    )}
                    {(license as any).semDataValidade && (
                      <Badge variant="outline" className="text-blue-700 border-blue-400 bg-blue-50 dark:bg-blue-950/30 dark:border-blue-700 dark:text-blue-300 flex items-center gap-1">
                        <Infinity className="h-3 w-3" />
                        Sem data de validade
                      </Badge>
                    )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      <p className="text-muted-foreground">
                        ...
                        {(license as any).semDataValidade
                          ? <span className="inline-flex items-center gap-1 text-blue-600 dark:text-blue-400"><Infinity className="h-3 w-3" />Sem vencimento</span>
                          : formatDateBR(license.validade)}
                      </p>
                    </div>
                  </div>
                  {(license as any).observacoes && (
                    <div className="mt-2 text-sm text-muted-foreground">
                      <p className="font-medium text-card-foreground">Observações:</p>
                      <p className="whitespace-pre-line">{(license as any).observacoes}</p>
                    </div>
                  )}
                  {license.arquivoPdf && (
                    <div className="mt-2">
        ...
        if (hasGroups) {
          return (
            <div className="space-y-3">
              {grouped.map(([groupName, groupLicenses]) => {
                const isOpen = !closedGroups.has(groupName);
                const ativas = groupLicenses.filter(l => l.status === 'ativa').length;
                const vencidas = groupLicenses.filter(l => l.status === 'vencida').length;
                const aVencer = groupLicenses.filter(l => l.status === 'a_vencer').length;
                const isSemGrupo = groupName === "Sem grupo";
                return (
                  <div key={groupName} className="border rounded-lg overflow-hidden shadow-sm">
                    <button
                      type="button"
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-semibold text-sm ${isSemGrupo ? "text-muted-foreground italic" : "text-card-foreground"}`}>
                          {groupName}
                        </span>
                        <Badge variant="secondary" className="text-xs">{groupLicenses.length}</Badge>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        {ativas > 0 && <span className="text-green-600 font-medium">{ativas} ativa{ativas !== 1 ? 's' : ''}</span>}
                        {aVencer > 0 && <span className="text-yellow-600 font-medium">{aVencer} a vencer</span>}
                        {vencidas > 0 && <span className="text-red-600 font-medium">{vencidas} vencida{vencidas !== 1 ? 's' : ''}</span>}
                      </div>
                    </button>
                    {isOpen && (
                      <div className="p-3 space-y-3 bg-background">
                        {groupLicenses.map(renderLicenseCard)}
                      </div>
                    )}
                  </div>
                );
        ...
        return (
          <div className="space-y-4">
            {licencas.map(renderLicenseCard)}
          </div>
        );
      })() : (

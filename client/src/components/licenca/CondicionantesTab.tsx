...
import {
  ...
  CheckCheck, Shield, Search, RotateCcw, XCircle as XCircleIcon,
  LogOut, Upload, Eye, AlertTriangle, Copy, ChevronsRight,
  CheckSquare, Square,
} from "lucide-react";
...
// ─── Dialog: Replicar Condicionantes ──────────────────────────────────────────

function ReplicarCondicionantesDialog({ condicionantes, licencaId, licenca }: {
  condicionantes: Condicionante[];
  licencaId: number;
  licenca: LicencaAmbiental;
}) {
  const [open, setOpen] = useState(false);
  const [selectedConds, setSelectedConds] = useState<Set<number>>(new Set());
  const [selectedLicencas, setSelectedLicencas] = useState<Set<number>>(new Set());
  const [searchLicenca, setSearchLicenca] = useState("");
  const [isPending, setIsPending] = useState(false);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const { data: todasLicencas = [] } = useQuery<LicencaAmbiental[]>({
    queryKey: ["/api/licencas"],
    enabled: open,
  });

  // Licenças disponíveis como destino (excluindo a atual)
  const licencasDestino = (todasLicencas as any[]).filter(l => l.id !== licencaId);
  const licencasFiltradas = searchLicenca
    ? licencasDestino.filter(l =>
        (l.numero || "").toLowerCase().includes(searchLicenca.toLowerCase()) ||
        (l.tipo || "").toLowerCase().includes(searchLicenca.toLowerCase()) ||
        (l.orgaoEmissor || "").toLowerCase().includes(searchLicenca.toLowerCase()) ||
        (l.empreendimentoNome || "").toLowerCase().includes(searchLicenca.toLowerCase())
      )
    : licencasDestino;

  const handleOpen = () => {
    // Pré-seleciona todas as condicionantes
    setSelectedConds(new Set(condicionantes.map(c => c.id)));
    setSelectedLicencas(new Set());
    setSearchLicenca("");
    setOpen(true);
  };

  const toggleCond = (id: number) => {
  ...
  const toggleAllConds = () => {
    if (selectedConds.size === condicionantes.length) setSelectedConds(new Set());
    else setSelectedConds(new Set(condicionantes.map(c => c.id)));
  };

  const toggleAllLicencas = () => {
    if (selectedLicencas.size === licencasFiltradas.length) setSelectedLicencas(new Set());
    else setSelectedLicencas(new Set(licencasFiltradas.map(l => l.id)));
  };

  const handleReplicar = async () => {
    if (selectedLicencas.size === 0) {
      toast({ title: "Selecione ao menos uma licença de destino", variant: "destructive" });
      return;
    }
    if (selectedConds.size === 0) {
      toast({ title: "Selecione ao menos uma condicionante para replicar", variant: "destructive" });
      return;
    }
    setIsPending(true);
    try {
      const res = await apiRequest("POST", `/api/licencas/${licencaId}/condicionantes/replicar`, {
        targetLicencaIds: Array.from(selectedLicencas),
        condicionanteIds: Array.from(selectedConds),
      });
      const data = await res.json();
      await queryClient.invalidateQueries({ queryKey: ["/api/licencas"] });
      toast({
        title: `${data.criadas} condicionante${data.criadas !== 1 ? "s" : ""} replicada${data.criadas !== 1 ? "s" : ""} com sucesso`,
        description: `Para ${selectedLicencas.size} licença${selectedLicencas.size !== 1 ? "s" : ""} de destino`,
      });
      setOpen(false);
    } catch (err: any) {
      toast({ title: "Erro ao replicar condicionantes", variant: "destructive" });
    } finally {
      setIsPending(false);
    }
  };

  if (condicionantes.length === 0) return null;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        Replicar para outras licenças
      </Button>
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b">
          <DialogTitle className="flex items-center gap-2 text-lg">
            <Copy className="h-5 w-5 text-primary" />
            Replicar Condicionantes
          </DialogTitle>
          <p className="text-sm text-muted-foreground mt-1">
            Selecione quais condicionantes deseja copiar e para quais licenças de destino.
            As condicionantes serão criadas com status <strong>Pendente</strong>.
          </p>
        </DialogHeader>

        <div className="flex flex-1 overflow-hidden min-h-0">
          {/* Coluna esquerda: condicionantes */}
          <div className="w-1/2 border-r flex flex-col">
            <div className="px-4 py-3 border-b bg-muted/30 flex items-center justify-between">
              <span className="text-sm font-semibold">
                Condicionantes ({selectedConds.size}/{condicionantes.length})
              </span>
              <button
                type="button"
                onClick={toggleAllConds}
                className="text-xs text-primary hover:underline flex items-center gap-1"
              >
                {selectedConds.size === condicionantes.length
                  ? <><Square className="h-3 w-3" /> Desmarcar todas</>
                  : <><CheckSquare className="h-3 w-3" /> Selecionar todas</>
                }
              </button>
            </div>
            <ScrollArea className="flex-1">
              <div className="p-3 space-y-1">
                {condicionantes.map(cond => {
                  const titulo = (cond as any).titulo || cond.descricao?.substring(0, 50);
                  const codigo = (cond as any).codigo;
                  return (
                    <label
                      key={cond.id}
                      className="flex items-start gap-2.5 p-2.5 rounded-lg hover:bg-muted/50 cursor-pointer group transition-colors"
                    >
                      <Checkbox
                        checked={selectedConds.has(cond.id)}
                        onCheckedChange={() => toggleCond(cond.id)}
                        className="mt-0.5 flex-shrink-0"
                      />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {codigo && (
                            <code className="text-xs font-mono bg-muted px-1 py-0.5 rounded text-muted-foreground">{codigo}</code>
                          )}
                          <span className="text-sm font-medium line-clamp-1">{titulo}</span>
                        </div>
                        {(cond as any).categoria && (
                          <span className="text-xs text-muted-foreground">{(cond as any).categoria}</span>
                        )}
                      </div>
                    </label>
                  );
          ...
          <div className="flex items-center justify-center w-0 relative">
            <div className="absolute z-10 bg-background border rounded-full p-1.5 shadow-sm">
              <ChevronsRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </div>

          {/* Coluna direita: licenças de destino */}
          <div className="w-1/2 flex flex-col">
            <div className="px-4 py-3 border-b bg-muted/30 flex items-center justify-between gap-2">
              <span className="text-sm font-semibold">
                Licenças de destino ({selectedLicencas.size})
              </span>
              <button
                type="button"
                onClick={toggleAllLicencas}
                className="text-xs text-primary hover:underline flex items-center gap-1 flex-shrink-0"
              >
                {selectedLicencas.size === licencasFiltradas.length && licencasFiltradas.length > 0
            ...
            <div className="px-3 py-2 border-b">
              <div className="relative">
                ...
                <Input
                  ...
                  value={searchLicenca}
                  onChange={e => setSearchLicenca(e.target.value)}
              ...
              <div className="p-3 space-y-1">
                {licencasFiltradas.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground text-sm">
                    {searchLicenca ? "Nenhuma licença encontrada" : "Nenhuma outra licença disponível"}
                  </div>
                ) : (
                  licencasFiltradas.map((lic: any) => (
                    <label
                      key={lic.id}
                      className="flex items-start gap-2.5 p-2.5 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors"
                    >
                      <Checkbox
                        checked={selectedLicencas.has(lic.id)}
                        onCheckedChange={() => toggleLicenca(lic.id)}
                        className="mt-0.5 flex-shrink-0"
                      />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-sm font-semibold">{lic.tipo}</span>
                          <code className="text-xs font-mono bg-muted px-1 py-0.5 rounded text-muted-foreground">{lic.numero}</code>
                        </div>
                        {lic.empreendimentoNome && (
                          <span className="text-xs text-muted-foreground block truncate">{lic.empreendimentoNome}</span>
                        )}
                        {lic.orgaoEmissor && (
        ...
        <div className="px-6 py-4 border-t bg-muted/20 flex items-center justify-between gap-4">
          <div className="text-sm text-muted-foreground">
            {selectedConds.size > 0 && selectedLicencas.size > 0 ? (
              <span className="text-foreground font-medium">
                Será criada{selectedConds.size !== 1 ? "m" : ""}{" "}
                <strong>{selectedConds.size * selectedLicencas.size}</strong> condicionante{selectedConds.size * selectedLicencas.size !== 1 ? "s" : ""} no total
              </span>
            ) : (
              "Selecione condicionantes e licenças de destino"
            )}
          </div>
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={isPending || selectedConds.size === 0 || selectedLicencas.size === 0}
              className="gap-2"
            >
              {isPending ? (
...
// ─── Dialog: Solicitar Encerramento ───────────────────────────────────────────

function SolicitarEncerramentoDialog({ condicionante, licencaId, onSuccess }: {
  condicionante: Condicionante;
  licencaId: number;
  onSuccess: () => void;
}) {
  ...
  const jaEncerrado = !!(condicionante as any).solicitadoEncerramento;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!dataEncerramento) {
      toast({ title: "Informe a data do pedido", variant: "destructive" });
      return;
    }
    setIsPending(true);
    try {
      ...
      const res = await fetch(`/api/condicionantes/${condicionante.id}/encerramento`, {
        method: "POST",
        body: formData,
        credentials: "include",
      });
      ...
      await queryClient.invalidateQueries({ queryKey: ["/api/licencas", licencaId, "condicionantes"] });
      toast({ title: "Encerramento solicitado com sucesso" });
      setOpen(false);
      onSuccess();
    } catch (err: any) {
  ...
  const handleRemover = async () => {
    if (!confirm("Remover a solicitação de encerramento desta condicionante?")) return;
    setIsPending(true);
    try {
      const res = await fetch(`/api/condicionantes/${condicionante.id}/encerramento`, {
        method: "DELETE",
      ...
      await queryClient.invalidateQueries({ queryKey: ["/api/licencas", licencaId, "condicionantes"] });
      toast({ title: "Solicitação de encerramento removida" });
    } catch {
    ...
    <>
      {jaEncerrado ? (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 px-2.5 py-1 rounded-full border border-orange-200 dark:border-orange-700">
            <LogOut className="h-3 w-3" />
            Encerramento Solicitado
            {(condicionante as any).dataEncerramento && (
              <span className="opacity-70">· {formatDate((condicionante as any).dataEncerramento)}</span>
            )}
          </span>
          {(condicionante as any).arquivoEncerramento && (
            <a
              href={`/api/condicionantes/${condicionante.id}/encerramento/arquivo`}
              target="_blank"
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <Button
            variant="outline"
            size="sm"
            className="h-8 gap-1.5 border-orange-300 text-orange-700 hover:bg-orange-50 hover:border-orange-400 dark:border-orange-700 dark:text-orange-400 dark:hover:bg-orange-900/20"
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="text-sm text-muted-foreground bg-muted/50 rounded-lg p-3 mb-1">
              <span className="font-medium text-foreground">{(condicionante as any).titulo || condicionante.descricao?.substring(0, 60)}</span>
              {(condicionante as any).codigo && (
                <span className="ml-2 font-mono text-xs text-muted-foreground">{(condicionante as any).codigo}</span>
              )}
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium">Documento do pedido (PDF ou imagem)</label>
                <div
                  ...
                  onClick={() => document.getElementById(`enc-file-${condicionante.id}`)?.click()}
                >
                  {file ? (
                    ...
                    <div className="text-muted-foreground space-y-1">
                      ...
                      <p className="text-xs">Clique para selecionar PDF, JPG ou PNG</p>
                    </div>
                  )}
                  <input
                    id={`enc-file-${condicionante.id}`}
                    type="file"
              ...
              >
                <div className="flex items-start gap-4 p-4">
                  ...
                  <div className="flex-shrink-0 flex flex-col items-end gap-2 min-w-[130px]">
                    ...
                    {/* Badge encerramento solicitado */}
                    {(cond as any).solicitadoEncerramento && (
                      <Badge className="gap-1 text-xs bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 border border-orange-200 dark:border-orange-700">
                        <LogOut className="h-3 w-3" />
                        Enc. Solicitado
                      </Badge>
                    )}
                ...
                {isSelected && (
                  <div className="border-t px-4 pt-3 pb-4 bg-muted/30 rounded-b-xl space-y-3">
                    ...
                    <div className="flex gap-2 pt-1 flex-wrap" onClick={e => e.stopPropagation()}>
                      ...
                      <SolicitarEncerramentoDialog
                        condicionante={cond}
                        licencaId={licencaId}
                        onSuccess={() => setSelectedCond(null)}
                      />
                    </div>

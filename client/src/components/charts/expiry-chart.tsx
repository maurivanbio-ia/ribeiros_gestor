...
export default function ExpiryChart() {
  ...
  useEffect(() => {
    ...
    const bgColors = counts.map((_, idx) =>
      idx === currentMonthIdx
        ? "#ea580c"                    // mês atual: laranja primário (destaque)
        : idx < currentMonthIdx
          ? "#fdba74"                  // meses passados: pêssego
          : "#0099a8"                  // meses futuros: teal
    );
      ...
      options: {
        ...
        scales: {
          y: {
            beginAtZero: true,
            ticks: { stepSize: 1 },
            title: { display: true, text: "Nº de Vencimentos", font: { size: 10 } },
          },
          x: {
            ticks: { font: { size: 10 } },
            grid: { display: false },
            title: { display: true, text: "Mês", font: { size: 10 } },
          },
        },
        plugins: {
          legend: {

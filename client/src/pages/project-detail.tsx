...
import { VinculosDocumentosTab } from "@/components/empreendimento/VinculosDocumentosTab";
import { CampanhasTab } from "@/components/empreendimento/CampanhasTab";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        {/* Nav de tabs com grupos retráteis */}
        <div className="flex flex-wrap gap-1 mb-6 p-1 bg-muted rounded-lg border">
          {/* Tabs principais — sempre visíveis */}
          {[
            { value: "licencas",  label: "Licenças",  Icon: FileText },
            { value: "projetos",  label: "Projetos",  Icon: FolderKanban },
            { value: "campanhas", label: "Campanhas", Icon: Target },
            { value: "demandas",  label: "Demandas",  Icon: ClipboardList },
          ...
          {/* Grupo: Gestão Contratual */}
          {(() => {
            const items = [
              { value: "contratos",    label: "Contratos",    Icon: Briefcase },
              { value: "cronograma",   label: "Cronograma",   Icon: Calendar },
              { value: "aditivos",     label: "Aditivos",     Icon: FilePlus2 },
              { value: "autorizacoes", label: "Autorizações", Icon: ShieldCheck },
            ];
            const isActive = items.some(i => i.value === activeTab);
            return (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap
                    ${isActive ? "bg-background text-foreground shadow-sm border" : "text-muted-foreground hover:text-foreground hover:bg-background/60"}`}>
                ...
                <DropdownMenuContent align="start" className="w-48">
                  <DropdownMenuLabel className="text-xs text-muted-foreground">Gestão Contratual</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {items.map(({ value, label, Icon }) => (
                    <DropdownMenuItem key={value} onClick={() => setActiveTab(value)}
                      className={`flex items-center gap-2 cursor-pointer ${activeTab === value ? "bg-accent font-medium" : ""}`}>
          ...
          {/* Grupo: Recursos & Operação */}
          {(() => {
            const items = [
              { value: "rh",          label: "RH",          Icon: Users },
              { value: "sst",         label: "SST",         Icon: Shield },
            ...
            return (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-48">
                  <DropdownMenuLabel className="text-xs text-muted-foreground">Recursos & Operação</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {items.map(({ value, label, Icon }) => (
                    <DropdownMenuItem key={value} onClick={() => setActiveTab(value)}
                      className={`flex items-center gap-2 cursor-pointer ${activeTab === value ? "bg-accent font-medium" : ""}`}>
          ...
          {(() => {
            const items = [
              { value: "documentos-tecnicos",  label: "Docs Técnicos",  Icon: BookOpen },
              { value: "atas-reuniao",         label: "Reuniões",       Icon: MessageSquare },
              { value: "riscos",               label: "Riscos",         Icon: ShieldAlert },
              { value: "decisoes",             label: "Decisões",       Icon: Lightbulb },
              { value: "entregaveis",          label: "Entregáveis",    Icon: PackageCheck },
              { value: "vinculos-documentos",  label: "Vínculos",       Icon: Network },
            ];
            const isActive = items.some(i => i.value === activeTab);
            return (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-48">
                  <DropdownMenuLabel className="text-xs text-muted-foreground">Documentação Técnica</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {items.map(({ value, label, Icon }) => (
                    <DropdownMenuItem key={value} onClick={() => setActiveTab(value)}
                      className={`flex items-center gap-2 cursor-pointer ${activeTab === value ? "bg-accent font-medium" : ""}`}>
        ...
        <TabsContent value="campanhas">
          <CampanhasTab empreendimentoId={parseInt(id!)} />
        </TabsContent>
        ...
        <TabsContent value="documentos-tecnicos">
          <DocumentosTecnicosTab empreendimentoId={parseInt(id!)} empreendimentoNome={project?.nome} />
        </TabsContent>
        ...
        <TabsContent value="vinculos-documentos">
          <VinculosDocumentosTab empreendimentoId={parseInt(id!)} />
        </TabsContent>
      </Tabs>
    </div>
  );

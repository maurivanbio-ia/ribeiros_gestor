...
export interface IStorage {
  ...
  // ========== PROJETO EQUIPE SYNC ==========
  syncProjetoEquipe(projetoId: number, membroIds: number[], unidade: string): Promise<void>;

  // ========== PROJETO EQUIPAMENTOS ==========
  getEquipamentossDoProjeto(projetoId: number): Promise<Equipamento[]>;
  syncProjetoEquipamentos(projetoId: number, equipamentoIds: number[]): Promise<void>;

  // ========== PROJETO VEICULOS ==========
  getVeiculosDoProjeto(projetoId: number): Promise<Veiculo[]>;
  syncProjetoVeiculos(projetoId: number, veiculoIds: number[]): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteEmpreendimento(id: number): Promise<void> {
    // Cascata completa em transação — ordem topológica dos FKs mapeados via DB
    await db.transaction(async (tx) => {

      // ── Nível 1: filhos de licencas_ambientais ────────────────────────────
      await tx.execute(sql`DELETE FROM condicionantes WHERE licenca_id IN (SELECT id FROM licencas_ambientais WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM entregas WHERE licenca_id IN (SELECT id FROM licencas_ambientais WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM processos_monitorados WHERE licenca_id IN (SELECT id FROM licencas_ambientais WHERE empreendimento_id = ${id})`);
      // documentos.licenca_id é nullable — apenas anula o vínculo
      await tx.execute(sql`UPDATE documentos SET licenca_id = NULL WHERE licenca_id IN (SELECT id FROM licencas_ambientais WHERE empreendimento_id = ${id})`);
      await tx.delete(licencasAmbientais).where(eq(licencasAmbientais.empreendimentoId, id));

      // ── Nível 2: filhos de campanhas ─────────────────────────────────────
      await tx.execute(sql`DELETE FROM campanha_intercorrencias WHERE campanha_id IN (SELECT id FROM campanhas WHERE empreendimento_id = ${id})`);
      // recibos referencia campanhas E tem empreendimento_id — deletar antes
      await tx.execute(sql`DELETE FROM recibos WHERE empreendimento_id = ${id}`);
      // financeiro_lancamentos.campanha_id nullable — anula
      await tx.execute(sql`UPDATE financeiro_lancamentos SET campanha_id = NULL WHERE campanha_id IN (SELECT id FROM campanhas WHERE empreendimento_id = ${id})`);
      await tx.delete(campanhas).where(eq(campanhas.empreendimentoId, id));

      // ── Nível 3: filhos de projetos ───────────────────────────────────────
      await tx.execute(sql`DELETE FROM projeto_intercorrencias WHERE projeto_id IN (SELECT id FROM projetos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM membros_projetos WHERE projeto_id IN (SELECT id FROM projetos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM projeto_equipamentos WHERE projeto_id IN (SELECT id FROM projetos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM projeto_veiculos WHERE projeto_id IN (SELECT id FROM projetos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM pedidos_reembolso WHERE projeto_id IN (SELECT id FROM projetos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`UPDATE financeiro_lancamentos SET projeto_id = NULL WHERE projeto_id IN (SELECT id FROM projetos WHERE empreendimento_id = ${id})`);
      await tx.delete(projetos).where(eq(projetos.empreendimentoId, id));

      // ── Nível 4: filhos de contratos ─────────────────────────────────────
      await tx.execute(sql`DELETE FROM contrato_aditivos WHERE contrato_id IN (SELECT id FROM contratos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM aditivos_contratos WHERE contrato_id IN (SELECT id FROM contratos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM contrato_pagamentos WHERE contrato_id IN (SELECT id FROM contratos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM contrato_documentos WHERE contrato_id IN (SELECT id FROM contratos WHERE empreendimento_id = ${id})`);
      // demandas.contrato_id e documentos.contrato_id são nullable — anula
      await tx.execute(sql`UPDATE demandas SET contrato_id = NULL WHERE contrato_id IN (SELECT id FROM contratos WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`UPDATE documentos SET contrato_id = NULL WHERE contrato_id IN (SELECT id FROM contratos WHERE empreendimento_id = ${id})`);
      await tx.delete(contratos).where(eq(contratos.empreendimentoId, id));

      // ── Nível 5: filhos de datasets ───────────────────────────────────────
      await tx.execute(sql`DELETE FROM dataset_versoes WHERE dataset_id IN (SELECT id FROM datasets WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM dataset_audit_trail WHERE dataset_id IN (SELECT id FROM datasets WHERE empreendimento_id = ${id})`);
      await tx.delete(datasets).where(eq(datasets.empreendimentoId, id));

      // ── Nível 6: filhos de colaboradores ─────────────────────────────────
      await tx.execute(sql`DELETE FROM seg_documentos_colaboradores WHERE colaborador_id IN (SELECT id FROM colaboradores WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM asos_ocupacionais WHERE colaborador_id IN (SELECT id FROM colaboradores WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM cat_acidentes WHERE colaborador_id IN (SELECT id FROM colaboradores WHERE empreendimento_id = ${id})`);
      await tx.delete(colaboradores).where(eq(colaboradores.empreendimentoId, id));

      // ── Nível 7: filhos de demandas ───────────────────────────────────────
      await tx.execute(sql`DELETE FROM comentarios_demandas WHERE demanda_id IN (SELECT id FROM demandas WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM subtarefas_demandas WHERE demanda_id IN (SELECT id FROM demandas WHERE empreendimento_id = ${id})`);
      await tx.execute(sql`DELETE FROM historico_demandas_movimentacoes WHERE demanda_id IN (SELECT id FROM demandas WHERE empreendimento_id = ${id})`);
      await tx.delete(demandas).where(eq(demandas.empreendimentoId, id));

      // ── Nível 8: tabelas diretas do empreendimento (sem filhos próprios) ──
      // tarefas: tarefa_atualizacoes e registro_horas têm onDelete: cascade
      await tx.execute(sql`DELETE FROM tarefas WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM campo_registros WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM ai_documents WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM amostras WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM atas_reuniao WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM autorizacoes WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM decisoes WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM documento_vinculos WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM documentos WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM empreendimento_responsaveis WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM entregaveis WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM minutas WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM modelos_layouts WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM newsletter_destaques WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM pareceres_tecnicos WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM pedidos_reembolso WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM planos_trabalho WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM processos_monitorados WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM propostas_comerciais WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM relacionamentos_cliente WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM riscos WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`DELETE FROM whatsapp_message_logs WHERE empreendimento_id = ${id}`);
      // blog_artigos e publicacoes: empreendimento_id é nullable — anula o vínculo
      await tx.execute(sql`UPDATE blog_artigos SET empreendimento_id = NULL WHERE empreendimento_id = ${id}`);
      await tx.execute(sql`UPDATE publicacoes SET empreendimento_id = NULL WHERE empreendimento_id = ${id}`);

      // ── Nível 9: tabelas Drizzle importadas ───────────────────────────────
  ...
  async createLicenca(licenca: InsertLicencaAmbiental): Promise<LicencaAmbiental> {
    // Quando semDataValidade=true, a validade é nula e o status é 'ativa'
    const validadeParaStatus = (licenca as any).semDataValidade ? null : licenca.validade;
    const status = this.calculateLicenseStatus(validadeParaStatus);
  ...
  async updateLicenca(id: number, licenca: Partial<InsertLicencaAmbiental>): Promise<LicencaAmbiental> {
    ...
    if (!(current as any).finalizada) {
      const semDataValidade = (licenca as any).semDataValidade ?? (current as any).semDataValidade;
      const validadeParaStatus = semDataValidade ? null : (licenca.validade ?? current.validade);
      updateData.status = this.calculateLicenseStatus(validadeParaStatus);
    }
  ...
  async getRhStats(unidade: string, empreendimentoId?: number): Promise<{ total: number; ativos: number; afastados: number }> {
    ...
    return {
      total: rh.length,
      ativos: rh.filter((r: any) => r.status === 'ativo').length,
      afastados: rh.filter((r: any) => r.status === 'afastado' || r.status === 'inativo').length,
    };
  }
  ...
  private calculateLicenseStatus(validade: string | null | undefined): string {
    // Quando não há data de validade (semDataValidade=true), a licença é ativa indefinidamente
  ...
  async getLancamentos(filters?: {
    ...
    if (filters?.projetoId) {
      conditions.push(eq(financeiroLancamentos.projetoId, filters.projetoId));
    }
    if (filters?.campanhaId) {
      conditions.push(eq(financeiroLancamentos.campanhaId, filters.campanhaId));
    }
  ...
  async getFinancialStats(empreendimentoId?: number, startDate?: Date, endDate?: Date, unidade?: string): Promise<{
    ...
    const totalDespesas = lancamentos
      .filter(l =>
        ...
        (l.tipo === "reembolso" && (l.status === "aprovado" || l.status === "pago"))
      )
      .reduce((sum, l) => sum + Number(l.valor), 0);
    ...
    // Get categories, empreendimentos, projetos, campanhas for breakdown
    ...
    const projetosList = await db.select().from(projetos);
    ...
    const porEmpreendimento = empreendimentosList.map(emp => {
      const empLancamentos = lancamentos.filter(l => l.empreendimentoId === emp.id);
    ...
    // Helper: lançamento conta como receita efetiva
    const isReceita = (l: any) => l.tipo === "receita" && l.status === "pago";
    // Helper: lançamento conta como despesa efetiva (despesas pagas + reembolsos aprovados ou pagos)
    const isDespesa = (l: any) =>
      (l.tipo === "despesa" && l.status === "pago") ||
      (l.tipo === "reembolso" && (l.status === "aprovado" || l.status === "pago"));

    // Per projeto
    const porProjeto = projetosList.map(proj => {
      const projLancamentos = lancamentos.filter(l => l.projetoId === proj.id);
      const receitas = projLancamentos.filter(isReceita).reduce((sum, l) => sum + Number(l.valor), 0);
      const despesas = projLancamentos.filter(isDespesa).reduce((sum, l) => sum + Number(l.valor), 0);
      return { projeto: proj.nome, projetoId: proj.id, receitas, despesas, lucro: receitas - despesas };
    }).filter(item => item.receitas > 0 || item.despesas > 0);

    // Per campanha
    const porCampanha = campanhasList.map(camp => {
      const campLancamentos = lancamentos.filter(l => l.campanhaId === camp.id);
      const receitas = campLancamentos.filter(isReceita).reduce((sum, l) => sum + Number(l.valor), 0);
      const despesas = campLancamentos.filter(isDespesa).reduce((sum, l) => sum + Number(l.valor), 0);
      return { campanha: camp.nome, campanhaId: camp.id, receitas, despesas, lucro: receitas - despesas };
    }).filter(item => item.receitas > 0 || item.despesas > 0);
  ...
  // ========== PROJETO EQUIPE SYNC ==========
  async syncProjetoEquipe(projetoId: number, membroIds: number[], unidade: string): Promise<void> {
    await db.delete(membrosProjetos).where(eq(membrosProjetos.projetoId, projetoId));
    if (membroIds.length > 0) {
      await db.insert(membrosProjetos).values(
        membroIds.map(mid => ({ projetoId, membroEquipeId: mid, unidade }))
      );
    }
  }

  // ========== PROJETO EQUIPAMENTOS ==========
  async getEquipamentossDoProjeto(projetoId: number): Promise<Equipamento[]> {
    const rows = await db
      .select({ equip: equipamentos })
      .from(projetoEquipamentos)
      .innerJoin(equipamentos, eq(projetoEquipamentos.equipamentoId, equipamentos.id))
      .where(eq(projetoEquipamentos.projetoId, projetoId));
    return rows.map(r => r.equip);
  }

  async syncProjetoEquipamentos(projetoId: number, equipamentoIds: number[]): Promise<void> {
    await db.delete(projetoEquipamentos).where(eq(projetoEquipamentos.projetoId, projetoId));
    if (equipamentoIds.length > 0) {
      await db.insert(projetoEquipamentos).values(
        equipamentoIds.map(eid => ({ projetoId, equipamentoId: eid }))
      );
    }
  }

  // ========== PROJETO VEICULOS ==========
  async getVeiculosDoProjeto(projetoId: number): Promise<Veiculo[]> {
    const rows = await db
      .select({ v: veiculos })
      .from(projetoVeiculos)
      .innerJoin(veiculos, eq(projetoVeiculos.veiculoId, veiculos.id))
      .where(eq(projetoVeiculos.projetoId, projetoId));
    return rows.map(r => r.v);
  }

  async syncProjetoVeiculos(projetoId: number, veiculoIds: number[]): Promise<void> {
    await db.delete(projetoVeiculos).where(eq(projetoVeiculos.projetoId, projetoId));
    if (veiculoIds.length > 0) {
      await db.insert(projetoVeiculos).values(
        veiculoIds.map(vid => ({ projetoId, veiculoId: vid }))
      );
    }
  }

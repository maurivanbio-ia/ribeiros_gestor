...
const SECONDARY_COLOR = [178, 205, 225] as [number, number, number]; // #B2CDE1
...
async function loadCoverImg(name: string): Promise<string | null> {
  try {
    const imgPath = path.join(process.cwd(), 'client', 'public', 'images', name);
...
// AmbientIA logo — loaded once at module level
...
export async function generatePlatformReportPDF(options: ReportOptions = {}): Promise<Buffer> {
  ...
  const [data, coverBg] = await Promise.all([
    fetchReportData(unidade),
    loadCoverImg('pdf-cover-avifauna.png'),
  ]);
  ...
  if (LOGO_BASE64) {
    doc.addImage(`data:image/png;base64,${LOGO_BASE64}`, 'PNG', 18, 8, 56, 24);
  } else {
    doc.setFontSize(22);
  ...
  metaItems.forEach((m, i) => {
    ...
    doc.text(m.label, mx, mdBaseY);
    doc.setFontSize(10);
    doc.setFont(undefined as any, 'bold');
    doc.setTextColor(10, 28, 50);
    doc.text(m.value, mx, mdBaseY + 7);
  });
  ...
  // ── Helper: page header band (called after each doc.addPage()) ──────────
  const addPageHeader = (sectionName: string = '') => {
    ...
    doc.setTextColor(255, 255, 255);
    if (LOGO_BASE64) {
      doc.addImage(`data:image/png;base64,${LOGO_BASE64}`, 'PNG', MARGINS.left, 1, 16, 9);
    } else {
      doc.text('AmbientIA', MARGINS.left, 7.5);
    }
    if (sectionName) {
      doc.setFont(undefined as any, 'normal');
  ...
  const addSectionTitle = (title: string, color: [number, number, number] = ECOBRASIL_COLORS.blue) => {
    ...
    const lightBg: [number, number, number] =
      color === ECOBRASIL_COLORS.red    ? [255, 242, 242] :
      color === ECOBRASIL_COLORS.yellow ? [255, 251, 232] :
      color === ECOBRASIL_COLORS.green  ? [234, 246, 239] :
      color === ECOBRASIL_COLORS.purple ? [230, 245, 248] :
                                          [234, 244, 253];
  ...
  doc.addPage();
  addPageHeader('Resumo Executivo');
  yPos = MARGINS.top;
  ...
  doc.addPage();
  addPageHeader('Análise Operacional');
  yPos = MARGINS.top;
  ...
  doc.addPage();
  addPageHeader('Análise de Projetos');
  yPos = MARGINS.top;
  ...
  doc.addPage();
  addPageHeader('Análise Financeira');
  yPos = MARGINS.top;
  ...
  if (data.financeiro.evolucaoMensal && data.financeiro.evolucaoMensal.length > 0) {
    ...
    yPos = drawBarChart(
      ...
      { label1: 'Receitas', label2: 'Despesas', color1: ECOBRASIL_COLORS.green, color2: ECOBRASIL_COLORS.red }
    );
  ...
  doc.addPage();
  addPageHeader('Dados Detalhados');
  yPos = MARGINS.top;
  ...
  // ── Footer on all pages (skip page 1 which has its own bottom bar) ────────
  ...
  for (let i = 2; i <= totalPages; i++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(...SECONDARY_COLOR);
    doc.text(`AmbientIA — Relatório 360° da Plataforma`, MARGINS.left, pageHeight - 3.5);
    doc.text(`Página ${i} de ${totalPages}`, pageWidth - MARGINS.right, pageHeight - 3.5, { align: 'right' });
  }
...
export async function generateFinanceReportPDF(options: ReportOptions = {}): Promise<Buffer> {
  ...
  const [data, coverBg] = await Promise.all([
    fetchReportData(unidade),
    loadCoverImg('pdf-cover-quelonios.png'),
  ]);
  ...
  // ── COVER PAGE — SGAI Premium with marine turtle background ──────────────
  // 1. Nature background photo (16:9 at full A4 width = 118mm tall)
  if (coverBg) {
    doc.addImage(coverBg, 'PNG', 0, 0, pageWidth, 118);
  }

  // 2. Dark navy overlay from y=98 downward
  doc.setFillColor(10, 28, 50);
  doc.rect(0, 98, pageWidth, pageHeight - 98, 'F');

  // 3. Forest-green accent stripe at transition
  doc.setFillColor(...ECOBRASIL_COLORS.green);
  doc.rect(0, 98, pageWidth, 3, 'F');

  // 4. AmbientIA logo / wordmark
  const finWY = 112;
  if (LOGO_BASE64) {
    doc.addImage(`data:image/png;base64,${LOGO_BASE64}`, 'PNG', 18, finWY, 48, 20);
  } else {
    doc.setFontSize(28);
    doc.setFont(undefined as any, 'bold');
    doc.setTextColor(255, 255, 255);
    doc.text('AmbientIA', 20, finWY + 14);
  }
  ...
  doc.setFont(undefined as any, 'normal');
  doc.setTextColor(...SECONDARY_COLOR);
  doc.text('Plataforma Inteligente de Gestão para Consultorias Ambientais', 20, finWY + 24);

  // 6. Blue thin separator
  doc.setFillColor(...ECOBRASIL_COLORS.blue);
  doc.rect(20, finWY + 15, pageWidth - 40, 0.8, 'F');

  // 7. Report type label (amber, letter-spaced)
  doc.setFontSize(7.5);
  doc.setFont(undefined as any, 'bold');
  doc.setTextColor(245, 168, 0);
  doc.setCharSpace(2);
  doc.text('RELATÓRIO FINANCEIRO', 20, finWY + 24);
  doc.setCharSpace(0);
  ...
  doc.setFont(undefined as any, 'bold');
  doc.setTextColor(255, 255, 255);
  doc.text('Consolidado', 20, finWY + 38);

  // 9. Subtitle with period
  doc.setFontSize(13);
  doc.setFont(undefined as any, 'normal');
  doc.setTextColor(...SECONDARY_COLOR);
  doc.text(`${monthNames[currentMonth - 1]} de ${currentYear}`, 20, finWY + 52);

  // 10. Green divider
  doc.setFillColor(...ECOBRASIL_COLORS.green);
  doc.rect(20, finWY + 58, 60, 2, 'F');

  // 11. Metadata
  const finUnidadeLabel = unidade === 'goiania' ? 'Goiânia' :
                          unidade === 'salvador' ? 'Salvador' :
                          unidade === 'luiz_eduardo_magalhaes' ? 'Luiz Eduardo Magalhães' :
                          'Todas as Unidades';
  const finMetaItems = [
    { label: 'UNIDADE', value: finUnidadeLabel },
    { label: 'GERADO EM', value: new Date().toLocaleDateString('pt-BR') },
  ];
  const finMdBaseY = finWY + 72;
  finMetaItems.forEach((m, i) => {
    const mx = 20 + i * 95;
    doc.setFontSize(6);
    doc.setFont(undefined as any, 'bold');
    doc.setTextColor(...SECONDARY_COLOR);
    doc.text(m.label, mx, finMdBaseY);
    doc.setFontSize(10);
    doc.setFont(undefined as any, 'bold');
    doc.setTextColor(255, 255, 255);
    doc.text(m.value, mx, finMdBaseY + 8);
  });
  ...
  doc.setFont(undefined as any, 'normal');
  doc.setTextColor(...SECONDARY_COLOR);
  doc.text(' › Relatório Financeiro Consolidado', MARGINS.left + 20, 7.5);
  yPos = MARGINS.top;
  ...
  doc.setFont(undefined as any, 'normal');
  doc.setTextColor(...SECONDARY_COLOR);
  doc.text(' › Relatório Financeiro — Categorias e Projetos', MARGINS.left + 20, 7.5);
  yPos = MARGINS.top;
  ...
  doc.setFont(undefined as any, 'normal');
  doc.setTextColor(...SECONDARY_COLOR);
  doc.text(' › Relatório Financeiro — Gráficos', MARGINS.left + 20, 7.5);
  yPos = MARGINS.top;
  ...
  // ── Footer on all pages (skip page 1 which has its own bottom bar) ────────
  const totalFinPages = doc.getNumberOfPages();
  for (let i = 2; i <= totalFinPages; i++) {
    doc.setPage(i);
    doc.setFillColor(...ECOBRASIL_COLORS.green);
    doc.rect(0, pageHeight - 10, pageWidth, 1.5, 'F');
    doc.setFillColor(...ECOBRASIL_COLORS.blue);
    ...
    doc.setFontSize(7);
    doc.setTextColor(...SECONDARY_COLOR);
    doc.text(`AmbientIA — Relatório Financeiro Consolidado`, MARGINS.left, pageHeight - 3.5);
    doc.text(`Página ${i} de ${totalFinPages}`, pageWidth - MARGINS.right, pageHeight - 3.5, { align: 'right' });
  }
...
export async function sendReportByEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: '"AmbientIA - Relatórios" <maurivan.bio@gmail.com>',
      to: recipientEmail,
      subject,
      text: body,
      html: htmlBody,

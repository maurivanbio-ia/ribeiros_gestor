...
import { uploadBytesWithRetry } from "./objectStorageHelper";
...
export class ObjectStorageService {
  ...
  // Gets a generic upload URL for any document file in Gestão de Dados
  async getDocumentUploadURL(originalName: string): Promise<{ uploadUrl: string; filePath: string }> {
    const privateObjectDir = this.getPrivateObjectDir();
    const objectId = randomUUID();
    const ext = originalName.split('.').pop() || 'bin';
    const fullPath = `${privateObjectDir}/documentos/${objectId}.${ext}`;
    const { bucketName, objectName } = parseObjectPath(fullPath);
    const uploadUrl = await signObjectURL({ bucketName, objectName, method: "PUT", ttlSec: 900 });
    return { uploadUrl, filePath: `/files/documentos/${objectId}.${ext}` };
  }
...
export async function uploadFile(buffer: Buffer, key: string, contentType: string): Promise<{ url: string; publicUrl: string }> {
  const success = await uploadBytesWithRetry(key, buffer, { contentType });
  if (!success) {
    throw new Error("Erro ao fazer upload do arquivo para o Object Storage");
  }
  const url = `/files/${key}`;
  return {
    url,

...
class WebSocketService {
  ...
  initialize(server: Server) {
    try {
      ...
      console.error('[WebSocket] Falha ao criar servidor WebSocket:', err);
      return;
    }

    this.wss.on('error', (err: NodeJS.ErrnoException) => {
      if (err.code === 'EADDRNOTAVAIL' || err.code === 'EADDRINUSE') {
        console.warn('[WebSocket] Endereço indisponível — notificações em tempo real desativadas:', err.message);
      } else {
        console.error('[WebSocket] Erro no servidor WebSocket:', err);
      }
    });

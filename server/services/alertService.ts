...
import { db } from '../db';
import { users, campanhas, cronogramaItens, entregaveis } from '@shared/schema';
...
export class AlertService {
  ...
  private readonly DEFAULT_CONFIGS = [
    ...
    // ── Alertas de Cronograma (campanhas, relatórios, marcos, etapas) ──
    { tipo: 'cronograma', diasAviso: 130, ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'cronograma', diasAviso: 90,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'cronograma', diasAviso: 60,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'cronograma', diasAviso: 30,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'cronograma', diasAviso: 15,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'cronograma', diasAviso: 7,   ativo: true, enviarEmail: false, enviarWhatsapp: false },

    // ── Alertas de Campanhas de Monitoramento ──
    { tipo: 'campanha', diasAviso: 130, ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'campanha', diasAviso: 90,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'campanha', diasAviso: 60,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'campanha', diasAviso: 30,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'campanha', diasAviso: 15,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'campanha', diasAviso: 7,   ativo: true, enviarEmail: false, enviarWhatsapp: false },

    // ── Alertas de Entregáveis ──
    { tipo: 'entregavel', diasAviso: 130, ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'entregavel', diasAviso: 90,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'entregavel', diasAviso: 60,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'entregavel', diasAviso: 30,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'entregavel', diasAviso: 15,  ativo: true, enviarEmail: false, enviarWhatsapp: false },
    { tipo: 'entregavel', diasAviso: 7,   ativo: true, enviarEmail: false, enviarWhatsapp: false },
  ];
  ...
  // Inicializa configurações padrão de alertas — garante que TODAS existam
    ...
    try {
      ...
      for (const config of this.DEFAULT_CONFIGS) {
        const exists = existingConfigs.some(
          e => e.tipo === config.tipo && e.diasAviso === config.diasAviso
        );
        if (!exists) {
      ...
      if (added > 0) {
        console.log(`[Alertas] ${added} configurações novas adicionadas`);
      }
    } catch (error) {
  ...
  private async processAlertsForConfig(config: AlertConfig): Promise<void> {
    try {
      let items: any[] = [];
      let getDateField: (item: any) => string | null;
      ...
      switch (config.tipo) {
        ...
        case 'campanha':
          items = await db.select().from(campanhas);
          getDateField = (item) => item.periodoFim;
          break;

        // ── Itens de Cronograma (campanhas, relatórios, marcos, etapas) ──
        case 'cronograma':
          items = await db
            .select()
            .from(cronogramaItens)
            .where(eq(cronogramaItens.concluido, false));
          getDateField = (item) => item.dataFim;
          break;
        ...
        case 'entregavel':
          items = await db
            ...
            .where(
              and(
                ne(entregaveis.status, 'aprovado'),
                ne(entregaveis.status, 'cancelado')
              )
            );
          getDateField = (item) => item.prazo;
          break;
      ...
      for (const item of items) {
        ...
        if (dateValue) {
          await this.checkItemForAlert(item, config, dateValue);
        }
      }
    } catch (error) {
  ...
  // Usa janela de ±1 dia para garantir captura mesmo se o cron falhou por 1 dia
  private async checkItemForAlert(
    ...
    try {
      ...
      // Dispara na janela: diasAviso até diasAviso-1 (janela de 2 dias para robustez)
      const inWindow = diasRestantes <= config.diasAviso && diasRestantes >= config.diasAviso - 1;
      if (!inWindow) return;

      // Verifica se já foi enviado alerta para este item/prazo
      const jaEnviado = await storage.checkAlertHistory(config.tipo, item.id, config.diasAviso);
      if (!jaEnviado) {
  ...
  private async sendAlertForItem(
    ...
    try {
      // Criar notificação interna no sistema (sempre)
      await this.createInternalNotification(item, config.tipo, diasRestantes);

      // Salvar histórico para tipos internos (sem email) — evita re-disparo
      if (!config.enviarEmail && !config.enviarWhatsapp) {
        await this.saveAlertHistory(item.id, config, 'sistema', 'enviado');
        console.log(`[Alerta interno] ${config.tipo} ID ${item.id} — ${diasRestantes} dias`);
        return;
      }
      ...
      if (config.enviarEmail) {
        try {
          ...
          console.log(`Email de alerta enviado para ${emailDestinos.length} destinatários - ${config.tipo} ID ${item.id}`);
        } catch (error) {
...
📧 ecobrasiloficial@gmail.com
  ...
  private async createInternalNotification(
    ...
    try {
      // Determina o campo de data correto por tipo
      const dateField =
        tipo === 'licenca'     ? item.validade :
        tipo === 'campanha'    ? item.periodoFim :
        tipo === 'cronograma'  ? item.dataFim :
        item.prazo;
      ...
      const unidadeEmpreendimento = (empreendimento?.unidade || item.unidade) as string | null;
      ...
      // ── Preparar título e mensagem por tipo ──
      const diasLabel = diasRestantes <= 0 ? 'VENCIDO' : `${diasRestantes} dias`;
      let titulo = '';
      let mensagem = '';
      let tipoLabel = '';
      const link: string = empreendimentoId ? `/empreendimentos/${empreendimentoId}` : '/cronograma';
      ...
      switch (tipo) {
        case 'licenca':
          ...
          break;
        case 'condicionante':
          tipoLabel = 'Condicionante';
          titulo = diasRestantes <= 0 ? 'Condicionante Vencida' : 'Condicionante Vencendo';
          mensagem = `${item.descricao || 'Condicionante'} — ${nomeEmpreendimento}: ${diasLabel}`;
          break;
        case 'entrega':
          tipoLabel = 'Entrega';
          titulo = diasRestantes <= 0 ? 'Entrega Vencida' : 'Entrega Vencendo';
          mensagem = `${item.titulo || item.descricao || 'Entrega'} — ${nomeEmpreendimento}: ${diasLabel}`;
          break;

        // ── Novos tipos ──
        case 'campanha': {
          tipoLabel = 'Campanha';
          const recLabel = item.recorrencia && item.recorrencia !== 'nenhuma'
            ? ` (${item.recorrencia})` : '';
          titulo = diasRestantes <= 0
            ? `Campanha Vencida${recLabel}`
            : `Início de Campanha em ${diasLabel}${recLabel}`;
          mensagem = `${item.nome || 'Campanha'} — ${nomeEmpreendimento}: ${diasLabel}`;
          break;
        }
        case 'cronograma': {
          const tipoItem = item.tipo || 'etapa';
          const tipoItemLabel =
            tipoItem === 'campanha'  ? 'Campanha' :
            tipoItem === 'relatorio' ? 'Relatório' :
            tipoItem === 'marco'     ? 'Marco' : 'Etapa';
          tipoLabel = tipoItemLabel;
          const recLabel = item.recorrencia && item.recorrencia !== 'nenhuma'
            ? ` (${item.recorrencia})` : '';
          titulo = diasRestantes <= 0
            ? `${tipoItemLabel} Vencido${recLabel}`
            : `${tipoItemLabel} em ${diasLabel}${recLabel}`;
          mensagem = `${item.titulo || tipoItemLabel} — ${nomeEmpreendimento}: ${diasLabel}`;
          break;
        }
        case 'entregavel': {
          tipoLabel = 'Entregável';
          titulo = diasRestantes <= 0 ? 'Entregável Vencido' : `Entregável em ${diasLabel}`;
          mensagem = `${item.titulo || 'Entregável'} — ${nomeEmpreendimento}: ${diasLabel}`;
          break;
        }
      }
      ...
      const defaultLink =
        tipo === 'cronograma' || tipo === 'campanha' || tipo === 'entregavel'
          ? (resolvedEmpreendimentoId ? `/empreendimentos/${resolvedEmpreendimentoId}` : '/cronograma')
          : (resolvedEmpreendimentoId ? `/empreendimentos/${resolvedEmpreendimentoId}` : '/empreendimentos');
  ...
  private async saveAlertHistory(
    itemId: number,
    config: AlertConfig,
    tipoNotificacao: 'email' | 'whatsapp' | 'sistema',
    status: 'enviado' | 'erro',
    erro?: string
  ): Promise<void> {

...
import { db } from '../db';
import { aiConversations, aiLogs, campoRegistros, projetos, campanhas } from '@shared/schema';
...
function detectTopics(msg: string): Set<string> {
  ...
  if (/empreendimento|fazenda|complexo|sitio|usina|empresa|plant/.test(m)) topics.add('empreendimentos');
  if (/\bprojeto(s)?\b/.test(m)) { topics.add('empreendimentos'); topics.add('projetos'); }
  if (/\bcampanha(s)?\b/.test(m)) topics.add('campanhas');
  ...
  if (/amostra|coleta.*lab|laboratorio|laboratório|parâmetro.*lab|laudo|ensaio|resultado.*analise|analise.*agua|analise.*solo/.test(m)) topics.add('amostras');
  if (/\bcampo\b|monitoramento.*campo|registro.*campo|fauna|flora|avifauna|herpeto|ictio|mastofauna|fitossocio|biometria|anilhamento|espécie|especie|biolog|taxonom|grupo.*taxonom|ponto.*escuta|unidade.*amostral/.test(m)) topics.add('campo');
  if (/monitoramento(?!.*campo)/.test(m)) { topics.add('amostras'); topics.add('campo'); }
  ...
  if (/document|arquivo|pdf|docx|xlsx|laudo|ofício|oficio|parecer|notif|ata\b|protocolo|gestão documental|gestão de dados|relat[oó]rio|minuta|plano de trabalho/.test(m)) topics.add('documentos');
...
async function buildDatabaseContext(unidade: string, message: string, empreendimentoId?: number): Promise<string> {
  ...
  try {
    ...
    if (topics.has('amostras')) {
      const amostrasList = await storage.getAmostras({ unidade, ...(empreendimentoId ? { empreendimentoId } : {}) });
      const concluidas = amostrasList.filter((a: any) => a.status === 'concluida').length;
      const emAnalise = amostrasList.filter((a: any) => a.status === 'em_analise' || a.status === 'enviada_lab').length;
      const pendentes = amostrasList.filter((a: any) => a.status === 'coletada').length;
      sections.push(
        `## AMOSTRAS LABORATORIAIS (total: ${amostrasList.length} | concluídas: ${concluidas} | em análise: ${emAnalise} | aguardando: ${pendentes})\n` +
        amostrasList.slice(0, 40).map((a: any, i: number) => {
          const parts: string[] = [];
          parts.push(`  ${i + 1}. [${a.codigo}] ${a.tipo}${a.subtipo ? `/${a.subtipo}` : ''}`);
          if (a.pontoColeta) parts.push(`Ponto: ${a.pontoColeta}`);
          if (a.dataColeta) parts.push(`Coleta: ${a.dataColeta}`);
          if (a.status) parts.push(`Status: ${a.status}`);
          if (a.laboratorioNome) parts.push(`Lab: ${a.laboratorioNome}`);
          if (a.dataResultado) parts.push(`Resultado em: ${a.dataResultado}`);
          if (a.parametrosAnalisados) parts.push(`Parâmetros: ${a.parametrosAnalisados}`);
          if (a.temperaturaColeta) parts.push(`Temp: ${a.temperaturaColeta}°C`);
          if (a.phColeta) parts.push(`pH: ${a.phColeta}`);
          if (a.condutividade) parts.push(`Condutividade: ${a.condutividade}`);
          if (a.intercorrencias) parts.push(`Intercorrências: ${a.intercorrencias}`);
          if (a.observacoes) parts.push(`Obs: ${String(a.observacoes).substring(0, 120)}`);
          return parts.join(' | ');
        }).join('\n')
      );
    }

    // ── CAMPO — MONITORAMENTO DE CAMPO (fauna/flora/biologia) ────────────────
    if (topics.has('campo')) {
      try {
        const conditions: any[] = [eq(campoRegistros.unidade, unidade)];
        if (empreendimentoId) conditions.push(eq(campoRegistros.empreendimentoId, empreendimentoId));
        const campoData = await db.select().from(campoRegistros)
          .where(and(...conditions))
          .orderBy(desc(campoRegistros.criadoEm))
          .limit(80);

        // Estatísticas por grupo taxonômico
        const gruposMap: Record<string, number> = {};
        const campanhas = new Set<string>();
        const especiesSet = new Set<string>();
        const ameacadas: string[] = [];

        for (const r of campoData) {
          const g = r.grupoTaxonomico || 'Não identificado';
          gruposMap[g] = (gruposMap[g] || 0) + 1;
          if (r.campanha) campanhas.add(r.campanha);
          if (r.nomeCientifico) especiesSet.add(r.nomeCientifico);
          const ameaca = [r.iucn, r.ibamaMma, r.cites].filter(Boolean).join('/');
          if (ameaca && ameaca.match(/VU|EN|CR|I|II|III/i) && r.nomeCientifico) {
            ameacadas.push(`${r.nomeCientifico} (${ameaca})`);
          }
        }

        const resumoGrupos = Object.entries(gruposMap)
          .sort((a, b) => b[1] - a[1])
          .map(([g, n]) => `${g}: ${n} registros`)
          .join(' | ');

        let campoSection = `## MONITORAMENTO DE CAMPO — FAUNA/FLORA (total: ${campoData.length} registros | ${especiesSet.size} espécies | ${campanhas.size} campanhas)\n`;
        campoSection += `Grupos: ${resumoGrupos || '(nenhum)'}\n`;
        if (campanhas.size > 0) campoSection += `Campanhas: ${Array.from(campanhas).join(', ')}\n`;
        if (ameacadas.length > 0) campoSection += `Espécies ameaçadas/listadas: ${ameacadas.slice(0, 10).join('; ')}\n`;
        campoSection += `\nRegistros recentes:\n`;
        campoSection += campoData.slice(0, 40).map((r, i) => {
          const parts: string[] = [];
          parts.push(`  ${i + 1}. [${r.grupoTaxonomico}] ${r.nomeCientifico || '(sp.)'}${r.nomeComum ? ` (${r.nomeComum})` : ''}`);
          if (r.campanha) parts.push(`Camp: ${r.campanha}`);
          if (r.data) parts.push(`Data: ${r.data}`);
          if (r.unidadeAmostral) parts.push(`UA: ${r.unidadeAmostral}`);
          if (r.metodo) parts.push(`Método: ${r.metodo}`);
          if (r.sexo) parts.push(`Sexo: ${r.sexo}`);
          if (r.iucn && r.iucn !== 'LC' && r.iucn !== 'NE') parts.push(`IUCN: ${r.iucn}`);
          if (r.ibamaMma) parts.push(`MMA: ${r.ibamaMma}`);
          if (r.cites) parts.push(`CITES: ${r.cites}`);
          if (r.pan) parts.push(`PAN: ${r.pan}`);
          if (r.pesoG) parts.push(`Peso: ${r.pesoG}g`);
          if (r.ctMm) parts.push(`CT: ${r.ctMm}mm`);
          // Parâmetros ambientais (ruído, qualidade_agua, etc.)
          if (r.parametros && typeof r.parametros === 'object') {
            const pEntries = Object.entries(r.parametros).filter(([, v]) => v != null);
            if (pEntries.length > 0) parts.push(`Parâmetros: ${pEntries.map(([k, v]) => `${k}=${v}`).join(', ')}`);
          }
          if (r.observacoes) parts.push(`Obs: ${String(r.observacoes).substring(0, 100)}`);
          return parts.join(' | ');
        }).join('\n');

        sections.push(campoSection);
      } catch (campoErr: any) {
        console.warn('[AI] Erro ao carregar dados de campo:', campoErr.message);
      }
    }

    // ── PROJETOS ─────────────────────────────────────────────────────────────
    if (topics.has('projetos')) {
      try {
        const projetosList = await storage.getProjetosByUnidade(unidade);
        const filtrados = empreendimentoId
          ? projetosList.filter((p: any) => p.empreendimentoId === empreendimentoId)
          : projetosList;
        const ativos = filtrados.filter((p: any) => p.status === 'ativo' || p.status === 'em_andamento').length;
        const concluidos = filtrados.filter((p: any) => p.status === 'concluido').length;
        const pausados = filtrados.filter((p: any) => p.status === 'pausado' || p.status === 'cancelado').length;
        sections.push(
          `## PROJETOS (total: ${filtrados.length} | ativos/andamento: ${ativos} | concluídos: ${concluidos} | pausados/cancelados: ${pausados})\n` +
          filtrados.slice(0, 40).map((p: any, i: number) => {
            const parts: string[] = [];
            parts.push(`  ${i + 1}. [ID:${p.id}] ${p.nome}`);
            if (p.status) parts.push(`Status: ${p.status}`);
            if (p.descricao) parts.push(`Descrição: ${String(p.descricao).substring(0, 100)}`);
            if (p.inicioPrevisto) parts.push(`Início previsto: ${p.inicioPrevisto}`);
            if (p.fimPrevisto) parts.push(`Fim previsto: ${p.fimPrevisto}`);
            if (p.inicioReal) parts.push(`Início real: ${p.inicioReal}`);
            if (p.fimReal) parts.push(`Fim real: ${p.fimReal}`);
            if (p.valorContratado && Number(p.valorContratado) > 0) parts.push(`Valor contratado: R$ ${Number(p.valorContratado).toLocaleString('pt-BR')}`);
            if (p.valorRecebido && Number(p.valorRecebido) > 0) parts.push(`Valor recebido: R$ ${Number(p.valorRecebido).toLocaleString('pt-BR')}`);
            if (p.orcamentoPrevisto && Number(p.orcamentoPrevisto) > 0) parts.push(`Orçamento previsto: R$ ${Number(p.orcamentoPrevisto).toLocaleString('pt-BR')}`);
            if (p.metaReducaoGastos && Number(p.metaReducaoGastos) > 0) parts.push(`Meta redução: ${p.metaReducaoGastos}%`);
            if (p.periodicidade) parts.push(`Periodicidade: ${p.periodicidade}`);
            if (p.bmmServicos) parts.push(`BMM: ${p.bmmServicos}`);
            if (p.ndReembolsaveis) parts.push(`ND: ${p.ndReembolsaveis}`);
            if (p.metodologia) parts.push(`Metodologia: ${String(p.metodologia).substring(0, 120)}`);
            return parts.join(' | ');
          }).join('\n')
        );
      } catch (projErr: any) {
        console.warn('[AI] Erro ao carregar projetos:', projErr.message);
      }
    }

    // ── CAMPANHAS ────────────────────────────────────────────────────────────
    if (topics.has('campanhas')) {
      try {
        const empreendimentos = await storage.getEmpreendimentos(unidade);
        const empIds = empreendimentos.map((e: any) => e.id);
        let campanhasList: any[] = [];
        if (empIds.length > 0) {
          const conditions: any[] = [inArray(campanhas.empreendimentoId, empIds)];
          if (empreendimentoId) conditions.push(eq(campanhas.empreendimentoId, empreendimentoId));
          campanhasList = await db.select().from(campanhas)
            .where(and(...conditions))
            .orderBy(desc(campanhas.criadoEm));
        }
        const planejadas = campanhasList.filter((c: any) => c.status === 'planejada').length;
        const emAndamento = campanhasList.filter((c: any) => c.status === 'em_andamento').length;
        const concluidas = campanhasList.filter((c: any) => c.status === 'concluida' || c.status === 'encerrada').length;
        sections.push(
          `## CAMPANHAS DE MONITORAMENTO (total: ${campanhasList.length} | planejadas: ${planejadas} | em andamento: ${emAndamento} | concluídas: ${concluidas})\n` +
          campanhasList.slice(0, 40).map((c: any, i: number) => {
            const parts: string[] = [];
            // find empreendimento name
            const emp = empreendimentos.find((e: any) => e.id === c.empreendimentoId);
            parts.push(`  ${i + 1}. [ID:${c.id}] ${c.nome}`);
            if (emp) parts.push(`Empreendimento: ${emp.nome}`);
            if (c.status) parts.push(`Status: ${c.status}`);
            if (c.periodoInicio) parts.push(`Início: ${c.periodoInicio}`);
            if (c.periodoFim) parts.push(`Fim: ${c.periodoFim}`);
            if (c.descricao) parts.push(`Descrição: ${String(c.descricao).substring(0, 100)}`);
            if (c.observacoesMetodologicas) parts.push(`Metodologia: ${String(c.observacoesMetodologicas).substring(0, 120)}`);
            if (c.motivoStatus) parts.push(`Motivo status: ${c.motivoStatus}`);
            return parts.join(' | ');
          }).join('\n')
        );
      } catch (campErr: any) {
        console.warn('[AI] Erro ao carregar campanhas:', campErr.message);
      }
    }
    ...
    if (topics.has('documentos')) {
      try {
        const docs = await storage.getDatasets({ unidade, empreendimentoId });
        if (docs.length > 0) {
          sections.push(
            `## DOCUMENTOS CADASTRADOS (${docs.length} arquivos na gestão documental)\n` +
            docs.slice(0, 60).map((d: any, i: number) => {
              const parts: string[] = [];
              const label = (d as any).titulo || (d as any).codigoArquivo || d.nome;
              parts.push(`  ${i + 1}. "${label}"`);
              if ((d as any).tipoDocumental) parts.push(`Tipo: ${(d as any).tipoDocumental}`);
              if ((d as any).numeroDocumento) parts.push(`Nº: ${(d as any).numeroDocumento}`);
              if ((d as any).orgaoEmissor) parts.push(`Órgão: ${(d as any).orgaoEmissor}`);
              if ((d as any).statusDocumental) parts.push(`Status: ${(d as any).statusDocumental}`);
              if ((d as any).dataEmissao) parts.push(`Emitido: ${(d as any).dataEmissao}`);
              if ((d as any).prazoAtendimento) parts.push(`Prazo: ${(d as any).prazoAtendimento}`);
              if ((d as any).empreendimentoNome) parts.push(`Projeto: ${(d as any).empreendimentoNome}`);
              if ((d as any).exigencias) parts.push(`Exigências: ${String((d as any).exigencias).substring(0, 100)}`);
              return parts.join(' | ');
            }).join('\n')
          );
        }
      } catch (docErr: any) {
        console.warn('[AI] Erro ao carregar documentos:', docErr.message);
      }
    }
...
export async function processQuery(options: QueryOptions): Promise<QueryResponse> {
  ...
  try {
    ...
    const systemPrompt = `Você é o SGAI Assistente, agente de inteligência artificial do Sistema de Gestão Ambiental Integrada (SGAI). Você é um especialista em gestão ambiental, licenciamento, documentação técnica e conformidade ambiental brasileira — inteligente, atento e humano.

## Sua personalidade
- Converse de forma natural e acolhedora, como um colega de trabalho experiente em gestão ambiental
...
- Destaque proativamente informações importantes que o usuário pode não ter pedido mas precisa saber (ex: licença vencendo, demanda atrasada, documento com prazo crítico)
...
- Consultar empreendimentos, licenças, condicionantes, demandas, contratos, documentos, amostras, financeiro, RH, frota e equipamentos
- Identificar documentos cadastrados na gestão documental: tipo, número, órgão emissor, status, prazos e exigências
- Alertar sobre prazos críticos e itens vencidos
- Responder sobre o conteúdo de documentos indexados (PDFs, relatórios, laudos, etc.)
- Explicar fluxos de licenciamento ambiental (IBAMA, INEMA, CETESB, FEAM e outros órgãos estaduais)
...
- Quando o usuário perguntar sobre um documento específico, cite o número, órgão emissor e status se disponíveis`;
...
export async function streamQuery(options: QueryOptions, res: any): Promise<void> {
  ...
  try {
    ...
    // ────────────────────────────────────────────────────────────────────────
    // MOTOR 1.5: Anthropic Claude via Replit AI Integrations (managed billing)
    // ────────────────────────────────────────────────────────────────────────
    if (!chatUsed && process.env.AI_INTEGRATIONS_ANTHROPIC_API_KEY) {
      try {
        console.log('[AI Stream] Trying Anthropic Claude (claude-haiku-4-5)');
        ...
        const claudeText = msg.content.filter((c: any) => c.type === 'text').map((c: any) => c.text).join('');
        if (claudeText) {
          fullContent = claudeText;
          sendEvent('token', { content: claudeText });
          chatUsed = 'Claude';
          console.log(`[AI Stream] ✓ Anthropic Claude OK (${fullContent.length} chars)`);
        }
      } catch (claudeErr: any) {
        console.warn(`[AI Stream] Anthropic Claude falhou: ${claudeErr.message} — tentando fallback...`);
        fullContent = '';
      }
    }

...
import { lookupSpecies } from "./speciesDB";

// ── Conservation-status normalisation helpers ────────────────────────────────
const IUCN_TEXT_MAP: Record<string, string> = {
...
function normalizeIucnCode(val?: string): string {
  if (!val || !val.trim()) return "";
  const key = val.trim().toLowerCase();
  return IUCN_TEXT_MAP[key] || val.trim();
}
function normalizeCitesCode(val?: string): string {
  if (!val || !val.trim()) return "";
  const v = val.trim().toLowerCase();
  if (v === "nenhum" || v === "none" || v === "ap. nenhum" || v === "apêndice nenhum" || v === "-" || v === "0") return "";
...
function normalizeMmaCode(val?: string, luCategoria?: string): string {
  if (!val || !val.trim()) return luCategoria || "";
  const v = val.trim().toLowerCase();
  if (["não", "nao", "no", "false", "0", "não listado", "nao listado"].includes(v)) return "";
  if (["sim", "yes", "true", "1"].includes(v)) return luCategoria || "";
  const iucnNorm = normalizeIucnCode(val);
  if (["LC","NT","VU","EN","CR","EW","EX","DD","NE"].includes(iucnNorm)) return iucnNorm;
  return val.trim();
}
const PAN_NEGATIVES_SERVER = [
  ...
  "não se aplica","nao se aplica","n/a","na","nd",
];
function normalizePanValue(val?: string, luPan?: string): string {
  if (luPan) return luPan; // official DB always wins
  if (!val || !val.trim()) return "";
  const v = val.trim().toLowerCase();
  if (PAN_NEGATIVES_SERVER.includes(v)) return "";
  if (["pan","sim","yes","true","1"].includes(v)) return luPan || "";
  return val.trim();
}
/** Apply SpeciesDB lookup to a campo registro data object, enriching conservation fields */
function applySpeciesLookup(r: any): any {
  const nome = (r.nomeCientifico || "").trim();
  if (!nome) return r;
  const lu = lookupSpecies(nome);
  const enriched = { ...r };
  // SpeciesDB always wins; fall back to normalized user input
  if (!enriched.iucn || enriched.iucn === "") enriched.iucn = lu?.iucn || normalizeIucnCode(r.iucn) || "";
  else enriched.iucn = lu?.iucn || normalizeIucnCode(enriched.iucn) || "";
  enriched.ibamaMma = normalizeMmaCode(r.ibamaMma, lu?.mma?.categoria);
  if (!enriched.cites || enriched.cites === "") enriched.cites = lu?.cites || normalizeCitesCode(r.cites) || "";
  else enriched.cites = lu?.cites || normalizeCitesCode(enriched.cites) || "";
  enriched.pan = lu?.pan?.pan ? lu.pan.pan : normalizePanValue(r.pan, undefined);
  if (!enriched.nomeComum && lu?.nomeVernacular) enriched.nomeComum = lu.nomeVernacular;
  return enriched;
}

import { registerObjectStorageRoutes } from "./replit_integrations/object_storage";
import { registerMapbiomasRoutes } from "./mapbiomas";
import { 
  ...
  documentoVinculos, insertDocumentoVinculoSchema,
  projetoIntercorrencias, insertProjetoIntercorrenciaSchema,
  campanhaIntercorrencias, insertCampanhaIntercorrenciaSchema,
  publicacoes as publicacoesTable,
  veiculos,
  equipamentos,
  fornecedores,
  treinamentos,
...
import { testGoogleDriveConnection, uploadToGoogleDrive, listGoogleDriveBackups, deleteOldGoogleDriveBackups, createGoogleDriveInstitutionalStructure, createGoogleDriveEmpreendimentoStructure, syncAllFilesToGoogleDrive } from "./services/googleDriveService";
import { seiaService } from "./services/seiaService";
import { newsletterService } from "./services/newsletterService";
import { blogService } from "./services/blogService";
import { blogArtigos, blogComentarios, blogReacoes, insertBlogArtigoSchema, insertBlogComentarioSchema, newsletterAssinantes, users, dropboxSyncLog } from "@shared/schema";
import {
...
import { registerModelosLayoutsRoutes } from "./routes/modelosLayouts.routes";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Garante o usuário admin@admin.com.br em qualquer ambiente (dev e produção)
  const initSystemAdmin = async () => {
    try {
      const email = "admin@admin.com.br";
      const existing = await storage.getUserByEmail(email);
      if (!existing) {
        await storage.createUser({ email, passwordHash: "admin123", role: "admin", cargo: "coordenador", unidade: "salvador" });
        console.log("[Seed] admin@admin.com.br criado com role=admin");
      } else if (existing.role !== "admin") {
        await db.update(users).set({ role: "admin", cargo: "coordenador" }).where(eq(users.email, email));
        console.log("[Seed] admin@admin.com.br promovido para role=admin");
      }
    } catch (e) {
      console.error("[Seed] Erro ao inicializar admin@admin.com.br:", e);
    }
  };
  ...
  // ── Modelos e Layouts routes ──────────────────────────────────────────────
  registerModelosLayoutsRoutes(app, { requireAuth });
...

{
  ...
  "fichaTecnica": {
    "tipoDocumental": "eia|rima|ras|rap|prad|pca|pba|pgas|pee|pgrs|pcpv|licenca_previa|licenca_instalacao|licenca_operacao|las|licenca|oficio|notificacao|parecer|auto_infracao|termo|art|relatorio|mapa|documento_legal|condicionante|artigo_cientifico|outro",
...
- Confiança "alta" = texto rico e completo | "media" = texto parcial | "baixa" = texto muito curto ou apenas nome do arquivo`;
  ...
  app.post('/api/datasets/ai-extrair', requireAuth, async (req: any, res) => {
    try {
      ...
      // 0) Gemini — tenta múltiplos modelos
      if (!rawText && process.env.GEMINI_API_KEY) {
        console.log('[AI-Extrair] Tentando Gemini...');
        try {
          const { GoogleGenerativeAI } = await import('@google/generative-ai');
          const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
          for (const m of ['gemini-2.5-flash-preview-04-17', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-1.5-pro']) {
            try {
              ...
              if (candidate && candidate.trim()) {
                console.log(`[AI-Extrair] Gemini OK (${m}), ${candidate.length} chars`);
                rawText = candidate;
                break;
              }
            } catch (e: any) {
              console.warn(`[AI-Extrair] Gemini ${m} falhou: ${e?.message?.slice(0, 100)}`);
              lastErrEx = e;
            }
          }
      ...
      // 1) OpenAI via fetch direto (bypassa o proxy do Replit; usa OPENAI_DIRECT_KEY)
      const openAiKey = process.env.OPENAI_DIRECT_KEY || process.env.OPENAI_API_KEY;
      if (!rawText && openAiKey) {
        console.log('[AI-Extrair] Tentando OpenAI (fetch direto)...');
        try {
          ...
          if (oaiRes.ok) {
            ...
            rawText = oaiJson.choices?.[0]?.message?.content || '';
            if (rawText) console.log(`[AI-Extrair] OpenAI OK, ${rawText.length} chars`);
          } else {
            const errBody = await oaiRes.text();
            console.warn(`[AI-Extrair] OpenAI HTTP ${oaiRes.status}: ${errBody.slice(0, 200)}`);
            lastErrEx = new Error(`OpenAI ${oaiRes.status}`);
          }
        } catch (e: any) {
          console.warn(`[AI-Extrair] OpenAI fetch falhou: ${e?.message}`);
          lastErrEx = e;
        }
      }
      ...
      if (!rawText && process.env.DEEPSEEK_API_KEY) {
        console.log('[AI-Extrair] Tentando DeepSeek...');
        try {
          const dsRes = await fetch('https://api.deepseek.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
              model: 'deepseek-chat',
              messages: [
                { role: 'system', content: 'Você é um extrator de dados documentais. Retorne APENAS JSON válido.' },
                { role: 'user', content: prompt }
              ],
              temperature: 0,
            }),
          ...
          } else { lastErrEx = new Error(`DeepSeek ${dsRes.status}`); }
        } catch (e: any) { lastErrEx = e; }
      }
      if (!rawText) throw lastErrEx || new Error('Todos os provedores de IA falharam');

      // Extract JSON — handles plain JSON, markdown fenced blocks, and trailing text
    ...
    try {
      ...
      if (!texto && dataset.objectPath) {
        try {
          ...
          const relativePath = dataset.objectPath.startsWith("/objects/")
            ? dataset.objectPath.slice("/objects/".length)
            : dataset.objectPath.startsWith("/files/")
              ? dataset.objectPath.slice("/files/".length)
              : dataset.objectPath;
          const buf = await getObjectBuffer(relativePath);
          if (!buf) throw new Error('Falha ao baixar arquivo ou arquivo vazio');
          pdfBuf = buf;
  ...
  // Função para gerar código do arquivo — formato simplificado: PROJETO-DISC-TIPO-FASE-STATUS
  function gerarCodigoArquivo(dados: {
    ...
    status?: string;
    extensao?: string;
    // campos legados (ignorados na geração, mantidos para compatibilidade)
    cliente?: string;
    uf?: string;
    subprojeto?: string;
    area?: string;
    ...
    // Encurta o nome do projeto: pega até 2 palavras significativas, sem stop-words
    function abreviarProjeto(nome: string): string {
      const stopWords = new Set(['de', 'da', 'do', 'das', 'dos', 'e', 'a', 'o', 'em', 'para']);
      const palavras = normalizarTexto(nome).split('_').filter(p => !stopWords.has(p.toLowerCase()) && p.length > 1);
      return palavras.slice(0, 2).join('_');
    }

    const partes: string[] = [];
    if (dados.projeto) partes.push(abreviarProjeto(dados.projeto));
    ...
    let codigo = partes.length > 0 ? partes.join("-") : "DOC";
    if (dados.extensao) {
  ...
  app.get('/api/backups/:fileName', requireAuth, async (req: any, res) => {
    try {
      ...
      // Try Google Drive first (find by name, download as Buffer to preserve binary)
      const { listGoogleDriveBackups, downloadDocumentFromGoogleDrive } = await import('./services/googleDriveService');
      const driveList = await listGoogleDriveBackups();
      if (driveList.success && driveList.files) {
        const match = driveList.files.find((f: any) => f.name === fileName || f.id === fileName);
        if (match) {
          const buf = await downloadDocumentFromGoogleDrive(match.id);
          if (buf) {
      ...
      // Fallback: Object Storage (text/JSON only)
  ...
  app.post('/api/dropbox/backup', requireAuth, async (req: any, res) => {
    try {
      ...
      const content = await downloadBackup(latestBackup.key);
  ...
  // GET /api/google-drive/test - Test Google Drive connection
  app.get('/api/google-drive/test', requireAuth, async (req: any, res) => {
    try {
  ...
  // GET /api/google-drive/backups - List Google Drive backups
  app.get('/api/google-drive/backups', requireAuth, async (req: any, res) => {
    try {
  ...
  // POST /api/google-drive/backup - Perform backup and send to Google Drive
  app.post('/api/google-drive/backup', requireAuth, async (req: any, res) => {
    try {
      const backupResult = await performBackup();
      if (!backupResult.success) {
        return res.status(500).json({ success: false, error: 'Falha ao criar backup local' });
      ...
      const uploadResult = await uploadToGoogleDrive(latestBackup.key, content);
      if (uploadResult.success) {
        await deleteOldGoogleDriveBackups(30);
      }
      res.json({
        success: uploadResult.success,
        localBackup: baseFileName,
  ...
  // DELETE /api/google-drive/cleanup - Delete old Google Drive backups
  app.delete('/api/google-drive/cleanup', requireAuth, async (req: any, res) => {
    try {
      const days = parseInt(req.query.days as string) || 30;
      const result = await deleteOldGoogleDriveBackups(days);
      res.json({ success: true, ...result });
    } catch (error: any) {
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  // POST /api/google-drive/folders/sync-all - Create ABNT folder structure in Google Drive
  app.post('/api/google-drive/folders/sync-all', requireAuth, async (req: any, res) => {
    try {
      await createGoogleDriveInstitutionalStructure();
      ...
      for (const emp of emps) {
        try {
          await createGoogleDriveEmpreendimentoStructure(
          ...
          console.error(`[GoogleDrive Folders] Error for ${emp.codigo}:`, err);
          errors++;
        }
      }
      ...
      console.error('[GoogleDrive Folders] Erro ao criar estrutura ABNT:', error);
      res.status(500).json({ success: false, error: error.message });
    }
  });

  // POST /api/google-drive/sync-files - Trigger full file sync to Google Drive
  app.post('/api/google-drive/sync-files', requireAuth, async (req: any, res) => {
    try {
      res.json({ success: true, message: 'Sincronização iniciada em segundo plano.' });
      syncAllFilesToGoogleDrive().then(result => {
        console.log(`[GoogleDriveSync] Concluído: ${result.synced} sincronizados, ${result.errors} erros`);
      }).catch(err => {
        console.error('[GoogleDriveSync] Erro na sincronização:', err.message);
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  // DELETE /api/dropbox/sync-log - Clear all sync history
  app.delete('/api/dropbox/sync-log', requireAuth, async (req: any, res) => {
    try {
      await db.delete(dropboxSyncLog);
      res.json({ success: true, message: 'Histórico de sincronização apagado.' });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
  ...
  app.post('/api/onedrive/backup', requireAuth, async (req: any, res) => {
    try {
      ...
      const latestBackup = backups[0];
      const baseFileName = latestBackup.key;
      const content = await downloadBackup(latestBackup.key);
  ...
  // Sessões de upload chunked em memória: sessionId -> { chunks, total, name, mime }
  const chunkSessions = new Map<string, { chunks: Buffer[]; total: number; name: string; mime: string }>();

  const chunkUploadMiddleware = (await import('multer')).default({
    storage: (await import('multer')).memoryStorage(),
    limits: { fileSize: 6 * 1024 * 1024 }, // cada chunk ≤ 6 MB (bem abaixo do limite do proxy)
  });

  // Recebe um chunk do arquivo e, ao concluir, faz upload ao Object Storage
  app.post('/api/object-storage/upload-url', requireAuth, chunkUploadMiddleware.single('chunk'), async (req, res) => {
    try {
      const { sessionId, chunkIndex, totalChunks, fileName, fileType } = req.body;
      if (!req.file || !sessionId || chunkIndex === undefined || !totalChunks || !fileName) {
        return res.status(400).json({ error: 'Parâmetros de chunk incompletos' });
      }
      const idx = parseInt(chunkIndex);
      const total = parseInt(totalChunks);

      if (!chunkSessions.has(sessionId)) {
        chunkSessions.set(sessionId, { chunks: new Array(total), total, name: fileName, mime: fileType || 'application/octet-stream' });
      }
      ...
      if (received < total) {
        return res.json({ status: 'chunk_received', received, total });
      }

      // Todos os chunks recebidos — montar e enviar ao Google Drive
      ...
      const result = await uploadDocumentToGoogleDrive(session.name, fullBuffer, session.mime);
      if (!result.success || !result.url) throw new Error(result.error || 'Falha ao salvar arquivo no armazenamento');
      console.log(`[upload-chunk] Arquivo ${session.name} (${(fullBuffer.length / 1024 / 1024).toFixed(1)} MB) enviado: ${result.fileId}`);
      res.json({ status: 'complete', url: result.url, filePath: result.url });
    } catch (err: any) {
      console.error('[upload-chunk] erro:', err);
      res.status(500).json({ error: err.message || 'Erro ao processar chunk' });
    }
  });

  // Download de documento — redireciona para URL pública ou base64 (legado)
  app.get('/api/datasets/:id/download', requireAuth, async (req, res) => {
    try {
      ...
      const ext = dataset.nome?.split('.').pop()?.toLowerCase() || '';
      const mimeMap: Record<string, string> = {
      ...
      const inlineMimes = ['application/pdf', 'image/png', 'image/jpeg', 'image/gif', 'text/plain', 'text/csv'];
      const disposition = inlineMimes.includes(mime) ? 'inline' : 'attachment';
      const safeName = encodeURIComponent(dataset.nome || 'arquivo');

      // Arquivo no Google Drive — download via connector
      if (url.startsWith('gdrive://')) {
        const fileId = url.slice('gdrive://'.length);
        const { downloadDocumentFromGoogleDrive } = await import('./services/googleDriveService');
        const bytes = await downloadDocumentFromGoogleDrive(fileId);
        if (!bytes) return res.status(503).json({ error: 'Arquivo temporariamente indisponível no Google Drive.' });
      ...
      // Arquivo no Object Storage (URL pública replit) — download via SDK
      const bucketId = process.env.DEFAULT_OBJECT_STORAGE_BUCKET_ID;
      const objStoragePrefix = `https://object-storage.replit.app/${bucketId}/`;
      if (url.startsWith(objStoragePrefix)) {
        const key = url.slice(objStoragePrefix.length);
        const { getObjectBuffer } = await import('./services/objectStorageHelper');
        const bytes = await getObjectBuffer(key);
        if (!bytes) return res.status(503).json({ error: 'Arquivo temporariamente indisponível, tente novamente.' });
        res.setHeader('Content-Type', mime);
        res.setHeader('Content-Disposition', `${disposition}; filename*=UTF-8''${safeName}`);
        res.setHeader('Content-Length', bytes.length);
        return res.send(bytes);
      ...
      // URL externa genérica — redireciona
      if (url.startsWith('http')) return res.redirect(url);

      // Base64 data: URL — devolve inline
      if (url.startsWith('data:')) {
        const [header, b64] = url.split(',');
        const mime = header.replace('data:', '').replace(';base64', '');
        const buf = Buffer.from(b64, 'base64');
        res.setHeader('Content-Type', mime);
        res.setHeader('Content-Disposition', `attachment; filename="${dataset.nome || 'arquivo'}"`);
        return res.send(buf);
      }

      res.status(404).json({ error: 'URL do documento não disponível' });
    } catch (err: any) {
      console.error('[datasets/download] erro:', err);
      res.status(500).json({ error: err.message || 'Erro ao obter documento' });
    }
  });
...
Retorne o texto extraído de forma estruturada e organizada.`
  ...
  app.patch('/api/projetos/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) return res.status(400).json({ error: 'Invalid projeto ID' });
      const projeto = await storage.updateProjeto(id, req.body);
      res.json(projeto);
    } catch (error) {
      console.error('Error patching projeto:', error);
      res.status(500).json({ error: 'Failed to update projeto' });
    }
  });
  ...
  // ==== PROJETO INTERCORRÊNCIAS ROUTES ====

  app.get('/api/projetos/:id/intercorrencias', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const items = await db.select().from(projetoIntercorrencias)
        .where(eq(projetoIntercorrencias.projetoId, projetoId))
        .orderBy(desc(projetoIntercorrencias.data));
      res.json(items);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.post('/api/projetos/:id/intercorrencias', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const data = insertProjetoIntercorrenciaSchema.parse({ ...req.body, projetoId });
      const [item] = await db.insert(projetoIntercorrencias).values(data).returning();
      res.json(item);
    } catch (err: any) { res.status(400).json({ message: err.message }); }
  });

  app.patch('/api/projetos/:projetoId/intercorrencias/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { tipo, titulo, descricao, data: dataVal } = req.body;
      const [item] = await db.update(projetoIntercorrencias)
        .set({ tipo, titulo, descricao, data: dataVal })
        .where(eq(projetoIntercorrencias.id, id))
        .returning();
      res.json(item);
    } catch (err: any) { res.status(400).json({ message: err.message }); }
  });

  app.delete('/api/projetos/:projetoId/intercorrencias/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await db.delete(projetoIntercorrencias).where(eq(projetoIntercorrencias.id, id));
      res.status(204).send();
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // Upload de imagem para intercorrências (projetos e campanhas)
  {
    const multerIC = (await import('multer')).default;
    const icUpload = multerIC({
      storage: multerIC.memoryStorage(),
      limits: { fileSize: 10 * 1024 * 1024 },
      fileFilter: (_req, file, cb) => {
        if (['image/jpeg', 'image/png', 'image/jpg'].includes(file.mimetype)) cb(null, true);
        else cb(new Error('Apenas imagens JPEG e PNG são permitidas'));
      },
    });
    app.post('/api/intercorrencias/upload-imagem', requireAuth, icUpload.single('imagem'), async (req: any, res) => {
      try {
        if (!req.file) return res.status(400).json({ message: 'Nenhuma imagem enviada' });
        const { uploadDocumentToGoogleDrive } = await import('./services/googleDriveService');
        const ext = req.file.mimetype === 'image/png' ? 'png' : 'jpg';
        const fileName = `intercorrencia_${Date.now()}.${ext}`;
        const result = await uploadDocumentToGoogleDrive(fileName, req.file.buffer, req.file.mimetype);
        if (!result.success || !result.url) return res.status(503).json({ message: 'Falha ao salvar imagem, tente novamente.' });
        res.json({ key: result.url });
      } catch (err: any) { res.status(500).json({ message: err.message }); }
    });

    // Proxy para servir imagens de intercorrências
    app.get('/api/intercorrencias/imagem', requireAuth, async (req: any, res) => {
      try {
        ...
        if (key.startsWith('gdrive://')) {
          const { downloadDocumentFromGoogleDrive } = await import('./services/googleDriveService');
          bytes = await downloadDocumentFromGoogleDrive(key.slice('gdrive://'.length));
        } else if (key.startsWith('intercorrencias/')) {
          const { getObjectBuffer } = await import('./services/objectStorageHelper');
          bytes = await getObjectBuffer(key);
        } else {
          return res.status(400).json({ message: 'Chave inválida' });
        }
        if (!bytes) return res.status(503).json({ message: 'Imagem temporariamente indisponível, tente novamente.' });
        ...
        res.setHeader('Content-Length', bytes.length);
        return res.send(bytes);
      } catch (err: any) { res.status(503).json({ message: 'Imagem temporariamente indisponível, tente novamente.' }); }
    });
  }
  ...
  app.get('/api/campanhas', requireAuth, async (req: any, res) => {
    try {
      ...
      if (user.role === 'admin') {
        ...
        result = await db
          .select({ id: campanhas.id, nome: campanhas.nome, empreendimentoId: campanhas.empreendimentoId, periodoInicio: campanhas.periodoInicio, periodoFim: campanhas.periodoFim, descricao: campanhas.descricao, observacoesMetodologicas: campanhas.observacoesMetodologicas, status: campanhas.status, motivoStatus: campanhas.motivoStatus, criadoEm: campanhas.criadoEm })
          .from(campanhas)
  ...
  app.get('/api/empreendimentos/:empreendimentoId/campanhas', requireAuth, async (req, res) => {
    try {
      ...
      const projetoId = req.query.projetoId ? parseInt(req.query.projetoId as string) : undefined;
      const conditions = [eq(campanhas.empreendimentoId, empreendimentoId)];
      if (projetoId) conditions.push(eq(campanhas.projetoId, projetoId));
      const result = await db.select().from(campanhas).where(and(...conditions));
      res.json(result);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.patch('/api/campanhas/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { observacoesMetodologicas, nome, descricao, periodoInicio, periodoFim, status, motivoStatus } = req.body;
      const setData: any = { atualizadoEm: new Date() };
      if (observacoesMetodologicas !== undefined) setData.observacoesMetodologicas = observacoesMetodologicas;
      if (nome !== undefined) setData.nome = nome;
      if (descricao !== undefined) setData.descricao = descricao;
      if (periodoInicio !== undefined) setData.periodoInicio = periodoInicio;
      if (periodoFim !== undefined) setData.periodoFim = periodoFim;
      if (status !== undefined) setData.status = status;
      if (motivoStatus !== undefined) setData.motivoStatus = motivoStatus;
  ...
  app.delete('/api/campanhas/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await db.delete(campanhaIntercorrencias).where(eq(campanhaIntercorrencias.campanhaId, id));
      await db.delete(campanhas).where(eq(campanhas.id, id));
      res.status(204).send();
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // ==== CAMPANHA INTERCORRÊNCIAS ROUTES ====

  app.get('/api/campanhas/:id/intercorrencias', requireAuth, async (req, res) => {
    try {
      const campanhaId = parseInt(req.params.id);
      const items = await db.select().from(campanhaIntercorrencias)
        .where(eq(campanhaIntercorrencias.campanhaId, campanhaId))
        .orderBy(desc(campanhaIntercorrencias.data));
      res.json(items);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  app.post('/api/campanhas/:id/intercorrencias', requireAuth, async (req, res) => {
    try {
      const campanhaId = parseInt(req.params.id);
      const data = insertCampanhaIntercorrenciaSchema.parse({ ...req.body, campanhaId });
      const [item] = await db.insert(campanhaIntercorrencias).values(data).returning();
      res.json(item);
    } catch (err: any) { res.status(400).json({ message: err.message }); }
  });

  app.patch('/api/campanhas/:campanhaId/intercorrencias/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { tipo, titulo, descricao, data: dataVal } = req.body;
      const [item] = await db.update(campanhaIntercorrencias)
        .set({ tipo, titulo, descricao, data: dataVal })
        .where(eq(campanhaIntercorrencias.id, id))
        .returning();
      res.json(item);
    } catch (err: any) { res.status(400).json({ message: err.message }); }
  });

  app.delete('/api/campanhas/:campanhaId/intercorrencias/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      await db.delete(campanhaIntercorrencias).where(eq(campanhaIntercorrencias.id, id));
      res.status(204).send();
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  app.post('/api/rh', requireAuth, async (req, res) => {
    try {
      const body = { ...req.body };
      if (body.dataInicio === '') body.dataInicio = null;
      if (body.dataFim === '') body.dataFim = null;
      const data = insertRhRegistroSchema.parse(body);
  ...
  app.patch('/api/rh/:id', requireAuth, async (req, res) => {
    try {
      ...
      if (body.dataInicio === '') body.dataInicio = null;
      if (body.dataFim === '') body.dataFim = null;
      const data = insertRhRegistroSchema.partial().parse(body);
  ...
  app.get('/api/auth/me', requireAuth, async (req, res) => {
    try {
      ...
      res.json({ id: user.id, email: user.email, role: user.role, cargo: user.cargo, unidade: user.unidade, nome });
    } catch (error) {
  ...
  const aiDocUpload = multerAI({
    storage: multerAI.memoryStorage(),
    limits: { fileSize: 50 * 1024 * 1024 }, // 50 MB — suporta PDFs grandes
  });

  // Handler de erro do Multer (arquivo grande, etc.)
  function handleMulterError(err: any, req: any, res: any, next: any) {
    if (err && err.code === 'LIMIT_FILE_SIZE') {
      return res.status(413).json({ message: 'Arquivo muito grande. Limite de 50 MB por documento.' });
    }
    next(err);
  }

  app.post("/api/ai/upload-doc", requireAuth, (req: any, res: any, next: any) => {
    aiDocUpload.single('file')(req, res, (err: any) => {
      if (err && err.code === 'LIMIT_FILE_SIZE') {
        return res.status(413).json({ message: 'Arquivo muito grande. O limite é 50 MB.' });
      }
    ...
    try {
      ...
      let text = '';
      let extractedBy = 'pdf-parse';
      const mime = file.mimetype;
      const isPdf = mime === 'application/pdf' || file.originalname?.toLowerCase().endsWith('.pdf');
      const isImage = mime.startsWith('image/');
      const isText = mime === 'text/plain' || mime === 'text/csv' || file.originalname?.endsWith('.txt');

      // Helper: extração via Gemini Files API (upload separado → sem custo de tokens inline)
      const extractWithGemini = async (fileMime: string, buffer: Buffer, prompt: string, fileName?: string): Promise<string> => {
        if (!process.env.GEMINI_API_KEY) return '';
        const os = await import('os');
        const path = await import('path');
        const fs = await import('fs');
        const tmpDir = os.tmpdir();
        const tmpFile = path.join(tmpDir, `sgai_upload_${Date.now()}_${fileName || 'doc'}`);
        let uploadedFileName: string | null = null;
        try {
          fs.writeFileSync(tmpFile, buffer);
          // Helper para chamada Gemini com retry automático em 429
          const geminiCall = async (callFn: () => Promise<string>): Promise<string> => {
            for (let attempt = 0; attempt < 2; attempt++) {
              try {
                ...
                if (is429 && attempt === 0) {
                  // Extrai delay sugerido pelo Google (ex: "retry in 35.16s")
                  const delayMatch = msg.match(/retry in (\d+(?:\.\d+)?)/i);
                  const waitMs = delayMatch ? Math.min(parseFloat(delayMatch[1]) * 1000, 45000) : 10000;
                  console.warn(`[AI Upload] Gemini 429, aguardando ${Math.round(waitMs/1000)}s antes de retentar...`);
          ...
          // Tentativa 1: Files API (eficiente para arquivos grandes)
          try {
            const { GoogleAIFileManager } = await import('@google/generative-ai/server');
            const { GoogleGenerativeAI } = await import('@google/generative-ai');
            const fileManager = new GoogleAIFileManager(process.env.GEMINI_API_KEY!);
            const uploadResp = await fileManager.uploadFile(tmpFile, { mimeType: fileMime, displayName: fileName || 'documento' });
            uploadedFileName = uploadResp.file.name;
            const fileUri = uploadResp.file.uri;
            let uploaded = uploadResp.file;
            for (let i = 0; i < 20 && (uploaded as any).state !== 'ACTIVE'; i++) {
              await new Promise(r => setTimeout(r, 1500));
              uploaded = await fileManager.getFile(uploadedFileName);
            }
            const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY!);
            const out = await geminiCall(async () => {
            ...
            if (out && out.length > 10) {
              console.log('[AI Upload] Gemini Files API OK');
              return out.substring(0, 12000);
            }
          } catch (filesErr: any) {
            console.warn('[AI Upload] Files API falhou, tentando inline:', filesErr.message);
          }
          // Tentativa 2: inline data (fallback para arquivos ≤4 MB)
          if (buffer.length <= 4 * 1024 * 1024) {
            ...
            if (out && out.length > 10) return out.substring(0, 12000);
          }
        } catch (err: any) {
          console.warn('[AI Upload] Gemini extração falhou:', err.message);
        } finally {
          try { (await import('fs')).unlinkSync(tmpFile); } catch (_) {}
          if (uploadedFileName) {
            try {
              const { GoogleAIFileManager } = await import('@google/generative-ai/server');
              const fileManager = new GoogleAIFileManager(process.env.GEMINI_API_KEY!);
      ...
      // Helper: extração via OpenAI GPT-4o Vision (imagens e PDFs convertidos)
      const extractWithOpenAI = async (fileMime: string, buffer: Buffer, prompt: string): Promise<string> => {
        if (!process.env.OPENAI_API_KEY) return '';
        try {
          const OpenAI = (await import('openai')).default;
          const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
          // Para PDFs: converte primeira página para PNG via pdf2pic
          let imageMime = fileMime;
          ...
          if (fileMime === 'application/pdf') {
            try {
              ...
              const tmpDir = os.tmpdir();
              const tmpOut = path.join(tmpDir, `oai_conv_${Date.now()}`);
              fs.mkdirSync(tmpOut, { recursive: true });
              const conv = fromBuffer(buffer, { density: 150, saveFilename: 'p', savePath: tmpOut, format: 'png', width: 1654, height: 2339 });
              const r: any = await conv(1, { responseType: 'base64' });
              if (r?.base64) { imageBuffer = Buffer.from(r.base64, 'base64'); imageMime = 'image/png'; }
              else if (r?.path && fs.existsSync(r.path)) { imageBuffer = fs.readFileSync(r.path); imageMime = 'image/png'; }
          ...
          if (out.length > 10) {
            console.log(`[AI Upload] OpenAI Vision OK: ${out.length} chars`);
            return out.substring(0, 12000);
          }
        } catch (e: any) {
          console.warn('[AI Upload] OpenAI Vision falhou:', e.message);
        }
        return '';
      };

      if (isPdf) {
        // Tentativa 1: pdf-parse (rápido, digital)
        try {
          const pdfParse = (await import('pdf-parse')).default;
          const data = await pdfParse(file.buffer);
          const parsed = data.text.replace(/\s+/g, ' ').trim();
          if (parsed.length >= 80) { text = parsed.substring(0, 12000); extractedBy = 'pdf-parse'; }
        } catch (_) {}

        // Tentativa 2: pdfjs-dist (mais robusto, lida com fontes não-padrão e formulários)
        if (!text || text.length < 80) {
          try {
            const pdfjsLib: any = await import('pdfjs-dist/legacy/build/pdf.mjs');
            const loadTask = pdfjsLib.getDocument({ data: new Uint8Array(file.buffer), disableFontFace: true, verbosity: 0 });
            const pdfDoc = await loadTask.promise;
            const pageTexts: string[] = [];
            for (let p = 1; p <= Math.min(pdfDoc.numPages, 30); p++) {
              const page = await pdfDoc.getPage(p);
              const content = await page.getTextContent();
              const pageStr = (content.items as any[]).map((i: any) => i.str || '').join(' ').trim();
              if (pageStr) pageTexts.push(pageStr);
            }
            const joined = pageTexts.join('\n').replace(/\s+/g, ' ').trim();
            if (joined.length >= 80) { text = joined.substring(0, 12000); extractedBy = 'pdfjs'; }
            else console.warn(`[AI Upload] pdfjs-dist extraiu apenas ${joined.length} chars (provável PDF escaneado)`);
          } catch (pdfJsErr: any) {
            console.warn('[AI Upload] pdfjs-dist falhou:', pdfJsErr.message);
          }
        }

        // Tentativa 3: OCR local com Tesseract (PDFs escaneados – funciona offline)
        if (!text || text.length < 80) {
          try {
            const os = await import('os');
            const path = await import('path');
            ...
            const ocrDir = path.join(os.tmpdir(), `sgai_ocr_${Date.now()}`);
            ...
            // Descobre o número de páginas (limita a 5 para evitar timeout)
            let pageCount = 1;
            try {
              const { default: pdfP } = await import('pdf-parse');
              const info = await pdfP(file.buffer);
              pageCount = Math.min(info.numpages || 1, 5);
            } catch (_) {}

            const ocrTexts: string[] = [];
            for (let p = 1; p <= pageCount; p++) {
              try {
                const imgResult: any = await convert(p, { responseType: 'base64' });
                const imgPath = imgResult.path || path.join(ocrDir, `page.${p}.png`);
                // Garante que o arquivo exista
                if (imgResult.base64 && !fs.existsSync(imgPath)) {
                  fs.writeFileSync(imgPath, Buffer.from(imgResult.base64, 'base64'));
                }
                if (!fs.existsSync(imgPath)) continue;
                const outBase = path.join(ocrDir, `out_${p}`);
                await execFileP('tesseract', [imgPath, outBase, '-l', 'por+eng', '--oem', '1', '--psm', '3']);
                const outFile = outBase + '.txt';
                if (fs.existsSync(outFile)) {
                  const pageText = fs.readFileSync(outFile, 'utf-8').trim();
                  if (pageText.length > 5) ocrTexts.push(pageText);
                }
              } catch (pageErr: any) {
                console.warn(`[AI Upload] OCR página ${p} falhou:`, pageErr.message);
              }
            }

            // Limpeza
            try { fs.rmSync(ocrDir, { recursive: true, force: true }); } catch (_) {}

            const ocrText = ocrTexts.join('\n').replace(/\s+/g, ' ').trim();
            if (ocrText.length >= 80) {
              text = ocrText.substring(0, 12000);
              extractedBy = 'tesseract-ocr';
              console.log(`[AI Upload] OCR local OK: ${text.length} chars em ${ocrTexts.length} página(s)`);
            } else {
              console.warn(`[AI Upload] OCR local extraiu apenas ${ocrText.length} chars`);
            }
          } catch (ocrErr: any) {
            console.warn('[AI Upload] OCR local falhou:', ocrErr.message);
          }
        }

        // Tentativa 4: Gemini Files API (último recurso, requer cota disponível)
        if (!text || text.length < 80) {
          text = await extractWithGemini(
            'application/pdf',
            file.buffer,
            'Extraia todo o texto deste documento PDF na íntegra, preservando a estrutura. Retorne apenas o texto extraído, sem comentários.',
            file.originalname
          );
          if (text) extractedBy = 'gemini';
        }

        // Tentativa 5: OpenAI GPT-4o Vision (converte 1ª página para imagem e envia)
        if (!text || text.length < 80) {
          text = await extractWithOpenAI(
            'application/pdf',
            file.buffer,
            'Extraia todo o texto deste documento PDF. Retorne apenas o texto extraído, sem comentários adicionais.'
          );
          if (text) extractedBy = 'openai-vision';
        }
      } else if (isImage) {
        // Gemini Vision (tentativa 1)
        text = await extractWithGemini(
          mime, file.buffer,
          'Extraia todo o texto visível nesta imagem. Retorne apenas o texto, sem comentários.',
          file.originalname
        );
        if (text) extractedBy = 'gemini-vision';

        // OpenAI Vision (tentativa 2)
        if (!text) {
          text = await extractWithOpenAI(
            mime, file.buffer,
            'Extraia todo o texto visível nesta imagem. Retorne apenas o texto, sem comentários.'
        ...
        if (!text) return res.status(422).json({ message: "Não foi possível extrair texto da imagem. Verifique se a imagem contém texto legível." });
      } else if (isText) {
        text = file.buffer.toString('utf-8').substring(0, 12000);
        extractedBy = 'text';
      } else {
        // Formato desconhecido: tenta Gemini e depois OpenAI
        text = await extractWithGemini(
          mime || 'application/octet-stream', file.buffer,
          'Extraia todo o texto deste documento. Retorne apenas o texto extraído.',
          file.originalname
        );
        if (!text) {
          text = await extractWithOpenAI(
        ...
        if (!text) return res.status(400).json({ message: "Formato não suportado. Envie um PDF, imagem ou arquivo .txt" });
        extractedBy = 'openai-vision';
      }

      if (!text.trim()) {
        const isPdfScan = isPdf;
        const hint = isPdfScan
          ? "Este PDF parece ser um documento escaneado (sem texto digital). Tente converter o PDF para texto antes de enviar, ou preencha os campos manualmente."
          : "O documento parece estar em branco, ser uma imagem sem texto ou em formato não reconhecido.";
        return res.status(422).json({ message: `Não foi possível extrair texto do arquivo. ${hint}` });
      }

      res.json({ text, name: file.originalname, size: file.size, chars: text.length, extractedBy });
    } catch (error: any) {
...
- Retorne apenas o texto melhorado, sem explicações`;
  ...
  app.post('/api/blog', requireBlogAdmin, async (req, res) => {
    try {
      ...
      const [rhRec] = req.user?.email ? await db.select({ nomeColaborador: rhRegistros.nomeColaborador })
        .from(rhRegistros).where(eq(rhRegistros.contatoEmail, req.user.email)).limit(1) : [];
      const autorNomeResolved = rhRec?.nomeColaborador ||
        (req.user?.email?.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) ?? 'Maurivan Vaz Ribeiro');
  ...
  app.post('/api/blog/criar-e-publicar', requireBlogAdmin, async (req, res) => {
    try {
      ...
      const [rhRec2] = req.user?.email ? await db.select({ nomeColaborador: rhRegistros.nomeColaborador })
        .from(rhRegistros).where(eq(rhRegistros.contatoEmail, req.user.email)).limit(1) : [];
      const autorNomeResolved2 = rhRec2?.nomeColaborador ||
        (req.user?.email?.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()) ?? 'Maurivan Vaz Ribeiro');
  ...
  // ======== PROJETO RECURSOS (equipamentos, veiculos, equipe) ========

  // PATCH sync equipe do projeto (replace all)
  app.patch('/api/projetos/:id/equipe', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const { membroIds } = req.body as { membroIds: number[] };
      const unidade = req.user.unidade;
      // Remove todos e re-adiciona
      await storage.syncProjetoEquipe(projetoId, membroIds || [], unidade);
      res.json({ ok: true });
    } catch (error) {
      console.error('Error syncing projeto equipe:', error);
      res.status(500).json({ error: 'Erro ao sincronizar equipe' });
    }
  });

  // GET equipamentos do projeto
  app.get('/api/projetos/:id/equipamentos', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const lista = await storage.getEquipamentossDoProjeto(projetoId);
      res.json(lista);
    } catch (error) {
      console.error('Error fetching projeto equipamentos:', error);
      res.status(500).json({ error: 'Erro ao buscar equipamentos do projeto' });
    }
  });

  // PATCH sync equipamentos do projeto
  app.patch('/api/projetos/:id/equipamentos', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const { equipamentoIds } = req.body;
      await storage.syncProjetoEquipamentos(projetoId, equipamentoIds || []);
      res.json({ ok: true });
    } catch (error) {
      console.error('Error syncing projeto equipamentos:', error);
      res.status(500).json({ error: 'Erro ao sincronizar equipamentos' });
    }
  });

  // GET veiculos do projeto
  app.get('/api/projetos/:id/veiculos', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const lista = await storage.getVeiculosDoProjeto(projetoId);
      res.json(lista);
    } catch (error) {
      console.error('Error fetching projeto veiculos:', error);
      res.status(500).json({ error: 'Erro ao buscar veículos do projeto' });
    }
  });

  // PATCH sync veiculos do projeto
  app.patch('/api/projetos/:id/veiculos', requireAuth, async (req, res) => {
    try {
      const projetoId = parseInt(req.params.id);
      const { veiculoIds } = req.body;
      await storage.syncProjetoVeiculos(projetoId, veiculoIds || []);
      res.json({ ok: true });
    } catch (error) {
      console.error('Error syncing projeto veiculos:', error);
  ...
  app.get('/api/relatorio-plataforma', requireAuth, async (req, res) => {
    try {
      ...
      let campoList: any[] = [];
      let intercorrenciasList: any[] = [];
      let amostrasList: any[] = [];
      let fornecedoresList: any[] = [];
      let treinamentosList: any[] = [];
      let propostasList: any[] = [];
      let publicacoesList: any[] = [];
      let processosMonitoradosList: any[] = [];
      let baseConhecimentoList: any[] = [];
      ...
      try {
        campoList = await db.select({
          id: campoRegistros.id,
          grupoTaxonomico: campoRegistros.grupoTaxonomico,
          campanha: campoRegistros.campanha,
          data: campoRegistros.data,
          nomeCientifico: campoRegistros.nomeCientifico,
          nomeComum: campoRegistros.nomeComum,
          unidadeAmostral: campoRegistros.unidadeAmostral,
          iucn: campoRegistros.iucn,
          ibamaMma: campoRegistros.ibamaMma,
          cites: campoRegistros.cites,
          empreendimentoId: campoRegistros.empreendimentoId,
        }).from(campoRegistros).limit(500);
        console.log('Loaded campo:', campoList.length);
      } catch (e) { console.error('Error loading campo:', e); }

      try {
        intercorrenciasList = await db.select().from(campanhaIntercorrencias).limit(500);
        console.log('Loaded intercorrencias:', intercorrenciasList.length);
      } catch (e) { console.error('Error loading intercorrencias:', e); }

      try {
        amostrasList = await storage.getAmostras({ unidade: unidadeFilter }) || [];
        console.log('Loaded amostras:', amostrasList.length);
      } catch (e) { console.error('Error loading amostras:', e); }

      try {
        fornecedoresList = await storage.getFornecedores({ unidade: unidadeFilter }) || [];
        console.log('Loaded fornecedores:', fornecedoresList.length);
      } catch (e) { console.error('Error loading fornecedores:', e); }

      try {
        treinamentosList = await storage.getTreinamentos({ unidade: unidadeFilter }) || [];
        console.log('Loaded treinamentos:', treinamentosList.length);
      } catch (e) { console.error('Error loading treinamentos:', e); }

      try {
        propostasList = await storage.getPropostasComerciais({ unidade: unidadeFilter }) || [];
        console.log('Loaded propostas:', propostasList.length);
      } catch (e) { console.error('Error loading propostas:', e); }

      try {
        publicacoesList = await (unidadeFilter
          ? db.select().from(publicacoesTable).where(eq(publicacoesTable.unidade, unidadeFilter)).limit(500)
          : db.select().from(publicacoesTable).limit(500));
        console.log('Loaded publicacoes:', publicacoesList.length);
      } catch (e) { console.error('Error loading publicacoes:', e); }

      try {
        processosMonitoradosList = await (unidadeFilter
          ? db.select().from(processosMonitorados).where(and(eq(processosMonitorados.ativo, true), eq(processosMonitorados.unidade, unidadeFilter))).limit(300)
          : db.select().from(processosMonitorados).where(eq(processosMonitorados.ativo, true)).limit(300));
        console.log('Loaded processos:', processosMonitoradosList.length);
      } catch (e) { console.error('Error loading processos:', e); }

      try {
        baseConhecimentoList = await storage.getBaseConhecimento({ unidade: unidadeFilter }) || [];
        console.log('Loaded base conhecimento:', baseConhecimentoList.length);
      } catch (e) { console.error('Error loading base conhecimento:', e); }
      ...
      const relatorio = {
        ...
        resumoGeral: {
          ...
          totalRegistrosCampo: campoList.length,
          totalIntercorrencias: intercorrenciasList.length,
          totalAmostras: amostrasList.length,
          totalFornecedores: fornecedoresList.length,
          totalTreinamentos: treinamentosList.length,
          totalPropostas: propostasList.length,
          totalPublicacoes: publicacoesList.length,
          totalProcessos: processosMonitoradosList.length,
          totalBaseConhecimento: baseConhecimentoList.length,
        },
        ...
        campo: {
          total: campoList.length,
          campanhas: [...new Set(campoList.map(r => r.campanha).filter(Boolean))].length,
          ameacadas: campoList.filter(r =>
            (r.iucn && !['LC','NE','DD','-',''].includes(r.iucn)) ||
            (r.ibamaMma && !['','-'].includes(r.ibamaMma)) ||
            (r.cites && !['','-'].includes(r.cites))
          ).length,
          porGrupo: campoList.reduce((acc: any, r) => {
            acc[r.grupoTaxonomico || 'Outros'] = (acc[r.grupoTaxonomico || 'Outros'] || 0) + 1;
            return acc;
          }, {}),
          porCampanha: campoList.reduce((acc: any, r) => {
            const c = r.campanha || 'Sem Campanha';
            acc[c] = (acc[c] || 0) + 1;
            return acc;
          }, {}),
          lista: campoList.slice(0, 25).map(r => ({
            grupoTaxonomico: r.grupoTaxonomico,
            campanha: r.campanha,
            data: r.data,
            nomeCientifico: r.nomeCientifico,
            nomeComum: r.nomeComum,
            unidadeAmostral: r.unidadeAmostral,
            iucn: r.iucn,
            ibamaMma: r.ibamaMma,
            cites: r.cites,
          })),
        ...
        intercorrencias: {
          total: intercorrenciasList.length,
          porTipo: intercorrenciasList.reduce((acc: any, i) => {
            acc[i.tipo || 'ocorrencia'] = (acc[i.tipo || 'ocorrencia'] || 0) + 1;
            return acc;
          }, {}),
          lista: intercorrenciasList.slice(0, 20).map((i: any) => ({
        ...
        amostras: {
          total: amostrasList.length,
          porStatus: amostrasList.reduce((acc: any, a: any) => {
            acc[a.status || 'outros'] = (acc[a.status || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          porTipo: amostrasList.reduce((acc: any, a: any) => {
            acc[a.tipo || 'outros'] = (acc[a.tipo || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          lista: amostrasList.slice(0, 20).map((a: any) => ({
            codigo: a.codigoAmostra || a.codigo || '-',
            tipo: a.tipo,
            status: a.status,
        ...
        fornecedores: {
          ...
          porTipo: fornecedoresList.reduce((acc: any, f: any) => {
            acc[f.tipo || 'outros'] = (acc[f.tipo || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          porAvaliacao: fornecedoresList.reduce((acc: any, f: any) => {
            const r = Number(f.avaliacao) || 0;
            const k = r >= 4 ? 'Excelente (4-5)' : r >= 3 ? 'Bom (3)' : r >= 1 ? 'Regular (1-2)' : 'Sem avaliação';
            acc[k] = (acc[k] || 0) + 1;
            return acc;
          }, {}),
          lista: fornecedoresList.slice(0, 20).map((f: any) => ({
            nome: f.nome,
            tipo: f.tipo,
        ...
        treinamentos: {
          total: treinamentosList.length,
          porStatus: treinamentosList.reduce((acc: any, t: any) => {
            acc[t.status || 'outros'] = (acc[t.status || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          porTipo: treinamentosList.reduce((acc: any, t: any) => {
            acc[t.tipo || 'outros'] = (acc[t.tipo || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          lista: treinamentosList.slice(0, 20).map((t: any) => ({
            titulo: t.titulo,
            tipo: t.tipo,
        ...
        propostas: {
          total: propostasList.length,
          valorTotal: propostasList.reduce((sum: number, p: any) => sum + Number(p.valorTotal || p.valor || 0), 0),
          porStatus: propostasList.reduce((acc: any, p: any) => {
            acc[p.status || 'outros'] = (acc[p.status || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          lista: propostasList.slice(0, 20).map((p: any) => ({
            titulo: p.titulo,
            clienteNome: p.clienteNome || p.cliente,
            valorTotal: p.valorTotal || p.valor,
            status: p.status,
            dataProposta: p.dataProposta || p.criadoEm,
            margemLucro: p.margemLucro,
          })),
        },

        publicacoes: {
          total: publicacoesList.length,
          porTipo: publicacoesList.reduce((acc: any, p: any) => {
            acc[p.tipo || 'artigo'] = (acc[p.tipo || 'artigo'] || 0) + 1;
            return acc;
          }, {}),
          porStatus: publicacoesList.reduce((acc: any, p: any) => {
            acc[p.status || 'publicado'] = (acc[p.status || 'publicado'] || 0) + 1;
            return acc;
          }, {}),
          porAno: publicacoesList.reduce((acc: any, p: any) => {
            const ano = String(p.anoPublicacao || 'N/A');
            acc[ano] = (acc[ano] || 0) + 1;
            return acc;
          }, {}),
          lista: publicacoesList.slice(0, 20).map((p: any) => ({
            titulo: p.titulo,
            autores: p.autores,
            tipo: p.tipo,
            status: p.status,
            anoPublicacao: p.anoPublicacao,
            revista: p.revista,
            doi: p.doi,
          })),
        },
        ...
        processosMonitorados: {
          total: processosMonitoradosList.length,
          porOrgao: processosMonitoradosList.reduce((acc: any, p: any) => {
            acc[p.orgao || 'outros'] = (acc[p.orgao || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          lista: processosMonitoradosList.slice(0, 20).map((p: any) => ({
        ...
        baseConhecimento: {
          total: baseConhecimentoList.length,
          porTipo: baseConhecimentoList.reduce((acc: any, b: any) => {
            acc[b.tipo || 'outros'] = (acc[b.tipo || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          porCategoria: baseConhecimentoList.reduce((acc: any, b: any) => {
            acc[b.categoria || 'outros'] = (acc[b.categoria || 'outros'] || 0) + 1;
            return acc;
          }, {}),
          lista: baseConhecimentoList.slice(0, 20).map((b: any) => ({
            titulo: b.titulo,
            tipo: b.tipo,
            categoria: b.categoria,
            status: b.status,
            versao: b.versao,
            visualizacoes: b.visualizacoes,
          })),
        },
      };
      ...
      const isDbConnError = error?.code === '57P01' || error?.code === 'ECONNRESET' || error?.code === 'ECONNREFUSED' || (error?.message || '').includes('terminating connection');
      if (isDbConnError) {
        console.warn('DB connection dropped during report generation — retrying in 1.5s...');
        await new Promise(r => setTimeout(r, 1500));
        try {
          // Minimal retry: return partial empty report so PDF still generates
          const retryReport = {
            geradoEm: new Date().toISOString(),
            unidade: req.user.role === 'admin' || req.user.cargo === 'diretor' ? 'Todas as Unidades' : req.user.unidade,
            resumoGeral: { totalEmpreendimentos: 0, totalLicencas: 0, totalDemandas: 0, totalVeiculos: 0, totalEquipamentos: 0, totalFuncionarios: 0, totalContratos: 0, totalCampanhas: 0, totalProjetos: 0, totalMembrosEquipe: 0, totalRegistrosCampo: 0, totalIntercorrencias: 0, totalAmostras: 0, totalFornecedores: 0, totalTreinamentos: 0, totalPropostas: 0, totalPublicacoes: 0, totalProcessos: 0, totalBaseConhecimento: 0 },
            licencas: { total: 0, vencidas: 0, proximasVencer: 0, vigentes: 0, porTipo: {}, lista: [] },
            demandas: { total: 0, abertas: 0, concluidas: 0, atrasadas: 0, porPrioridade: {}, lista: [] },
            frota: { total: 0, disponiveis: 0, emManutencao: 0, emUso: 0, lista: [] },
            equipamentos: { total: 0, ativos: 0, emManutencao: 0, porTipo: {}, lista: [] },
            rh: { total: 0, ativos: 0, ferias: 0, afastados: 0, porCargo: {}, lista: [] },
            contratos: { total: 0, ativos: 0, vencendo: 0, valorTotal: 0, lista: [] },
            campanhas: { total: 0, ativas: 0, concluidas: 0, lista: [] },
            projetos: { total: 0, emAndamento: 0, concluidos: 0, planejamento: 0, atrasados: 0, porStatus: {}, porTipo: {}, lista: [] },
            financeiro: { totalReceitas: 0, totalDespesas: 0, saldoAtual: 0, totalPendente: 0, porCategoria: [], porEmpreendimento: [], evolucaoMensal: [] },
            tarefas: { total: 0, pendentes: 0, concluidas: 0, porCategoria: {} },
            empreendimentos: { total: 0, porTipo: {}, lista: [] },
            campo: { total: 0, campanhas: 0, ameacadas: 0, porGrupo: {}, porCampanha: {}, lista: [] },
            intercorrencias: { total: 0, porTipo: {}, lista: [] },
            amostras: { total: 0, porStatus: {}, porTipo: {}, lista: [] },
            fornecedores: { total: 0, porTipo: {}, porAvaliacao: {}, lista: [] },
            treinamentos: { total: 0, porStatus: {}, porTipo: {}, lista: [] },
            propostas: { total: 0, valorTotal: 0, porStatus: {}, lista: [] },
            publicacoes: { total: 0, porTipo: {}, porStatus: {}, porAno: {}, lista: [] },
            processosMonitorados: { total: 0, porOrgao: {}, lista: [] },
            baseConhecimento: { total: 0, porTipo: {}, porCategoria: {}, lista: [] },
            _aviso: 'Relatório gerado com dados parciais devido a instabilidade temporária de conexão. Tente novamente para dados completos.',
          };
          return res.json(retryReport);
        } catch (retryErr) {
  ...
  app.post('/api/base-conhecimento/upload-arquivo', requireAuth, bcUpload.single('file'), async (req, res) => {
    try {
      ...
      const driveResult = await uploadDocumentToGoogleDrive(fileName2, req.file.buffer, req.file.mimetype);
      if (!driveResult.success || !driveResult.fileId) {
        throw new Error(driveResult.error || 'Falha ao salvar no Google Drive');
      }
      const caminho2 = `gdrive://${driveResult.fileId}`;
  ...
  const geoUpload = multerGeo({
    storage: multerGeo.memoryStorage(),
    limits: { fileSize: 100 * 1024 * 1024 }, // 100 MB
  });

  app.post('/api/camadas-geoespaciais/upload', requireAuth, (req, res, next) => {
    geoUpload.single('file')(req, res, (err) => {
      if (err) {
        if ((err as any).code === 'LIMIT_FILE_SIZE') {
          return res.status(413).json({ error: 'Arquivo muito grande. O limite é 100 MB.' });
        }
        return res.status(400).json({ error: `Erro no recebimento do arquivo: ${err.message}` });
      }
      next();
    });
  }, async (req, res) => {
    try {
      ...
      if (fileName.endsWith('.kmz')) {
        ...
        const toGeoJSONMod = await import('@mapbox/togeojson');
        const toGeoJSON = (toGeoJSONMod as any).default ?? toGeoJSONMod;
        ...
        // Shapefile support — ZIP contendo .shp + .dbf, ou .shp direto
        ...
        if (fileName.endsWith('.zip')) {
          ...
          const allFiles = Object.keys(zip.files).filter(n => !n.includes('__MACOSX') && !zip.files[n].dir);
          const shpEntry = allFiles.find(n => n.toLowerCase().endsWith('.shp'));
          const dbfEntry = allFiles.find(n => n.toLowerCase().endsWith('.dbf'));

          if (!shpEntry) {
            return res.status(400).json({ error: 'Nenhum arquivo .shp encontrado dentro do ZIP. Certifique-se de incluir pelo menos o arquivo .shp (e .dbf para atributos).' });
          }

          shpBuffer = Buffer.from(await zip.files[shpEntry].async('arraybuffer'));
          if (dbfEntry) {
            dbfBuffer = Buffer.from(await zip.files[dbfEntry].async('arraybuffer'));
          }
        } else {
          // Upload direto de .shp (sem atributos)
          shpBuffer = fileBuffer;
        }

        // Converte para GeoJSON usando a biblioteca shapefile
        const collection = await shapefileLib.read(
          shpBuffer.buffer.slice(shpBuffer.byteOffset, shpBuffer.byteOffset + shpBuffer.byteLength),
          dbfBuffer
            ? dbfBuffer.buffer.slice(dbfBuffer.byteOffset, dbfBuffer.byteOffset + dbfBuffer.byteLength)
            : undefined
        );

        if (!collection || !collection.features) {
          return res.status(400).json({ error: 'Shapefile inválido ou sem geometrias.' });
        }

        // Filtra features sem geometria
        geojsonData = {
          type: 'FeatureCollection',
          features: collection.features.filter((f: any) => f && f.geometry),
        };

        if (geojsonData.features.length === 0) {
          return res.status(400).json({ error: 'Shapefile sem geometrias válidas.' });
        }

      } else {
        return res.status(400).json({ error: 'Formato não suportado. Use KMZ, KML, GeoJSON, ou Shapefile (.zip com .shp/.dbf ou .shp direto).' });
      }
  ...
  app.get('/api/team-members', requireAuth, async (req: any, res) => {
    try {
      ...
      const rawResult = await db.select({
        ...
        whatsapp: users.whatsapp,
        nomeRh: rhRegistros.nomeColaborador,
      }).from(users)
        .leftJoin(rhRegistros, eq(rhRegistros.contatoEmail, users.email))
        .where(conditions.length > 0 ? and(...conditions) : undefined)
        .orderBy(users.email);

      const result = rawResult.map(u => ({
        ...u,
        nome: u.nomeRh || u.email.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase()),
        nomeRh: undefined,
      }));
  ...
  app.delete('/api/users/:id', requireAuth, async (req: any, res) => {
    try {
      ...
      // Verificar senha do admin antes de prosseguir
      const { confirmPassword } = req.body || {};
      if (!confirmPassword) return res.status(400).json({ error: 'Senha de confirmação obrigatória' });
      const adminUser = await storage.getUser(req.session.userId!);
      if (!adminUser) return res.status(401).json({ error: 'Sessão inválida' });
      const { compare } = await import('bcrypt');
      const senhaOk = await compare(confirmPassword, adminUser.password || '');
      ...
      const run = (q: Promise<any>) => q.catch(() => {});

      // ── Deletar linhas exclusivas do usuário ──────────────────────────────────
      await run(db.execute(sql`DELETE FROM ai_conversations WHERE user_id = ${id}`));
      await run(db.execute(sql`DELETE FROM ai_logs WHERE user_id = ${id}`));
      await run(db.execute(sql`DELETE FROM comunicado_curtidas WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM comunicado_enquete_votos WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM comunicado_leitura_obrigatoria WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM comunicado_mencoes WHERE usuario_mencionado_id = ${id}`));
      await run(db.execute(sql`DELETE FROM comunicado_reacoes WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM comunicado_visualizacoes WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM historicos_pontuacao WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM membros_equipe WHERE user_id = ${id}`));
      await run(db.execute(sql`DELETE FROM offline_sync_queue WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM pontuacoes_gamificacao WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM realtime_notifications WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM registro_horas WHERE colaborador_id = ${id}`));
      await run(db.execute(sql`DELETE FROM treinamento_participantes WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM usuario_conquistas WHERE usuario_id = ${id}`));
      await run(db.execute(sql`DELETE FROM whatsapp_alert_configs WHERE user_id = ${id}`));
      await run(db.execute(sql`DELETE FROM whatsapp_contact_group_members WHERE user_id = ${id}`));
      await run(db.execute(sql`DELETE FROM whatsapp_message_logs WHERE user_id = ${id}`));
      await run(db.execute(sql`DELETE FROM session WHERE sess::jsonb->'userId' = ${id}::text::jsonb`));

      // ── Nulificar colunas anuláveis ───────────────────────────────────────────
      await run(db.execute(sql`UPDATE arquivos SET uploader_id = NULL WHERE uploader_id = ${id}`));
      await run(db.execute(sql`UPDATE audit_logs SET usuario_id = NULL WHERE usuario_id = ${id}`));
      await run(db.execute(sql`UPDATE contrato_documentos SET uploader_id = NULL WHERE uploader_id = ${id}`));
      await run(db.execute(sql`UPDATE empreendimentos SET coordenador_id = NULL WHERE coordenador_id = ${id}`));
      await run(db.execute(sql`UPDATE condicionantes SET responsavel_id = NULL WHERE responsavel_id = ${id}`));
      await run(db.execute(sql`UPDATE amostras SET coletor_id = NULL WHERE coletor_id = ${id}`));
      await run(db.execute(sql`UPDATE historico_reembolso SET usuario_id = NULL WHERE usuario_id = ${id}`));
      await run(db.execute(sql`UPDATE metas_custo_projeto SET coordenador_id = NULL WHERE coordenador_id = ${id}`));
      await run(db.execute(sql`UPDATE membros_equipe SET coordenador_id = NULL WHERE coordenador_id = ${id}`));
      await run(db.execute(sql`UPDATE pedidos_reembolso SET coordenador_id = NULL WHERE coordenador_id = ${id}`));
      await run(db.execute(sql`UPDATE pedidos_reembolso SET diretor_id = NULL WHERE diretor_id = ${id}`));
      await run(db.execute(sql`UPDATE pedidos_reembolso SET financeiro_id = NULL WHERE financeiro_id = ${id}`));
      await run(db.execute(sql`UPDATE registro_horas SET aprovado_por = NULL WHERE aprovado_por = ${id}`));
      await run(db.execute(sql`UPDATE solicitacoes_recursos SET diretor_id = NULL WHERE diretor_id = ${id}`));
      await run(db.execute(sql`UPDATE solicitacoes_recursos SET solicitante_id = NULL WHERE solicitante_id = ${id}`));
      await run(db.execute(sql`UPDATE notificacoes SET user_id = NULL WHERE user_id = ${id}`));
      await run(db.execute(sql`UPDATE historico_movimentacoes SET user_id = NULL WHERE user_id = ${id}`));
      await run(db.execute(sql`UPDATE historico_demandas_movimentacoes SET usuario_id = NULL WHERE usuario_id = ${id}`));

      // ── Reatribuir registros NOT NULL ao admin executor ───────────────────────
      await run(db.execute(sql`UPDATE amostras SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE base_conhecimento SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE blog_artigos SET autor_id = ${adminId} WHERE autor_id = ${id}`));
      await run(db.execute(sql`UPDATE camadas_geoespaciais SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE comunicado_comentarios SET autor_id = ${adminId} WHERE autor_id = ${id}`));
      await run(db.execute(sql`UPDATE comunicado_eventos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE comunicado_templates SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE comunicados SET autor_id = ${adminId} WHERE autor_id = ${id}`));
      await run(db.execute(sql`UPDATE dds_registros SET responsavel_id = ${adminId} WHERE responsavel_id = ${id}`));
      await run(db.execute(sql`UPDATE demandas SET responsavel_id = ${adminId} WHERE responsavel_id = ${id}`));
      await run(db.execute(sql`UPDATE demandas SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE documentos SET uploaded_by = ${adminId} WHERE uploaded_by = ${id}`));
      await run(db.execute(sql`UPDATE empreendimentos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE equipamentos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE financeiro_lancamentos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE fornecedores SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE links_uteis SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE murais SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE newsletter_edicoes SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE orcamentos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE pedidos_reembolso SET solicitante_id = ${adminId} WHERE solicitante_id = ${id}`));
      await run(db.execute(sql`UPDATE processos_monitorados SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE projetos SET coordenador_id = ${adminId} WHERE coordenador_id = ${id}`));
      await run(db.execute(sql`UPDATE projetos SET responsavel_id = ${adminId} WHERE responsavel_id = ${id}`));
      await run(db.execute(sql`UPDATE propostas_comerciais SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE ramais_contatos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE scheduled_reports SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE tarefa_atualizacoes SET usuario_id = ${adminId} WHERE usuario_id = ${id}`));
      await run(db.execute(sql`UPDATE tarefas SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE tarefas SET responsavel_id = ${adminId} WHERE responsavel_id = ${id}`));
      await run(db.execute(sql`UPDATE treinamentos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE veiculos SET criado_por = ${adminId} WHERE criado_por = ${id}`));
      await run(db.execute(sql`UPDATE whatsapp_contact_groups SET created_by = ${adminId} WHERE created_by = ${id}`));
      await run(db.execute(sql`UPDATE comentarios_demandas SET autor_id = ${adminId} WHERE autor_id = ${id}`));

      const [deleted] = await db.delete(users).where(eq(users.id, id)).returning({ id: users.id, email: users.email });
      if (!deleted) return res.status(404).json({ error: 'Usuário não encontrado' });
      res.json({ success: true, deleted });
    } catch (error: any) {
      console.error('[DELETE /api/users/:id] Erro:', error.message);
      res.status(500).json({ error: error.message });
    }
  });
...
Retorne APENAS o JSON, sem markdown.`;
  ...
  // ─── VÍNCULOS DE DOCUMENTOS ──────────────────────────────────────────────
  app.get("/api/documento-vinculos", requireAuth, async (req, res) => {
    try {
      const { empreendimentoId } = req.query;
      const items = empreendimentoId
        ? await db.select().from(documentoVinculos).where(eq(documentoVinculos.empreendimentoId, parseInt(empreendimentoId as string))).orderBy(desc(documentoVinculos.criadoEm))
        : await db.select().from(documentoVinculos).orderBy(desc(documentoVinculos.criadoEm));
      res.json(items);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  app.post("/api/documento-vinculos", requireAuth, async (req: any, res) => {
    try {
      const data = insertDocumentoVinculoSchema.parse({ ...req.body, criadoPor: req.user?.email });
      const [item] = await db.insert(documentoVinculos).values(data).returning();
      res.json(item);
    } catch (e: any) { res.status(400).json({ error: e.message }); }
  });
  app.delete("/api/documento-vinculos/:id", requireAuth, async (req, res) => {
    try {
      await db.delete(documentoVinculos).where(eq(documentoVinculos.id, parseInt(req.params.id)));
      res.json({ success: true });
    } catch (e: any) { res.status(400).json({ error: e.message }); }
  });
  ...
  // ── One-time migration: add parametros column for non-fauna monitoring types ──
  try {
    await db.execute(sql`ALTER TABLE campo_registros ADD COLUMN IF NOT EXISTS parametros jsonb`);
  } catch (migErr: any) {
    if (!migErr?.message?.includes('already exists')) console.warn('campo migration warning:', migErr?.message);
  }

  // GET /api/campo/species-lookup?nome=Genus+species
  app.get('/api/campo/species-lookup', requireAuth, (req, res) => {
    const nome = String(req.query.nome || "").trim();
    if (!nome) return res.status(400).json({ message: "nome é obrigatório" });
    res.json(lookupSpecies(nome));
  });

  // POST /api/campo/species-lookup-batch  { names: string[] }
  // Returns a map of scientificName -> enriched status fields
  app.post('/api/campo/species-lookup-batch', requireAuth, (req, res) => {
    const names: string[] = Array.isArray(req.body?.names) ? req.body.names : [];
    const result: Record<string, {
      iucn: string; mma: string; cites: string; pan: string; nomeVernacular: string;
    }> = {};
    for (const raw of names) {
      const nome = String(raw || "").trim();
      if (!nome) continue;
      const lu = lookupSpecies(nome);
      // lu.iucn is a plain string code e.g. "LC", "EN" — NOT an object
      const iucnCat  = lu?.iucn ?? "";
      const mmaCat   = lu?.mma?.categoria ?? "";
      const citesApp = lu?.cites ?? "";
      const panName  = lu?.pan?.pan ?? "";
      const vernac   = lu?.nomeVernacular ?? lu?.pan?.nomePopular ?? "";
      result[nome] = {
        iucn:  iucnCat,
        mma:   mmaCat,
        cites: citesApp ? `Apêndice ${citesApp}` : "",
        pan:   panName,
        nomeVernacular: vernac,
      };
    }
    res.json(result);
  });
  ...
  app.get('/api/campo', requireAuth, async (req, res) => {
    try {
      ...
      // Re-enrich with speciesDB so PAN/IUCN/CITES are always up-to-date,
      // even for records imported before the official DB was available.
      const enriched = result.map((r: any) => {
        const nome = (r.nomeCientifico || "").trim();
        if (!nome) return r;
        const lu = lookupSpecies(nome);
        if (!lu?.found) return r;
        return {
          ...r,
          iucn:     lu.iucn     || r.iucn,
          ibamaMma: lu.mma?.categoria || r.ibamaMma,
          cites:    lu.cites    || r.cites,
          pan:      lu.pan?.pan || r.pan || "",    // official PAN name wins; else keep stored value
          nomeComum: r.nomeComum || lu.nomeVernacular || "",
        };
      });

      // Batch-load fotos for all records in a single query
      const ids = enriched.map((r: any) => r.id);
      let fotosMap: Record<number, { id: number; url: string }[]> = {};
      if (ids.length > 0) {
        const allFotos = await db.select().from(campoFotos).where(inArray(campoFotos.registroId, ids));
        allFotos.forEach((f: any) => {
          if (!fotosMap[f.registroId]) fotosMap[f.registroId] = [];
          fotosMap[f.registroId].push({ id: f.id, url: f.url });
        });
      }

      const withFotos = enriched.map((r: any) => ({ ...r, fotos: fotosMap[r.id] || [] }));
      res.json(withFotos);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // GET campo registro by id with photos
  app.get('/api/campo/:id', requireAuth, async (req, res, next) => {
    try {
      const id = parseInt(req.params.id);
      if (!Number.isFinite(id)) return next(); // permite que outras rotas (ex: /outliers) sejam tentadas
  ...
  // ── F5: Helper para gravar audit log de registros de campo ──────────────
  async function logCampoAudit(
    registroId: number,
    userId: number | null,
    userName: string,
    acao: 'create' | 'update' | 'delete',
    valorAnterior: Record<string, any> | null,
    valorNovo: Record<string, any> | null,
  ) {
    try {
      let camposAlterados: string[] = [];
      if (acao === 'update' && valorAnterior && valorNovo) {
        const skip = new Set(['atualizadoEm', 'criadoEm', 'sincronizado']);
        for (const k of Object.keys(valorNovo)) {
          if (skip.has(k)) continue;
          const a = valorAnterior[k]; const b = valorNovo[k];
          const an = a === null || a === undefined ? '' : String(a);
          const bn = b === null || b === undefined ? '' : String(b);
          if (an !== bn) camposAlterados.push(k);
        }
        if (camposAlterados.length === 0) return; // nada mudou
      }
      await db.insert(campoAuditLog).values({
        registroId,
        userId: userId || null,
        userName: userName || 'desconhecido',
        acao,
        camposAlterados: camposAlterados.length ? camposAlterados : null,
        valorAnterior,
        valorNovo,
      });
    } catch (e) {
      console.error('Erro ao gravar audit log de campo:', e);
    }
  }

  // GET histórico de alterações de um registro (F5) — com isolamento por unidade
  app.get('/api/campo/:id/audit', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ message: 'ID inválido' });
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: 'Unauthorized' });
      // Verifica posse: o registro precisa existir e pertencer à mesma unidade do usuário
      const [reg] = await db.select({ id: campoRegistros.id, unidade: campoRegistros.unidade })
        .from(campoRegistros).where(eq(campoRegistros.id, id));
      if (!reg) return res.status(404).json({ message: 'Registro não encontrado' });
      if (reg.unidade && user.unidade && reg.unidade !== user.unidade) {
        return res.status(403).json({ message: 'Sem acesso a este registro' });
      }
      const logs = await db.select().from(campoAuditLog)
        .where(eq(campoAuditLog.registroId, id))
        .orderBy(desc(campoAuditLog.criadoEm))
        .limit(200);
      res.json(logs);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // POST identificação por foto via IA (F7)
  const identifyPhotoUpload = (await import('multer')).default({
    storage: (await import('multer')).memoryStorage(),
    limits: { fileSize: 8 * 1024 * 1024 }
  });
  app.post('/api/campo/identify-photo', requireAuth, identifyPhotoUpload.single('foto'), async (req, res) => {
    try {
      ...
      if (!apiKey) return res.status(503).json({ message: 'IA de identificação não configurada (OPENAI_API_KEY ausente).' });
      const { default: OpenAI } = await import('openai');
      const client = new OpenAI({ apiKey });
      const bioma = (req.body.bioma || 'Cerrado / Mata Atlântica brasileiros').toString();
      const grupo = (req.body.grupo || 'fauna').toString();
      ...
      const completion = await client.chat.completions.create({
        ...
        response_format: { type: 'json_object' },
        messages: [
          { role: 'system', content: 'Você é um biólogo taxonomista especialista em fauna e flora brasileiras. Dada uma foto, retorne as 3 espécies mais prováveis em ordem decrescente de confiança. Considere o bioma e o grupo informados. Responda APENAS em JSON com a chave "sugestoes" = array de 3 objetos { nomeCientifico, nomeComum, confianca (0-100), justificativa }. nomeCientifico em formato binomial (Genus species).' },
          { role: 'user', content: [
            { type: 'text', text: `Bioma: ${bioma}. Grupo taxonômico esperado: ${grupo}. Identifique a espécie na foto.` },
      ...
      res.status(500).json({ message: err?.message || 'Falha na identificação' });
    }
  });
  ...
  app.post('/api/campo', requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: "Unauthorized" });
      const data = applySpeciesLookup({ ...req.body, unidade: user.unidade || 'goiania' });

      const nomeCient = (data.nomeCientifico || "").trim();
      const empId = data.empreendimentoId ? Number(data.empreendimentoId) : null;

      const [registro] = await db.insert(campoRegistros).values(data).returning();

      // Detecção de espécies novas — feita APÓS o insert, comparando o id mínimo:
      // se não existe outro registro anterior com essa espécie nesse empreendimento,
      // este é o primeiro. Resistente a concorrência (não há check-then-insert).
      let isNewSpecies = false;
      if (registro && nomeCient && empId) {
        try {
          const previous = await db.select({ id: campoRegistros.id })
            .from(campoRegistros)
            .where(and(
              eq(campoRegistros.empreendimentoId, empId),
              sql`LOWER(TRIM(${campoRegistros.nomeCientifico})) = LOWER(${nomeCient})`,
              sql`${campoRegistros.id} < ${registro.id}`
            ))
            .limit(1);
          isNewSpecies = previous.length === 0;
        } catch (e) {
          console.error('Erro ao verificar espécie nova:', e);
        }
      }
      if (isNewSpecies && registro) {
        try {
          const { notificationService } = await import('./services/notificationService');
          await notificationService.createNotification({
            tipo: 'especie_nova',
            titulo: `🚨 Primeira ocorrência: ${nomeCient}`,
            mensagem: `Espécie ${nomeCient} registrada pela primeira vez neste empreendimento${data.unidadeAmostral ? ` (UA: ${data.unidadeAmostral})` : ''}.`,
            severidade: 'media',
            itemId: registro.id,
            metadados: { empreendimentoId: empId, nomeCientifico: nomeCient, grupoTaxonomico: data.grupoTaxonomico },
          });
        } catch (e) {
          console.error('Erro ao notificar espécie nova:', e);
        }
      }

      if (registro) {
        const user2 = await storage.getUser(req.session.userId!);
        await logCampoAudit(registro.id, req.session.userId || null, user2?.nome || user2?.email || 'usuário', 'create', null, registro as any);
      }

      res.status(201).json({ ...registro, isNewSpecies });
    } catch (err: any) { res.status(400).json({ message: err.message }); }
  });

  // POST bulk import from spreadsheet
  app.post('/api/campo/import', requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: "Unauthorized" });
      ...
      // Enrich each record with CITES / IUCN / MMA / PAN / vernacular data
      const today = new Date().toISOString().slice(0, 10);
      const withMeta = registros.map((r: any) => {
        // Sanitize required fields: data must be a valid ISO date string
        const rawDate = (r.data || "").trim();
        const safeDate = /^\d{4}-\d{2}-\d{2}/.test(rawDate) ? rawDate.slice(0, 10) : today;
        const enriched = { ...r, data: safeDate, unidade: user.unidade || 'goiania', sincronizado: true };
        const nome = (r.nomeCientifico || "").trim();
        const lu = nome ? lookupSpecies(nome) : null;

        // Normalize & enrich — SpeciesDB wins over free-text spreadsheet values
        enriched.iucn     = lu?.iucn || normalizeIucnCode(r.iucn) || "";
        enriched.ibamaMma = normalizeMmaCode(r.ibamaMma, lu?.mma?.categoria);
        enriched.cites    = lu?.cites || normalizeCitesCode(r.cites) || "";
        enriched.pan      = lu?.pan?.pan ? lu.pan.pan : normalizePanValue(r.pan, undefined);
        if (!enriched.nomeComum && lu?.nomeVernacular) enriched.nomeComum = lu.nomeVernacular;
      ...
      const inserted = await db.insert(campoRegistros).values(withMeta).returning({ id: campoRegistros.id });
  ...
  app.post('/api/campo/sync', requireAuth, async (req, res) => {
    try {
      ...
      const withUnidade = registros.map((r: any) => applySpeciesLookup({ ...r, unidade: user.unidade || 'goiania', sincronizado: true }));
  ...
  app.put('/api/campo/:id', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const [before] = await db.select().from(campoRegistros).where(eq(campoRegistros.id, id));
      const enriched = applySpeciesLookup(req.body);
      const [updated] = await db.update(campoRegistros).set({ ...enriched, atualizadoEm: new Date() }).where(eq(campoRegistros.id, id)).returning();
      if (!updated) return res.status(404).json({ message: "Registro não encontrado" });
      const user = await storage.getUser(req.session.userId!);
      await logCampoAudit(id, req.session.userId || null, user?.nome || user?.email || 'usuário', 'update', before as any, updated as any);
      res.json(updated);
    } catch (err: any) { res.status(400).json({ message: err.message }); }
  });

  // POST backfill conservation status for existing records missing data
  app.post('/api/campo/backfill', requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: "Unauthorized" });

      // Fetch all records for this unit that have a scientific name
      const all = await db.select().from(campoRegistros).where(eq(campoRegistros.unidade, user.unidade || 'goiania'));
      let updated = 0;
      ...
      for (const r of all) {
        ...
        if (!nome) continue;
        const lu = lookupSpecies(nome);
        if (!lu.found) continue;

        // Only update if we have new data that wasn't there before
        const newIucn     = lu.iucn || "";
        const newMma      = normalizeMmaCode(r.ibamaMma || "", lu.mma?.categoria);
        const newCites    = lu.cites || normalizeCitesCode(r.cites || "") || "";
        const newPan      = lu.pan?.pan || normalizePanValue(r.pan || "", undefined);
        const newNome     = r.nomeComum || lu.nomeVernacular || "";
        ...
        await db.update(campoRegistros)
          .set({
            iucn:      newIucn  || r.iucn     || "",
            ibamaMma:  newMma   || r.ibamaMma || "",
            cites:     newCites || r.cites    || "",
            pan:       newPan   || r.pan      || "",
            nomeComum: newNome  || r.nomeComum|| "",
            atualizadoEm: new Date(),
          })
          .where(eq(campoRegistros.id, r.id));
        updated++;
      }

      res.json({ total: all.length, updated });
    } catch (err: any) {
  ...
  // Helper: reseta sequência para 1 se a tabela estiver vazia
  async function resetCampoSequenceIfEmpty() {
    const [{ total }] = await db.select({ total: sql<number>`count(*)::int` }).from(campoRegistros);
    if (total === 0) {
      await db.execute(sql`SELECT setval(pg_get_serial_sequence('campo_registros', 'id'), 1, false)`);
    }
  }

  // DELETE campo registro (individual)
  app.delete('/api/campo/:id', requireAuth, async (req, res) => {
    try {
      ...
      if (before) {
        const user = await storage.getUser(req.session.userId!);
        await logCampoAudit(id, req.session.userId || null, user?.nome || user?.email || 'usuário', 'delete', before as any, null);
      }
      await resetCampoSequenceIfEmpty();
      res.json({ message: "Registro excluído" });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // DELETE campo registros (bulk)
  app.delete('/api/campo', requireAuth, async (req, res) => {
    try {
      const { ids } = req.body as { ids: number[] };
      if (!Array.isArray(ids) || !ids.length) return res.status(400).json({ message: "ids obrigatório" });
      await db.delete(campoFotos).where(inArray(campoFotos.registroId, ids));
      await db.delete(campoRegistros).where(inArray(campoRegistros.id, ids));
      await resetCampoSequenceIfEmpty();
      res.json({ message: `${ids.length} registros excluídos` });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  // ────────────────────────────────────────────────────────────────────────
  // F9: Detecção automática de outliers ecológicos
  // Para cada espécie com ≥ MIN_OCC ocorrências georreferenciadas, calcula
  // o centróide geográfico (média lat/lon) e o desvio-padrão da distância em
  // km. Marca como outlier qualquer registro cuja distância ao centróide
  // ultrapasse max(OUT_SIGMA * desvio, OUT_FLOOR_KM) — alerta de identificação
  // suspeita (espécie fora do range esperado de bioma).
  // ────────────────────────────────────────────────────────────────────────
  app.get('/api/campo/outliers', requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: 'Unauthorized' });
      const MIN_OCC = 5;
      const OUT_SIGMA = 3;
      const OUT_FLOOR_KM = 300;
      const conditions: any[] = [eq(campoRegistros.unidade, user.unidade || 'goiania')];
      if (req.query.empreendimentoId) {
        const empId = parseInt(req.query.empreendimentoId as string);
        if (!Number.isFinite(empId)) return res.status(400).json({ message: "empreendimentoId inválido" });
        // Valida posse: empreendimento precisa pertencer à unidade do usuário
        const [emp] = await db.select({ id: empreendimentos.id, unidade: empreendimentos.unidade })
          .from(empreendimentos).where(eq(empreendimentos.id, empId));
        if (!emp) return res.status(404).json({ message: "Empreendimento não encontrado" });
        if (emp.unidade && user.unidade && emp.unidade !== user.unidade) {
          return res.status(403).json({ message: "Sem acesso a este empreendimento" });
        }
        conditions.push(eq(campoRegistros.empreendimentoId, empId));
      }
      // Projeção mínima de campos para evitar pressão de memória em unidades grandes
      const registros = await db.select({
        id: campoRegistros.id,
        latitude: campoRegistros.latitude,
        longitude: campoRegistros.longitude,
        nomeCientifico: campoRegistros.nomeCientifico,
        nomeComum: campoRegistros.nomeComum,
        grupoTaxonomico: campoRegistros.grupoTaxonomico,
        data: campoRegistros.data,
        unidadeAmostral: campoRegistros.unidadeAmostral,
      }).from(campoRegistros).where(and(...conditions));
      ...
      // agrupa por espécie
      const bySpecies = new Map<string, { id: number; lat: number; lon: number; data?: string | null; unidadeAmostral?: string | null; grupoTaxonomico?: string | null; nomeComum?: string | null; }[]>();
      registros.forEach(r => {
        const sp = (r.nomeCientifico || '').trim().toLowerCase();
        if (!sp) return;
        const lat = parseNum(r.latitude); const lon = parseNum(r.longitude);
        if (lat === null || lon === null) return;
        if (Math.abs(lat) > 90 || Math.abs(lon) > 180) return;
        if (!bySpecies.has(sp)) bySpecies.set(sp, []);
        bySpecies.get(sp)!.push({ id: r.id, lat, lon, data: r.data, unidadeAmostral: r.unidadeAmostral, grupoTaxonomico: r.grupoTaxonomico, nomeComum: r.nomeComum });
      });
      const outliers: any[] = [];
      bySpecies.forEach((points, sp) => {
        if (points.length < MIN_OCC) return;
        const meanLat = points.reduce((s, p) => s + p.lat, 0) / points.length;
        const meanLon = points.reduce((s, p) => s + p.lon, 0) / points.length;
        const dists = points.map(p => haversineKm([meanLat, meanLon], [p.lat, p.lon]));
        const meanD = dists.reduce((s, d) => s + d, 0) / dists.length;
        const variance = dists.reduce((s, d) => s + (d - meanD) ** 2, 0) / dists.length;
        const stdD = Math.sqrt(variance);
        const threshold = Math.max(OUT_SIGMA * stdD, OUT_FLOOR_KM);
        points.forEach((p, i) => {
          if (dists[i] > threshold) {
            outliers.push({
              ...
              grupoTaxonomico: p.grupoTaxonomico,
              data: p.data,
              unidadeAmostral: p.unidadeAmostral,
              latitude: p.lat,
              longitude: p.lon,
              distanciaKm: Math.round(dists[i]),
              centroide: { latitude: Number(meanLat.toFixed(5)), longitude: Number(meanLon.toFixed(5)) },
              ocorrenciasDaEspecie: points.length,
              thresholdKm: Math.round(threshold),
              motivo: `Distância de ${Math.round(dists[i])}km do centróide das ${points.length} ocorrências desta espécie (threshold ${Math.round(threshold)}km).`,
            });
          }
        });
      });
      outliers.sort((a, b) => b.distanciaKm - a.distanciaKm);
      res.json({
        total: outliers.length,
        params: { minOcorrencias: MIN_OCC, sigma: OUT_SIGMA, distanciaMinimaKm: OUT_FLOOR_KM },
        speciesAnalisadas: [...bySpecies.values()].filter(v => v.length >= MIN_OCC).length,
        outliers,
      });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });
  ...
  // ────────────────────────────────────────────────────────────────────────
  // F10: Alertas inteligentes (espécie ausente há N campanhas / queda de riqueza por UA)
  // ────────────────────────────────────────────────────────────────────────
  app.get('/api/campo/alertas-inteligentes', requireAuth, async (req, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: 'Unauthorized' });
      const MIN_ABSENT_CAMP = 3;     // espécie tem que estar ausente nas N últimas campanhas
      const MIN_CAMP_HIST = 4;        // exige ao menos N campanhas no histórico para alertar absence
      const DROP_PCT = 0.40;          // 40% de queda de riqueza
      const DROP_PCT_HIGH = 0.60;     // ≥60% → severidade alta
      const MIN_BASELINE_RIQUEZA = 3; // só alerta queda se baseline tinha ≥3 espécies
      ...
      const THREAT = new Set(['CR','EX','EW','EN','VU','NT','DD']);
      // ── (A) Espécie ausente: por empreendimento, ordena campanhas cronologicamente
      //   e flagga espécies que não aparecem nas N últimas campanhas.
      // ── (B) Queda de riqueza: por UA, compara a última campanha contra a média
      //   das anteriores (precisa de ≥2 campanhas anteriores).
      const alertas: any[] = [];
      // agrupa por empreendimento
      const byEmp = new Map<number | null, typeof registros>();
      registros.forEach(r => {
        const k = r.empreendimentoId ?? null;
        if (!byEmp.has(k)) byEmp.set(k, [] as any);
        (byEmp.get(k) as any).push(r);
      });

      byEmp.forEach((regs, empId) => {
        // Ordena campanhas deterministicamente. Para cada campanha guarda:
        //   minDate (menor data não-nula observada, ou null se nenhuma)
        //   numLabel (valor numérico do rótulo se for parseável, ex.: "5" → 5)
        const campInfo = new Map<string, { minDate: string | null; numLabel: number | null }>();
        regs.forEach(r => {
          if (!r.campanha) return;
          const cur = campInfo.get(r.campanha);
          const d = r.data || null;
          if (!cur) {
            const parsed = parseFloat(r.campanha);
            campInfo.set(r.campanha, { minDate: d, numLabel: Number.isFinite(parsed) ? parsed : null });
          } else if (d && (!cur.minDate || d < cur.minDate)) {
            cur.minDate = d;
          }
        });
        // Critério: 1) numLabel asc (se ambas numéricas) 2) minDate asc (nulls last) 3) string asc
        const campanhasOrd = [...campInfo.entries()].sort((a, b) => {
          const [la, ia] = a, [lb, ib] = b;
          if (ia.numLabel !== null && ib.numLabel !== null && ia.numLabel !== ib.numLabel) return ia.numLabel - ib.numLabel;
          if (ia.minDate && ib.minDate && ia.minDate !== ib.minDate) return ia.minDate < ib.minDate ? -1 : 1;
          if (!!ia.minDate !== !!ib.minDate) return ia.minDate ? -1 : 1; // com data antes de sem data
          return la.localeCompare(lb);
        }).map(([c]) => c);
        if (campanhasOrd.length < MIN_CAMP_HIST) return; // histórico insuficiente
        // Index lookup O(1) para evitar O(n²) com indexOf
        const campIdxMap = new Map<string, number>();
        campanhasOrd.forEach((c, i) => campIdxMap.set(c, i));

        // (A) última campanha de cada espécie
        const lastSeen = new Map<string, { campIdx: number; nomeComum?: string | null; grupo?: string | null; iucn?: string | null; mma?: string | null; }>();
        regs.forEach(r => {
          const sp = (r.nomeCientifico || '').trim();
          if (!sp || !r.campanha) return;
          const idx = campIdxMap.get(r.campanha);
          if (idx === undefined) return;
          const prev = lastSeen.get(sp);
          if (!prev || idx > prev.campIdx) {
            lastSeen.set(sp, { campIdx: idx, nomeComum: r.nomeComum, grupo: r.grupoTaxonomico, iucn: r.iucn, mma: r.ibamaMma });
          }
        });
        const lastCampIdx = campanhasOrd.length - 1;
        lastSeen.forEach((info, sp) => {
          const ausenteEm = lastCampIdx - info.campIdx;
          if (ausenteEm >= MIN_ABSENT_CAMP) {
            const ameacada = THREAT.has(info.iucn || '') || THREAT.has(info.mma || '');
            alertas.push({
              tipo: 'especie_ausente',
              severidade: ausenteEm >= 5 || ameacada ? 'alta' : 'media',
              empreendimentoId: empId,
              nomeCientifico: sp,
              nomeComum: info.nomeComum,
              grupoTaxonomico: info.grupo,
              ultimaCampanha: campanhasOrd[info.campIdx],
              campanhaAtual: campanhasOrd[lastCampIdx],
              campanhasAusente: ausenteEm,
              ameacada,
              titulo: `${sp}${info.nomeComum ? ` (${info.nomeComum})` : ''} não detectada há ${ausenteEm} campanhas`,
              detalhe: `Última detecção: ${campanhasOrd[info.campIdx]}. Não aparece nas ${ausenteEm} campanhas seguintes (até ${campanhasOrd[lastCampIdx]}).${ameacada ? ' Espécie em status de ameaça.' : ''}`,
            });
          }
        });

        // (B) Queda de riqueza por UA (apenas dentro deste empreendimento)
        const byUA = new Map<string, Map<string, Set<string>>>(); // UA → camp → set(sp)
        regs.forEach(r => {
          const ua = r.unidadeAmostral?.trim();
          const sp = r.nomeCientifico?.trim();
          if (!ua || !r.campanha || !sp) return;
          if (!byUA.has(ua)) byUA.set(ua, new Map());
          const m = byUA.get(ua)!;
          if (!m.has(r.campanha)) m.set(r.campanha, new Set());
          m.get(r.campanha)!.add(sp.toLowerCase());
        });
        byUA.forEach((campMap, ua) => {
          const camps = campanhasOrd.filter(c => campMap.has(c));
          if (camps.length < 3) return;
          const ultima = camps[camps.length - 1];
          const anteriores = camps.slice(0, -1);
          const riquezaUlt = campMap.get(ultima)!.size;
          const riquezasAnt = anteriores.map(c => campMap.get(c)!.size);
          const baseline = riquezasAnt.reduce((s, n) => s + n, 0) / riquezasAnt.length;
          if (baseline < MIN_BASELINE_RIQUEZA) return;
          if (riquezaUlt >= baseline * (1 - DROP_PCT)) return;
          const queda = (baseline - riquezaUlt) / baseline;
          alertas.push({
            tipo: 'queda_riqueza',
            severidade: queda >= DROP_PCT_HIGH ? 'alta' : 'media',
            empreendimentoId: empId,
            unidadeAmostral: ua,
            campanhaAtual: ultima,
            riquezaAtual: riquezaUlt,
            riquezaBaseline: Number(baseline.toFixed(1)),
            campanhasComparadas: anteriores.length,
            quedaPct: Math.round(queda * 100),
            titulo: `Queda de ${Math.round(queda * 100)}% na riqueza da UA "${ua}"`,
            detalhe: `Riqueza caiu de média ${baseline.toFixed(1)} espécies (${anteriores.length} campanhas anteriores) para ${riquezaUlt} na campanha "${ultima}".`,
          });
        });

        const lastCampName = campanhasOrd[lastCampIdx];

        // (C) Espécie nova: detectada pela primeira vez na última campanha (mín 4 campanhas históricas)
        if (campanhasOrd.length >= 4) {
          const allPrevSp = new Set<string>();
          const lastCampSp = new Set<string>();
          regs.forEach(r => {
            const sp = (r.nomeCientifico || '').trim().toLowerCase();
            if (!sp || !r.campanha) return;
            const idx = campIdxMap.get(r.campanha);
            if (idx === undefined) return;
            if (idx === lastCampIdx) lastCampSp.add(sp);
            else allPrevSp.add(sp);
          });
          lastCampSp.forEach(spLower => {
            if (allPrevSp.has(spLower)) return;
            const found = regs.find(r => (r.nomeCientifico || '').trim().toLowerCase() === spLower && r.campanha === lastCampName);
            alertas.push({
              tipo: 'especie_nova',
              severidade: 'info',
              empreendimentoId: empId,
              nomeCientifico: found?.nomeCientifico || spLower,
              nomeComum: found?.nomeComum || null,
              grupoTaxonomico: found?.grupoTaxonomico || null,
              campanhaAtual: lastCampName,
              campanhasAnalisadas: campanhasOrd.length - 1,
              titulo: `${found?.nomeCientifico || spLower}${found?.nomeComum ? ` (${found.nomeComum})` : ''} — primeira detecção`,
              detalhe: `Registrada pela primeira vez na campanha "${lastCampName}" após ${campanhasOrd.length - 1} campanhas anteriores sem registro.`,
            });
          });
        }

        // (D) Dominância excessiva: uma espécie > 70% da abundância em uma UA na última campanha
        const DOMINANCE_THRESH = 0.70;
        const MIN_RECORDS_UA_DOM = 5;
        const uaSpCountLast = new Map<string, Map<string, number>>();
        regs.forEach(r => {
          if (r.campanha !== lastCampName) return;
          const ua = r.unidadeAmostral?.trim();
          const sp = r.nomeCientifico?.trim();
          if (!ua || !sp) return;
          if (!uaSpCountLast.has(ua)) uaSpCountLast.set(ua, new Map());
          const m = uaSpCountLast.get(ua)!;
          m.set(sp, (m.get(sp) || 0) + 1);
        });
        uaSpCountLast.forEach((spMap, ua) => {
          const total = [...spMap.values()].reduce((a, b) => a + b, 0);
          if (total < MIN_RECORDS_UA_DOM) return;
          const [topSp, topCount] = [...spMap.entries()].sort((a, b) => b[1] - a[1])[0];
          const pct = topCount / total;
          if (pct < DOMINANCE_THRESH) return;
          const found = regs.find(r => r.nomeCientifico?.trim() === topSp && r.campanha === lastCampName);
          alertas.push({
            tipo: 'dominancia_excessiva',
            severidade: pct >= 0.85 ? 'alta' : 'media',
            empreendimentoId: empId,
            unidadeAmostral: ua,
            campanhaAtual: lastCampName,
            nomeCientifico: topSp,
            nomeComum: found?.nomeComum || null,
            grupoTaxonomico: found?.grupoTaxonomico || null,
            dominanciaPct: Math.round(pct * 100),
            totalRegistros: total,
            totalEspecies: spMap.size,
            titulo: `Dominância de ${Math.round(pct * 100)}% de ${topSp} na UA "${ua}"`,
            detalhe: `${topSp}${found?.nomeComum ? ` (${found.nomeComum})` : ''} representa ${Math.round(pct * 100)}% dos ${total} registros na UA "${ua}" (campanha "${lastCampName}"). ${spMap.size} espécie(s) detectada(s) no total.`,
          });
        });

        // (E) UA ausente: UA regularmente amostrada ausente na última campanha
        const MIN_PREV_WITH_UA = 3;
        const uaAllCamps = new Map<string, Set<string>>();
        regs.forEach(r => {
          const ua = r.unidadeAmostral?.trim();
          if (!ua || !r.campanha) return;
          if (!uaAllCamps.has(ua)) uaAllCamps.set(ua, new Set());
          uaAllCamps.get(ua)!.add(r.campanha);
        });
        uaAllCamps.forEach((campSet, ua) => {
          if (campSet.has(lastCampName)) return;
          const prevCount = campanhasOrd.slice(0, -1).filter(c => campSet.has(c)).length;
          if (prevCount < MIN_PREV_WITH_UA) return;
          const lastCampWithUA = [...campanhasOrd].reverse().find(c => c !== lastCampName && campSet.has(c));
          alertas.push({
            tipo: 'ua_ausente',
            severidade: prevCount >= 5 ? 'alta' : 'media',
            empreendimentoId: empId,
            unidadeAmostral: ua,
            campanhaAtual: lastCampName,
            campanhasComUA: prevCount,
            ultimaCampanhaComUA: lastCampWithUA || null,
            titulo: `UA "${ua}" ausente na campanha "${lastCampName}"`,
            detalhe: `Unidade amostral presente em ${prevCount} campanhas anteriores, mas sem registros na campanha "${lastCampName}". Última coleta confirmada: campanha "${lastCampWithUA || 'desconhecida'}".`,
          });
        });

        // (F) Lacuna temporal: intervalo > 90 dias entre campanhas consecutivas
        const GAP_DAYS_MEDIA = 90;
        const GAP_DAYS_ALTA = 180;
        for (let i = 1; i < campanhasOrd.length; i++) {
          const prevDate = campInfo.get(campanhasOrd[i - 1])?.minDate;
          const currDate = campInfo.get(campanhasOrd[i])?.minDate;
          if (!prevDate || !currDate) continue;
          const gap = Math.round((new Date(currDate).getTime() - new Date(prevDate).getTime()) / 86400000);
          if (gap < GAP_DAYS_MEDIA) continue;
          alertas.push({
            tipo: 'lacuna_temporal',
            severidade: gap >= GAP_DAYS_ALTA ? 'alta' : 'media',
            empreendimentoId: empId,
            campanhaPrev: campanhasOrd[i - 1],
            campanhaAtual: campanhasOrd[i],
            gapDias: gap,
            dataPrev: prevDate,
            dataAtual: currDate,
            titulo: `Lacuna de ${gap} dias entre campanhas "${campanhasOrd[i - 1]}" e "${campanhasOrd[i]}"`,
            detalhe: `Intervalo de ${gap} dias entre "${campanhasOrd[i - 1]}" (${prevDate}) e "${campanhasOrd[i]}" (${currDate}). Lacunas longas comprometem a continuidade amostral e dificultam comparações de diversidade.`,
          });
        }

        // (G) Abundância anômala: espécie com pico > média+2σ nas campanhas anteriores
        if (campanhasOrd.length >= 5) {
          const spCampCount = new Map<string, Map<string, number>>();
          regs.forEach(r => {
            const sp = (r.nomeCientifico || '').trim();
            if (!sp || !r.campanha) return;
            if (!spCampCount.has(sp)) spCampCount.set(sp, new Map());
            const m = spCampCount.get(sp)!;
            m.set(r.campanha, (m.get(r.campanha) || 0) + 1);
          });
          spCampCount.forEach((campMap, sp) => {
            const lastCount = campMap.get(lastCampName) || 0;
            if (lastCount < 3) return;
            const prevCounts = campanhasOrd.slice(0, -1).map(c => campMap.get(c) || 0);
            if (prevCounts.filter(v => v > 0).length < 3) return;
            const mean = prevCounts.reduce((a, b) => a + b, 0) / prevCounts.length;
            const variance = prevCounts.reduce((a, b) => a + (b - mean) ** 2, 0) / prevCounts.length;
            const std = Math.sqrt(variance);
            if (std < 0.5) return;
            const zScore = (lastCount - mean) / std;
            if (zScore < 2.0) return;
            const found = regs.find(r => r.nomeCientifico?.trim() === sp && r.campanha === lastCampName);
            alertas.push({
              tipo: 'abundancia_anomala',
              severidade: zScore >= 3.0 ? 'alta' : 'media',
              empreendimentoId: empId,
              nomeCientifico: sp,
              nomeComum: found?.nomeComum || null,
              grupoTaxonomico: found?.grupoTaxonomico || null,
              campanhaAtual: lastCampName,
              contagem: lastCount,
              media: Number(mean.toFixed(1)),
              desvioPadrao: Number(std.toFixed(1)),
              zScore: Number(zScore.toFixed(1)),
              titulo: `Pico anômalo de ${sp} na campanha "${lastCampName}" (z=${zScore.toFixed(1)}σ)`,
              detalhe: `${lastCount} registros vs média histórica ${mean.toFixed(1)} ± ${std.toFixed(1)}. Z-score de ${zScore.toFixed(1)}σ indica desvio estatisticamente significativo. Pode indicar erro de coleta ou evento ecológico real.`,
            });
          });
        }

        // (H) Espécie ameaçada em declínio: queda contínua em ≥3 campanhas consecutivas por UA
        const THREAT_SET = new Set(['CR','EX','EW','EN','VU','NT']);
        const MIN_DECLINE_STREAK = 2; // 2 quedas = 3 campanhas consecutivas decrescentes
        const uaSpCampCount = new Map<string, Map<string, number>>();
        regs.forEach(r => {
          const sp = (r.nomeCientifico || '').trim();
          const ua = (r.unidadeAmostral || '').trim();
          if (!sp || !ua || !r.campanha) return;
          if (!THREAT_SET.has(r.iucn || '') && !THREAT_SET.has(r.ibamaMma || '')) return;
          const key = `${ua}\x00${sp}`;
          if (!uaSpCampCount.has(key)) uaSpCampCount.set(key, new Map());
          const m = uaSpCampCount.get(key)!;
          m.set(r.campanha, (m.get(r.campanha) || 0) + 1);
        });
        uaSpCampCount.forEach((campMap, key) => {
          const [ua, sp] = key.split('\x00');
          const ordCounts = campanhasOrd.map(c => campMap.get(c) ?? 0);
          if (ordCounts.filter(v => v > 0).length < 3) return;
          let streak = 0;
          for (let i = ordCounts.length - 1; i >= 1; i--) {
            if (ordCounts[i] < ordCounts[i - 1]) streak++;
            else break;
          }
          if (streak < MIN_DECLINE_STREAK) return;
          const found = regs.find(r => r.nomeCientifico?.trim() === sp);
          alertas.push({
            tipo: 'ameacada_declinio',
            severidade: 'alta',
            empreendimentoId: empId,
            unidadeAmostral: ua,
            nomeCientifico: sp,
            nomeComum: found?.nomeComum || null,
            grupoTaxonomico: found?.grupoTaxonomico || null,
            iucn: found?.iucn || null,
            mma: found?.ibamaMma || null,
            campanhaAtual: lastCampName,
            declineStreak: streak + 1,
            contagemAtual: ordCounts[ordCounts.length - 1],
            contagemPico: Math.max(...ordCounts),
            titulo: `Declínio contínuo de ${sp} (ameaçada) na UA "${ua}"`,
            detalhe: `${streak + 1} campanhas consecutivas com queda de abundância${found?.iucn ? ` (IUCN: ${found.iucn})` : ''}${found?.ibamaMma ? ` (MMA: ${found.ibamaMma})` : ''}. Contagem atual: ${ordCounts[ordCounts.length - 1]}. Máximo histórico: ${Math.max(...ordCounts)}.`,
          });
        });
      });

      // Ordena: ameacada_declinio > alta > media > info; depois por magnitude desc
      const sevOrder: Record<string, number> = { alta: 0, media: 1, info: 2 };
      const tipoOrder: Record<string, number> = {
        ameacada_declinio: 0, especie_ausente: 1, queda_riqueza: 2,
        dominancia_excessiva: 3, ua_ausente: 4, abundancia_anomala: 5,
        lacuna_temporal: 6, especie_nova: 7,
      };
      alertas.sort((a, b) => {
        const sa = sevOrder[a.severidade] ?? 9;
        const sb = sevOrder[b.severidade] ?? 9;
        if (sa !== sb) return sa - sb;
        const ta = tipoOrder[a.tipo] ?? 9;
        const tb = tipoOrder[b.tipo] ?? 9;
        if (ta !== tb) return ta - tb;
        const va = a.campanhasAusente ?? a.quedaPct ?? a.gapDias ?? a.zScore ?? a.declineStreak ?? 0;
        const vb = b.campanhasAusente ?? b.quedaPct ?? b.gapDias ?? b.zScore ?? b.declineStreak ?? 0;
        return vb - va;
      });

      res.json({
        total: alertas.length,
        params: { minCampanhasHistorico: MIN_CAMP_HIST, minCampanhasAusente: MIN_ABSENT_CAMP, dropPct: DROP_PCT, dropPctAlto: DROP_PCT_HIGH },
        contagens: {
          ...
          especieAusente: alertas.filter(a => a.tipo === 'especie_ausente').length,
          quedaRiqueza: alertas.filter(a => a.tipo === 'queda_riqueza').length,
          especieNova: alertas.filter(a => a.tipo === 'especie_nova').length,
          dominanciaExcessiva: alertas.filter(a => a.tipo === 'dominancia_excessiva').length,
          uaAusente: alertas.filter(a => a.tipo === 'ua_ausente').length,
          lacunaTemporal: alertas.filter(a => a.tipo === 'lacuna_temporal').length,
          abundanciaAnomala: alertas.filter(a => a.tipo === 'abundancia_anomala').length,
          ameacadaDeclinio: alertas.filter(a => a.tipo === 'ameacada_declinio').length,
        },
        alertas,
      });
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  ...
  app.post('/api/campo/:id/fotos', requireAuth, campoFotoUpload.single('foto'), async (req, res) => {
    try {
      ...
      if (!Number.isFinite(registroId)) return res.status(400).json({ message: 'ID inválido' });
      if (!req.file) return res.status(400).json({ message: "Nenhum arquivo enviado" });
      // F6: cap server-side de 8 fotos por registro — atômico via transação com lock no registro
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ message: 'Unauthorized' });

      // Pre-check rápido sem custo de upload em casos óbvios (não substitui o lock atômico abaixo)
      const [preReg] = await db.select({ id: campoRegistros.id, unidade: campoRegistros.unidade })
        .from(campoRegistros).where(eq(campoRegistros.id, registroId));
      if (!preReg) return res.status(404).json({ message: 'Registro não encontrado' });
      if (preReg.unidade && user.unidade && preReg.unidade !== user.unidade) {
        return res.status(403).json({ message: 'Sem acesso a este registro' });
      }

      // Faz upload primeiro (operação externa que não deve ficar dentro da transação)
      const { uploadDocumentToGoogleDrive } = await import('./services/googleDriveService');
      const safeName = req.file.originalname.replace(/[^a-zA-Z0-9._-]/g, '_');
      const fileName = `campo_${registroId}_${Date.now()}_${safeName}`;
      const result = await uploadDocumentToGoogleDrive(fileName, req.file.buffer, req.file.mimetype);
      if (!result.success || !result.url) return res.status(503).json({ message: 'Falha ao salvar foto, tente novamente.' });

      // F6: count + insert atômicos com lock no registro pai (serializa uploads concorrentes)
      const txResult = await db.transaction(async (tx) => {
        const lockedRows = await tx.execute(sql`SELECT id, unidade FROM campo_registros WHERE id = ${registroId} FOR UPDATE`);
        const locked: any = (lockedRows as any).rows?.[0] || (Array.isArray(lockedRows) ? (lockedRows as any)[0] : null);
        if (!locked) return { ok: false as const, code: 404, msg: 'Registro não encontrado' };
        if (locked.unidade && user.unidade && locked.unidade !== user.unidade) {
          return { ok: false as const, code: 403, msg: 'Sem acesso a este registro' };
        }
        const [{ total }] = await tx.select({ total: sql<number>`count(*)::int` }).from(campoFotos).where(eq(campoFotos.registroId, registroId));
        if (total >= 8) return { ok: false as const, code: 400, msg: 'Limite de 8 fotos por registro atingido' };
        const [foto] = await tx.insert(campoFotos).values({
          registroId,
          url: result.url!,
      ...
      if (!txResult.ok) return res.status(txResult.code).json({ message: txResult.msg });
      res.json(txResult.foto);
    } catch (err: any) { res.status(500).json({ message: err.message }); }
  });

  // GET serve photo (authenticated proxy — works for both stored keys and legacy full URLs)
  app.get('/api/campo/fotos/:id/view', requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const [foto] = await db.select().from(campoFotos).where(eq(campoFotos.id, id));
      if (!foto) return res.status(404).json({ message: "Foto não encontrada" });
      const url = foto.url || '';
      let bytes: Buffer | null = null;
      if (url.startsWith('gdrive://')) {
        const { downloadDocumentFromGoogleDrive } = await import('./services/googleDriveService');
        bytes = await downloadDocumentFromGoogleDrive(url.slice('gdrive://'.length));
      } else {
        // Legacy: Object Storage key or full URL
        let key = url;
    ...
    try {
      ...
      if (foto) {
        try {
          ...
          let key = foto.url;
          if (key.startsWith('https://')) key = key.split('/').slice(4).join('/');
  ...
  // GET conformidade — condicionantes vinculadas a licenças do empreendimento
  app.get('/api/campo/conformidade', requireAuth, async (req, res) => {
    try {
      const { empreendimentoId } = req.query;
      if (!empreendimentoId) return res.json([]);
      const licencas = await db.select({ id: licencasAmbientais.id })
        .from(licencasAmbientais)
        .where(eq(licencasAmbientais.empreendimentoId, parseInt(empreendimentoId as string)));
      if (!licencas.length) return res.json([]);
      const licIds = licencas.map(l => l.id);
      const conds = await db.select().from(condicionantes)
        .where(inArray(condicionantes.licencaId, licIds))
  ...
  app.post('/api/__temp_delete_all_users', async (req, res) => {
    try {
      // Nulificar / deletar dependentes antes de remover users
      const steps = [
        // Tabelas que podem ter NULLs nas FKs
        `UPDATE ai_conversations SET user_id = NULL WHERE user_id IS NOT NULL`,
        `UPDATE ai_logs SET user_id = NULL WHERE user_id IS NOT NULL`,
        `UPDATE amostras SET coletor_id = NULL WHERE coletor_id IS NOT NULL`,
        `UPDATE amostras SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE arquivos SET uploader_id = NULL WHERE uploader_id IS NOT NULL`,
        `UPDATE audit_logs SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE base_conhecimento SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE blog_artigos SET autor_id = NULL WHERE autor_id IS NOT NULL`,
        `UPDATE camadas_geoespaciais SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE comentarios_demandas SET autor_id = NULL WHERE autor_id IS NOT NULL`,
        `UPDATE comunicado_comentarios SET autor_id = NULL WHERE autor_id IS NOT NULL`,
        `UPDATE comunicado_curtidas SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE comunicado_enquete_votos SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE comunicado_eventos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE comunicado_leitura_obrigatoria SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE comunicado_mencoes SET usuario_mencionado_id = NULL WHERE usuario_mencionado_id IS NOT NULL`,
        `UPDATE comunicado_reacoes SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE comunicado_templates SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE comunicado_visualizacoes SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE comunicados SET autor_id = NULL WHERE autor_id IS NOT NULL`,
        `UPDATE condicionantes SET responsavel_id = NULL WHERE responsavel_id IS NOT NULL`,
        `UPDATE contrato_documentos SET uploader_id = NULL WHERE uploader_id IS NOT NULL`,
        `UPDATE dds_registros SET responsavel_id = NULL WHERE responsavel_id IS NOT NULL`,
        `UPDATE demandas SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE demandas SET responsavel_id = NULL WHERE responsavel_id IS NOT NULL`,
        `UPDATE documentos SET uploaded_by = NULL WHERE uploaded_by IS NOT NULL`,
        `UPDATE empreendimentos SET coordenador_id = NULL WHERE coordenador_id IS NOT NULL`,
        `UPDATE empreendimentos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE equipamentos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE financeiro_lancamentos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE fornecedores SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE historico_reembolso SET usuario_id = NULL WHERE usuario_id IS NOT NULL`,
        `UPDATE links_uteis SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE metas_custo_projeto SET coordenador_id = NULL WHERE coordenador_id IS NOT NULL`,
        `UPDATE murais SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE newsletter_edicoes SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE orcamentos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE pedidos_reembolso SET coordenador_id = NULL WHERE coordenador_id IS NOT NULL`,
        `UPDATE pedidos_reembolso SET diretor_id = NULL WHERE diretor_id IS NOT NULL`,
        `UPDATE pedidos_reembolso SET financeiro_id = NULL WHERE financeiro_id IS NOT NULL`,
        `UPDATE pedidos_reembolso SET solicitante_id = NULL WHERE solicitante_id IS NOT NULL`,
        `UPDATE processos_monitorados SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE projetos SET coordenador_id = NULL WHERE coordenador_id IS NOT NULL`,
        `UPDATE propostas_comerciais SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE ramais_contatos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE registro_horas SET aprovado_por = NULL WHERE aprovado_por IS NOT NULL`,
        `UPDATE registro_horas SET colaborador_id = NULL WHERE colaborador_id IS NOT NULL`,
        `UPDATE scheduled_reports SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE solicitacoes_recursos SET diretor_id = NULL WHERE diretor_id IS NOT NULL`,
        `UPDATE solicitacoes_recursos SET solicitante_id = NULL WHERE solicitante_id IS NOT NULL`,
        `UPDATE tarefas SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE tarefas SET responsavel_id = NULL WHERE responsavel_id IS NOT NULL`,
        `UPDATE treinamentos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE veiculos SET criado_por = NULL WHERE criado_por IS NOT NULL`,
        `UPDATE whatsapp_alert_configs SET user_id = NULL WHERE user_id IS NOT NULL`,
        `UPDATE whatsapp_contact_groups SET created_by = NULL WHERE created_by IS NOT NULL`,
        // Tabelas que referenciam users com delete direto (registros de junção)
        `DELETE FROM historico_demandas_movimentacoes WHERE usuario_id IS NOT NULL`,
        `DELETE FROM historicos_pontuacao WHERE usuario_id IS NOT NULL`,
        `DELETE FROM membros_equipe WHERE user_id IS NOT NULL OR coordenador_id IS NOT NULL`,
        `DELETE FROM offline_sync_queue WHERE usuario_id IS NOT NULL`,
        `DELETE FROM pontuacoes_gamificacao WHERE usuario_id IS NOT NULL`,
        `DELETE FROM realtime_notifications WHERE usuario_id IS NOT NULL`,
        `DELETE FROM tarefa_atualizacoes WHERE usuario_id IS NOT NULL`,
        `DELETE FROM treinamento_participantes WHERE usuario_id IS NOT NULL`,
        `DELETE FROM usuario_conquistas WHERE usuario_id IS NOT NULL`,
        `DELETE FROM whatsapp_contact_group_members WHERE user_id IS NOT NULL`,
        `DELETE FROM whatsapp_message_logs WHERE user_id IS NOT NULL`,
      ];
      ...
      for (const step of steps) {
        try {
          ...
          errors.push(`${step.slice(0, 60)}: ${e.message}`);
        }
      }

      const result = await db.execute(sql`DELETE FROM users`);
      res.json({ ok: true, deleted: (result as any).rowCount ?? 'ok', warnings: errors });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  // ── Campo: análise IA de gráficos/tabelas ─────────────────────────────────
  app.post("/api/campo/ai-analyze", async (req, res) => {
    try {
      const { tipo, contexto } = req.body as { tipo: string; contexto: string };
      if (!tipo || !contexto) return res.status(400).json({ error: "Parâmetros inválidos" });

      const systemPrompt = `Você é um ecólogo especialista em levantamento de fauna e flora, com amplo conhecimento em análise de biodiversidade, ecologia de comunidades e monitoramento ambiental. Responda SEMPRE em português do Brasil, com linguagem técnico-científica, mas clara e objetiva. Estruture sua resposta em dois blocos bem definidos:

1. **Resultados** — descreva tecnicamente o que os dados mostram, com referência aos valores, padrões e tendências observados.
2. **Discussão** — interprete os resultados no contexto ecológico, discuta implicações para a conservação ou gestão e mencione limitações metodológicas relevantes.

Seja direto e objetivo. Não use frases introdutórias genéricas. Máximo 350 palavras.`;

      const userPrompt = `Analise os seguintes dados de monitoramento ambiental para o componente "${tipo}":\n\n${contexto}`;
      const fullPrompt = systemPrompt + "\n\n" + userPrompt;

      const errors: string[] = [];

      // 0) Anthropic Claude via Replit AI Integrations (prioridade máxima — cota gerenciada)
      if (process.env.AI_INTEGRATIONS_ANTHROPIC_API_KEY) {
        try {
          ...
          if (text) {
            console.log("[AI Analyze] Anthropic claude-haiku-4-5 OK");
            return res.json({ analise: text, model: "claude-haiku-4-5 (Anthropic)" });
          }
        } catch (e: any) {
          errors.push(`Anthropic: ${e?.message?.slice(0, 120)}`);
          console.warn("[AI Analyze] Anthropic falhou:", e?.message?.slice(0, 120));
        }
      }

      // 1) Gemini via REST direto — inclui variantes lite/exp com cotas separadas
        ...
        for (const modelName of geminiModels) {
          try {
            ...
            if (gRes.ok) {
              ...
              if (text?.trim()) {
                console.log(`[AI Analyze] ${modelName} OK`);
                return res.json({ analise: text, model: `${modelName} (Gemini)` });
              }
            } else {
              const errBody = await gRes.text();
              errors.push(`Gemini ${modelName}: HTTP ${gRes.status} ${errBody.slice(0, 200)}`);
              console.warn(`[AI Analyze] Gemini ${modelName} falhou: HTTP ${gRes.status}`, errBody.slice(0, 200));
              // se for 404, tenta próximo modelo; se for 429, também tenta próximo
              continue;
            }
          } catch (e: any) {
            errors.push(`Gemini ${modelName}: ${e?.message}`);
            console.warn(`[AI Analyze] Gemini ${modelName} erro:`, e?.message);
          }
        }
      }

      // 2) Fallback: OpenAI via fetch direto (bypassa proxy do Replit)
      const openAiKey = process.env.OPENAI_DIRECT_KEY || process.env.OPENAI_API_KEY;
      if (openAiKey) {
        try {
          const oaiRes = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          if (oaiRes.ok) {
            ...
            if (text) { console.log("[AI Analyze] OpenAI OK"); return res.json({ analise: text, model: "gpt-4o-mini (OpenAI)" }); }
          } else {
            const errBody = await oaiRes.text();
            errors.push(`OpenAI HTTP ${oaiRes.status}: ${errBody.slice(0, 120)}`);
            console.warn("[AI Analyze] OpenAI falhou:", oaiRes.status, errBody.slice(0, 120));
          }
        } catch (e: any) {
          errors.push(`OpenAI fetch: ${e.message}`);
        }
      }

      res.status(500).json({ error: `Serviço de IA temporariamente indisponível. Tente novamente em alguns instantes. (${errors.slice(0,2).map(e=>e.split(":")[0]).join(", ")})` });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  // ── Smart Universal Spreadsheet Import ──────────────────────────────────────
  app.post('/api/planilha/detectar', requireAuth, async (req: any, res) => {
    try {
      const { headers, sampleRows, fileName } = req.body;
      if (!headers?.length) return res.status(400).json({ error: 'Nenhum cabeçalho encontrado' });

      const MODULES_DESC = `Módulos disponíveis no sistema SGAI (Sistema de Gestão Ambiental):
1. financeiro: Lançamentos financeiros. Campos: tipo (receita/despesa/reembolso), valor, data, categoria, descrição, empreendimento
2. rh: Recursos Humanos / colaboradores. Campos: nomeColaborador, cargo, cpf, status (ativo/inativo), dataInicio, valor/salario, contatoEmail, contatoTelefone, regimeContratacao (CLT/PJ), cnpj
3. empreendimentos: Empreendimentos / projetos ambientais. Campos: nome, cliente, tipo (hidreletrica/parque_eolico/usina_solar/termoeletrica/linha_transmissao/mina/pchs/outro), status (ativo/em_planejamento/em_execucao/concluido/inativo), municipio, uf, localizacao, responsavelInterno, descricao
4. fornecedores: Fornecedores e prestadores. Campos: razaoSocial, cnpj, cpf, tipo (laboratorio/transportadora/consultoria/equipamentos/servicos/materiais/outro), email, telefone, cidade, uf, avaliacao (1-5), status (ativo/inativo/bloqueado), servicosPrestados
5. treinamentos: Treinamentos e capacitações NR. Campos: titulo, tipo (nr/tecnico/obrigatorio/reciclagem/desenvolvimento), modalidade (presencial/online/hibrido), dataInicio, dataFim, cargaHoraria, status (agendado/em_andamento/concluido/cancelado), instituicao, instrutor, local, custoTotal
6. veiculos: Frota de veículos. Campos: placa, marca, modelo, ano, tipo (carro/caminhonete/caminhao/van/moto), status (disponivel/em_uso/manutencao/indisponivel), kmAtual, combustivel (gasolina/etanol/diesel/flex), localizacaoAtual, responsavelAtual
7. equipamentos: Equipamentos de campo e laboratório. Campos: nome, tipo (GPS/drone/armadilha/estacao_meteorologica/camera_trap/outro), status (disponivel/em_uso/manutencao), localizacaoAtual, responsavel, marca, modelo, numeroSerie, numeroPatrimonio, valorAquisicao, dataAquisicao
8. demandas: Demandas / tarefas / pendências. Campos: titulo, descricao, setor, status (aberta/em_andamento/concluida/cancelada), prioridade (baixa/media/alta/urgente), prazo, responsavel
9. campo: Registros biológicos de campo / monitoramento. Campos: nomeCientifico, nomeComum, grupoTaxonomico, campanha, data, latitude, longitude, abundancia, unidadeAmostral, iucn, cites, ibamaMma, pan, metodo, observacoes`;

      const sampleStr = [headers.join(' | ')].concat(
        ((sampleRows || []) as any[][]).slice(0, 3).map((r: any[]) =>
          r.map(v => (v === null || v === undefined ? '' : String(v))).join(' | ')
        )
      ).join('\n');

      const prompt = `Você é especialista em dados ambientais e gestão de projetos. Analise esta planilha e identifique o módulo do sistema SGAI ao qual ela pertence.

Nome do arquivo: ${fileName || 'planilha.xlsx'}

Cabeçalhos e exemplos de dados:
${sampleStr}

${MODULES_DESC}

Responda APENAS com JSON válido (sem markdown, sem explicações externas):
{
  "modulo": "<um dos: financeiro|rh|empreendimentos|fornecedores|treinamentos|veiculos|equipamentos|demandas|campo>",
  "confianca": <número de 0.0 a 1.0>,
  "descricao": "<resumo do que foi identificado, ex: Planilha de lançamentos financeiros com receitas e despesas>",
  "mapeamento": [
    {"coluna": "<cabeçalho exato>", "campo": "<campo do módulo ou null>", "label": "<nome amigável em pt-BR>", "obrigatorio": true|false}
  ]
}

Inclua TODOS os cabeçalhos no mapeamento. Para campos sem correspondência use "campo": null.`;
      ...
      if (process.env.GEMINI_API_KEY) {
        try {
          const { GoogleGenerativeAI } = await import('@google/generative-ai');
          const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
          for (const modelName of ['gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-2.5-flash-preview-04-17']) {
            try {
              const m = genAI.getGenerativeModel({ model: modelName, generationConfig: { temperature: 0.1, maxOutputTokens: 2048 } });
              const result = await m.generateContent(prompt);
              responseText = result.response.text();
              if (responseText) break;
            } catch {}
          }
        } catch (e: any) { console.warn('[SmartImport] Gemini falhou:', e.message); }
      }

      // Try DeepSeek (OpenAI-compatible, usually has quota)
      if (!responseText && process.env.DEEPSEEK_API_KEY) {
        try {
          ...
          if (dsRes.ok) {
            const j = await dsRes.json() as any;
            responseText = j.choices?.[0]?.message?.content || '';
          }
        } catch (e: any) { console.warn('[SmartImport] DeepSeek falhou:', e.message); }
      }

      // Try OpenAI as last AI option
      if (!responseText) {
        const key = process.env.OPENAI_DIRECT_KEY || process.env.OPENAI_API_KEY;
        if (key) {
          try {
            const oaiRes = await fetch('https://api.openai.com/v1/chat/completions', {
              method: 'POST',
            ...
            if (oaiRes.ok) {
              const j = await oaiRes.json() as any;
              responseText = j.choices?.[0]?.message?.content || '';
            }
          } catch {}
        }
      ...
      // ── Heuristic fallback — always works without any AI ─────────────────
      if (!responseText) {
        const hdr = headers.map((h: string) => h.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''));
        const score = (kws: string[]) => kws.filter(k => hdr.some((h: string) => h.includes(k))).length;
        const scores: Record<string,number> = {
          financeiro:      score(['valor','lancamento','receita','despesa','categoria','tipo','data']),
          rh:              score(['colaborador','funcionario','cargo','cpf','salario','admissao','contratacao','regime','rg']),
          empreendimentos: score(['empreendimento','cliente','municipio','localizacao','bioma','potencia','estado']),
          fornecedores:    score(['fornecedor','razao','cnpj','servico','avaliacao','laboratorio','contrato']),
          treinamentos:    score(['treinamento','nr','carga','horaria','modalidade','instrutor','certificado','instituicao']),
          veiculos:        score(['placa','veiculo','marca','modelo','km','combustivel','ano','frota']),
          equipamentos:    score(['equipamento','serie','patrimonio','serie','sensor','drone','gps','armadilha','camera']),
          demandas:        score(['demanda','tarefa','prioridade','prazo','setor','status','responsavel','titulo']),
          campo:           score(['especie','taxonom','cientifico','iucn','cites','latitude','longitude','campanha','fauna','flora','abundancia']),
        };
        const best = Object.entries(scores).sort((a,b) => b[1]-a[1])[0];
        const modulo = best[1] > 0 ? best[0] : 'demandas';
        const confianca = Math.min(0.95, best[1] * 0.15);

        // Build a basic mapping from header -> closest field
        const FIELD_KWS: Record<string, string[][]> = {
          financeiro:      [['tipo',['tipo','lancamento']],['valor',['valor','montante','quantia']],['data',['data','dt']],['categoria',['categoria','cat']],['descricao',['descricao','obs','historico']],['empreendimento',['empreendimento','projeto','emp']]],
          rh:              [['nomeColaborador',['nome','colaborador','funcionario']],['cargo',['cargo','funcao','ocupacao']],['cpf',['cpf']],['status',['status','situacao']],['dataInicio',['admissao','inicio','contratacao','data']],['valor',['salario','valor','remuneracao']],['contatoEmail',['email','e-mail']],['contatoTelefone',['telefone','celular','fone']]],
          empreendimentos: [['nome',['nome','empreendimento']],['cliente',['cliente','contratante']],['tipo',['tipo']],['status',['status','situacao']],['municipio',['municipio','cidade']],['uf',['uf','estado','uf']]],
          fornecedores:    [['razaoSocial',['razao','nome','fornecedor']],['cnpj',['cnpj']],['tipo',['tipo','categoria']],['email',['email','e-mail']],['telefone',['telefone','fone']],['avaliacao',['avaliacao','nota','estrela']],['status',['status']]],
          treinamentos:    [['titulo',['titulo','treinamento','nome']],['tipo',['tipo','nr','categoria']],['modalidade',['modalidade','formato']],['dataInicio',['inicio','data','dt']],['dataFim',['fim','termino','conclusao']],['cargaHoraria',['carga','hora']],['instituicao',['instituicao','empresa','org']]],
          veiculos:        [['placa',['placa']],['marca',['marca']],['modelo',['modelo']],['ano',['ano']],['tipo',['tipo','categoria']],['status',['status']],['kmAtual',['km','quilometragem','odometro']],['combustivel',['combustivel','abastecimento']]],
          equipamentos:    [['nome',['nome','equipamento','descricao']],['tipo',['tipo','categoria']],['status',['status','situacao']],['localizacaoAtual',['local','localizacao']],['numeroSerie',['serie','numero','patrimonio']]],
          demandas:        [['titulo',['titulo','demanda','tarefa']],['descricao',['descricao','obs']],['setor',['setor','area']],['status',['status']],['prioridade',['prioridade']],['prazo',['prazo','vencimento','data']]],
          campo:           [['nomeCientifico',['cientifico','especie']],['nomeComum',['comum','popular']],['grupoTaxonomico',['grupo','taxonom','fauna','flora']],['campanha',['campanha']],['data',['data','dt']],['latitude',['lat']],['longitude',['lon']],['abundancia',['abundancia','qtd','quantidade']],['unidadeAmostral',['ua','unidade','amostral','ponto','estacao']],['localizacao',['localizacao','jusante','montante','trecho','posicao']],['metodo',['metodo','tecnica','armadilha']],['esforcoAmostral',['esforco','effort','horas']],['unidadeEsforco',['unidade.esforco','rede.hora','anzol']]],
        };
        const mapeamento = headers.map((coluna: string) => {
          const col = coluna.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
          const fieldList = FIELD_KWS[modulo] || [];
          let bestField: string | null = null;
          let bestScore = 0;
          for (const [field, kws] of fieldList) {
            const s = (kws as string[]).filter(k => col.includes(k)).length;
            if (s > bestScore) { bestScore = s; bestField = field; }
          }
          return { coluna, campo: bestField, label: coluna, obrigatorio: false };
        });

        return res.json({ modulo, confianca, descricao: `Detectado via análise heurística: ${modulo}`, mapeamento });
      }
  ...
  app.post('/api/planilha/importar', requireAuth, async (req: any, res) => {
    try {
      const { modulo: moduloRaw, registros, unidade: unidadeReq } = req.body;
      const modulo = (moduloRaw || '').toLowerCase().trim();
      const userId = req.session?.userId || 1;
      const unidade = unidadeReq || req.session?.unidade || 'salvador';

      if (!modulo || !Array.isArray(registros) || registros.length === 0)
        return res.status(400).json({ error: 'Módulo e registros são obrigatórios' });
      ...
      if (modulo === 'financeiro') {
        const categorias = await db.select({ id: categoriasFinanceiras.id, nome: categoriasFinanceiras.nome }).from(categoriasFinanceiras);
        const emps = await db.select({ id: empreendimentos.id, nome: empreendimentos.nome }).from(empreendimentos);
        const catMap = new Map(categorias.map(c => [c.nome.toLowerCase(), c.id]));
        const empMap = new Map(emps.map(e => [e.nome.toLowerCase(), e.id]));

        for (let i = 0; i < registros.length; i++) {
          const r = registros[i];
          try {
            const tipoRaw = str(r.tipo).toLowerCase();
            const tipoValido = ['receita','despesa','reembolso','solicitacao_recurso'].includes(tipoRaw);
            if (!tipoValido) { pulados.push(`Linha ${i+2}: tipo inválido "${tipoRaw}"`); continue; }
            const valor = parseNum(r.valor);
            if (!valor) { pulados.push(`Linha ${i+2}: valor inválido`); continue; }
            const dataLanc = parseDate(r.data);
            if (!dataLanc) { pulados.push(`Linha ${i+2}: data inválida`); continue; }
            const catName = str(r.categoria).toLowerCase();
            let categoriaId = catMap.get(catName);
            if (!categoriaId) for (const [k,v] of catMap) { if (k.includes(catName) || catName.includes(k)) { categoriaId = v; break; } }
            if (!categoriaId) { pulados.push(`Linha ${i+2}: categoria não encontrada "${catName}"`); continue; }
            const empName = str(r.empreendimento).toLowerCase();
            let empreendimentoId = empMap.get(empName) || undefined;
            if (!empreendimentoId) for (const [k,v] of empMap) { if (k.includes(empName) || empName.includes(k)) { empreendimentoId = v; break; } }
            await db.insert(financeiroLancamentos).values({
              tipo: tipoRaw as any, categoriaId: categoriaId!, empreendimentoId,
              valor: String(valor), descricao: str(r.descricao), data: dataLanc,
              status: str(r.status) || 'aguardando', criadoPor: userId, unidade,
            });
            inseridos++;
          } catch (e: any) { erros.push(`Linha ${i+2}: ${e.message?.slice(0,80)}`); }
        }
        ...
        for (let i = 0; i < registros.length; i++) {
          const r = registros[i];
          try {
            const nome = str(r.nomeColaborador || r.nome);
            if (!nome) { pulados.push(`Linha ${i+2}: nome obrigatório`); continue; }
            await db.insert(rhRegistros).values({
              nomeColaborador: nome, cargo: str(r.cargo) || null, cpf: str(r.cpf) || null,
              status: str(r.status) || 'ativo', dataInicio: parseDate(r.dataInicio) || null,
              valor: r.valor ? String(parseNum(r.valor)) : null,
              valorTipo: str(r.valorTipo) || 'mes',
              contatoEmail: str(r.contatoEmail || r.email) || null,
              contatoTelefone: str(r.contatoTelefone || r.telefone) || null,
              regimeContratacao: str(r.regimeContratacao || r.regime) || null,
              cnpj: str(r.cnpj) || null, rg: str(r.rg) || null,
              unidade, criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
          } catch (e: any) { erros.push(`Linha ${i+2}: ${e.message?.slice(0,80)}`); }
        }

      } else if (modulo === 'empreendimentos') {
        for (let i = 0; i < registros.length; i++) {
          const r = registros[i];
          try {
            ...
            await db.insert(empreendimentos).values({
              nome, cliente: str(r.cliente) || 'N/A',
              localizacao: str(r.localizacao || r.municipio) || 'N/A',
              responsavelInterno: str(r.responsavelInterno || r.responsavel) || 'N/A',
              tipo: str(r.tipo) || 'outro', status: str(r.status) || 'ativo',
              municipio: str(r.municipio) || null, uf: str(r.uf) || null,
              descricao: str(r.descricao) || null,
              unidade, criadoPor: userId, criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
          } catch (e: any) { erros.push(`Linha ${i+2}: ${e.message?.slice(0,80)}`); }
        }
        ...
        for (let i = 0; i < registros.length; i++) {
          const r = registros[i];
          try {
            const razaoSocial = str(r.razaoSocial || r.nome);
            if (!razaoSocial) { pulados.push(`Linha ${i+2}: razão social obrigatória`); continue; }
            const aval = r.avaliacao ? Math.min(5, Math.max(1, parseInt(str(r.avaliacao)))) : null;
            await db.insert(fornecedores).values({
              razaoSocial, nomeFantasia: str(r.nomeFantasia) || null,
              cnpj: str(r.cnpj) || null, cpf: str(r.cpf) || null,
              tipo: str(r.tipo) || 'outro', email: str(r.email) || null,
              telefone: str(r.telefone) || null, cidade: str(r.cidade) || null,
              uf: str(r.uf) || null, avaliacao: aval,
              status: str(r.status) || 'ativo',
              servicosPrestados: str(r.servicosPrestados || r.servicos) || null,
              observacoes: str(r.observacoes) || null,
              unidade, criadoPor: userId, criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
          } catch (e: any) { erros.push(`Linha ${i+2}: ${e.message?.slice(0,80)}`); }
        ...
        for (let i = 0; i < registros.length; i++) {
          ...
          try {
            ...
            if (!titulo) { pulados.push(`Linha ${i+2}: título obrigatório`); continue; }
            const dataInicio = parseDate(r.dataInicio || r.data);
            if (!dataInicio) { pulados.push(`Linha ${i+2}: data de início obrigatória`); continue; }
            await db.insert(treinamentos).values({
              titulo, tipo: str(r.tipo) || 'tecnico',
              modalidade: str(r.modalidade) || 'presencial',
              dataInicio, dataFim: parseDate(r.dataFim) || null,
              cargaHoraria: r.cargaHoraria ? parseInt(str(r.cargaHoraria)) : null,
              status: str(r.status) || 'agendado',
              instituicao: str(r.instituicao) || null, instrutor: str(r.instrutor) || null,
              local: str(r.local) || null,
              custoTotal: r.custoTotal ? String(parseNum(r.custoTotal)) : null,
              descricao: str(r.descricao) || null,
              unidade, criadoPor: userId, criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
        ...
        for (let i = 0; i < registros.length; i++) {
          const r = registros[i];
          try {
            const placa = str(r.placa).toUpperCase().replace(/\s/g, '');
            if (!placa) { pulados.push(`Linha ${i+2}: placa obrigatória`); continue; }
            const existing = await db.select({ id: veiculos.id }).from(veiculos).where(eq(veiculos.placa, placa)).limit(1);
            if (existing.length) { pulados.push(`Linha ${i+2}: placa ${placa} já cadastrada`); continue; }
            await db.insert(veiculos).values({
              placa, marca: str(r.marca) || 'N/A', modelo: str(r.modelo) || 'N/A',
              ano: parseInt(str(r.ano)) || new Date().getFullYear(),
              tipo: str(r.tipo) || 'carro',
              status: str(r.status) || 'disponivel',
              kmAtual: parseInt(str(r.kmAtual || r.quilometragem)) || 0,
              combustivel: str(r.combustivel) || 'flex',
              seguro: str(r.seguro) || 'N/A',
              proximaRevisao: parseDate(r.proximaRevisao || r.revisao) || new Date().toISOString().slice(0,10),
              localizacaoAtual: str(r.localizacaoAtual || r.localizacao) || 'N/A',
              responsavelAtual: str(r.responsavelAtual || r.responsavel) || null,
              observacoes: str(r.observacoes) || null,
              unidade, criadoPor: userId, criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
        ...
        for (let i = 0; i < registros.length; i++) {
          ...
          try {
            ...
            await db.insert(equipamentos).values({
              nome, tipo: str(r.tipo) || 'outro',
              localizacaoAtual: str(r.localizacaoAtual || r.localizacao) || 'N/A',
              status: str(r.status) || 'disponivel',
              responsavel: str(r.responsavel) || null,
              marca: str(r.marca) || null, modelo: str(r.modelo) || null,
              numeroSerie: str(r.numeroSerie || r.serie) || null,
              numeroPatrimonio: str(r.numeroPatrimonio || r.patrimonio) || null,
              valorAquisicao: r.valorAquisicao ? String(parseNum(r.valorAquisicao)) : null,
              dataAquisicao: parseDate(r.dataAquisicao) || null,
              observacoes: str(r.observacoes) || null,
              unidade, criadoPor: userId, criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
        ...
        for (let i = 0; i < registros.length; i++) {
          ...
          try {
            ...
            await db.insert(demandas).values({
              titulo, descricao: str(r.descricao) || null,
              setor: str(r.setor) || 'Geral',
              status: str(r.status) || 'aberta',
              prioridade: str(r.prioridade) || 'media',
              prazo: parseDate(r.prazo) || null,
              responsavel: str(r.responsavel) || null,
              criadoPor: userId, unidade,
        ...
        for (let i = 0; i < registros.length; i++) {
          const r = registros[i];
          try {
            const enriched = applySpeciesLookup({
              nomeCientifico: str(r.nomeCientifico || r.especie),
              nomeComum: str(r.nomeComum || r.nomePopular),
              grupoTaxonomico: str(r.grupoTaxonomico || r.grupo),
              campanha: str(r.campanha), data: parseDate(r.data),
              latitude: parseNum(r.latitude), longitude: parseNum(r.longitude),
              abundancia: parseInt(str(r.abundancia || r.quantidade)) || 1,
              unidadeAmostral: str(r.unidadeAmostral || r.ua),
              localizacao: str(r.localizacao || r.trecho || r.posicao),
              metodo: str(r.metodo),
              esforcoAmostral: parseFloat(str(r.esforcoAmostral || r.esforco)) || null,
              unidadeEsforco: str(r.unidadeEsforco),
              iucn: str(r.iucn), cites: str(r.cites), ibamaMma: str(r.ibamaMma),
              observacoes: str(r.observacoes),
            });
            await db.insert(campoRegistros).values({
              ...enriched, criadoPor: userId, unidade,
              criadoEm: new Date(), atualizadoEm: new Date(),
            });
            inseridos++;
          } catch (e: any) { erros.push(`Linha ${i+2}: ${e.message?.slice(0,80)}`); }
        ...
        return res.status(400).json({ error: `Módulo "${modulo}" não suportado para importação` });
      }

      return res.json({ inseridos, erros: erros.slice(0, 20), pulados: pulados.slice(0, 20), total: registros.length });
    } catch (err: any) {
      return res.status(500).json({ error: err.message });
    }
  });
  ...
  // EMAIL AUTOMAÇÃO
  // ══════════════════════════════════════════════════════════════════════════
  const { sincronizarConta, gerarRelatorioExecutivo, isContaBloqueada } = await import("./services/emailAutomacaoService");
  const { emailContas: emailContasT, emailsCapturados: emailsCapturadosT } = await import("../shared/schema");
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS emails_capturados (
      id SERIAL PRIMARY KEY,
      conta_id INTEGER,
      message_id TEXT,
      ...
      programa_ambiental TEXT,
      condicionante TEXT,
      numero_processo TEXT,
      responsavel_sugerido TEXT,
      responsavel_externo TEXT,
      prazo_identificado TEXT,
      data_vencimento TIMESTAMP,
      anexos JSONB,
      evidencia_cumprimento BOOLEAN DEFAULT false,
      nivel_confianca REAL,
      precisa_validacao BOOLEAN DEFAULT false,
      vinculo_pendente BOOLEAN DEFAULT false,
      sugestoes_vinculo JSONB,
      observacoes_ia TEXT,
      demanda_criada_id INTEGER,
      demanda_atualizada BOOLEAN DEFAULT false,
  ...
  // GET listar contas de e-mail
  app.get('/api/email-automacao/contas', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const contas = await db.select({
        id: emailContasT.id, nome: emailContasT.nome, email: emailContasT.email,
        host: emailContasT.host, port: emailContasT.port, secure: emailContasT.secure,
        ativo: emailContasT.ativo, unidade: emailContasT.unidade,
        ultimaSincEm: emailContasT.ultimaSincEm, totalEmails: emailContasT.totalEmails,
        criadoEm: emailContasT.criadoEm,
      }).from(emailContasT).where(eq(emailContasT.unidade, user.unidade || 'geral'))
        .orderBy(desc(emailContasT.criadoEm));
      res.json(contas);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // POST cadastrar conta de e-mail
  app.post('/api/email-automacao/contas', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const { nome, email, host, port, secure, usuario, senha } = req.body;
      if (!nome || !email || !host || !usuario || !senha) return res.status(400).json({ error: 'Campos obrigatórios: nome, email, host, usuario, senha' });
      const [nova] = await db.insert(emailContasT).values({
        nome: String(nome).toLowerCase().trim(),
        email: String(email).toLowerCase().trim(),
        host: String(host).toLowerCase().trim(),
        port: port || 993, secure: secure !== false,
        usuario: String(usuario).toLowerCase().trim(),
        senha, unidade: user.unidade || 'geral', criadoPor: user.id,
      }).returning();
      res.json(nova);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  // PUT atualizar conta
  app.put('/api/email-automacao/contas/:id', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const id = parseInt(req.params.id);
      const { nome, email, host, port, secure, usuario, senha, ativo } = req.body;
      const update: any = {};
      if (nome !== undefined) update.nome = String(nome).toLowerCase().trim();
      if (email !== undefined) update.email = String(email).toLowerCase().trim();
      if (host !== undefined) update.host = String(host).toLowerCase().trim();
      if (port !== undefined) update.port = port;
      if (secure !== undefined) update.secure = secure;
      if (usuario !== undefined) update.usuario = String(usuario).toLowerCase().trim();
      if (senha !== undefined && senha) update.senha = senha;
      if (ativo !== undefined) update.ativo = ativo;
      const [updated] = await db.update(emailContasT).set(update)
        .where(and(eq(emailContasT.id, id), eq(emailContasT.unidade, user.unidade || 'geral'))).returning();
  ...
  app.delete('/api/email-automacao/contas/:id', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const id = parseInt(req.params.id);
      await db.delete(emailContasT).where(and(eq(emailContasT.id, id), eq(emailContasT.unidade, user.unidade || 'geral')));
      res.json({ success: true });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // POST testar credenciais diretas (sem precisar salvar a conta primeiro)
  app.post('/api/email-automacao/testar-credenciais', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const { host: rawHost, port: rawPort, secure: rawSecure, usuario: rawUsuario, senha: rawSenha } = req.body;
      const host = String(rawHost || '').trim();
      const port = Number(rawPort) || 993;
      const secure = rawSecure === true || rawSecure === 'true' || rawSecure === 1;
      const usuario = String(rawUsuario || '').trim();
      const senha = String(rawSenha || '');
      if (!host || !usuario || !senha) return res.status(400).json({ success: false, error: 'Preencha host, usuário e senha antes de testar.' });

      console.log(`[EmailAutomação TESTAR-CRED] ${usuario} → ${host}:${port} SSL=${secure}`);
      const rawLog: string[] = [];
      try {
        await new Promise<void>((resolve) => {
          import('tls').then((tlsMod) => {
            import('net').then((netMod) => {
              let settled = false;
              const done = (err?: string) => { if (settled) return; settled = true; if (err) rawLog.push(`ERRO_REDE: ${err}`); resolve(); };
              const timer = setTimeout(() => done('timeout 10s'), 10000);
              const onData = (data: Buffer) => { rawLog.push(`S: ${data.toString().trim().slice(0, 600)}`); if (rawLog.length >= 8) { clearTimeout(timer); done(); } };
              const onError = (e: Error) => { clearTimeout(timer); done(e.message); };
              if (secure) {
                const sock = tlsMod.connect({ host, port, rejectUnauthorized: false } as any, () => { rawLog.push(`TLS_OK cipher=${(sock as any).getCipher?.()?.name ?? '?'}`); sock.write('T1 CAPABILITY\r\n'); });
                sock.on('data', onData); sock.on('error', onError);
              } else {
                const sock = netMod.createConnection(port, host, () => { rawLog.push('TCP_OK'); sock.write('T1 CAPABILITY\r\n'); });
                sock.on('data', onData); sock.on('error', onError);
              }
            });
          }).catch((e: any) => { rawLog.push(`IMPORT_ERR: ${e.message}`); resolve(); });
        });
      } catch (probeErr: any) { rawLog.push(`PROBE_ERR: ${probeErr.message}`); }

      const { ImapFlow } = await import('imapflow');
      const tlsBypass = { rejectUnauthorized: false, checkServerIdentity: () => undefined };
      const imapOpts: any = { host, port, secure, auth: { user: usuario, pass: senha }, logger: false, connectionTimeout: 30000, greetingTimeout: 20000, socketTimeout: 60000, disableAutoIdle: true, tls: tlsBypass };
      if (!secure) imapOpts.tlsOptions = tlsBypass;
      const client = new (ImapFlow as any)(imapOpts);
      try {
        await client.connect();
        await client.logout();
        console.log(`[EmailAutomação TESTAR-CRED] OK: ${usuario}`);
        res.json({ success: true, message: `Conexão bem-sucedida com ${host}:${port}`, detalhe: rawLog.join('\n') });
      } catch (connErr: any) {
        const rawMsg = connErr?.message || String(connErr);
        const serverResp = connErr?.response ?? connErr?.serverResponse ?? connErr?.responseText ?? "";
        const fullMsg = serverResp ? `${rawMsg} | server: ${serverResp}` : rawMsg;
        console.error(`[EmailAutomação TESTAR-CRED] ERRO: ${fullMsg}`);
        res.status(400).json({ success: false, error: traduzirErroIMAP(fullMsg), rawError: fullMsg, detalhe: rawLog.join('\n') });
      }
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // POST testar conexão IMAP
  app.post('/api/email-automacao/contas/:id/testar', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const id = parseInt(req.params.id);
      const [conta] = await db.select().from(emailContasT).where(and(eq(emailContasT.id, id), eq(emailContasT.unidade, user.unidade || 'geral')));
      if (!conta) return res.status(404).json({ error: 'Conta não encontrada' });

      const host = String(conta.host || '').trim();
      const port = Number(conta.port) || 993;
      const secure = conta.secure === true || conta.secure === (1 as any);
      const usuario = String(conta.usuario || '').trim();
      const senha = String(conta.senha || '');

      console.log(`[EmailAutomação TESTAR] ${conta.email} → ${host}:${port} SSL=${secure} user=${usuario}`);

      // Diagnóstico raw — captura conversa IMAP sem tentar login
      const rawLog: string[] = [];
      try {
        await new Promise<void>((resolve) => {
          import('tls').then((tlsMod) => {
            import('net').then((netMod) => {
              ...
              const done = (err?: string) => {
                ...
                if (err) rawLog.push(`ERRO_REDE: ${err}`);
                resolve();
              };
              const timer = setTimeout(() => done('timeout 10s'), 10000);
              const onData = (data: Buffer) => {
                rawLog.push(`S: ${data.toString().trim().slice(0, 600)}`);
                if (rawLog.length >= 8) { clearTimeout(timer); done(); }
              };
              const onError = (e: Error) => { clearTimeout(timer); done(e.message); };
              if (secure) {
                const sock = tlsMod.connect({ host, port, rejectUnauthorized: false } as any, () => {
                  rawLog.push(`TLS_OK cipher=${(sock as any).getCipher?.()?.name ?? '?'}`);
                  sock.write('T1 CAPABILITY\r\n');
                });
                sock.on('data', onData);
                sock.on('error', onError);
              } else {
                const sock = netMod.createConnection(port, host, () => {
                  rawLog.push('TCP_OK');
                  sock.write('T1 CAPABILITY\r\n');
                });
                sock.on('data', onData);
        ...
        rawLog.push(`PROBE_ERR: ${probeErr.message}`);
      }
      console.log(`[EmailAutomação TESTAR] RAW:\n${rawLog.join('\n')}`);

      // Agora autentica com ImapFlow
      const { ImapFlow } = await import('imapflow');
      const tlsBypass2 = { rejectUnauthorized: false, checkServerIdentity: () => undefined };
      const imapOpts: any = {
        host, port, secure,
      ...
      if (!secure) imapOpts.tlsOptions = tlsBypass2;
      const client = new (ImapFlow as any)(imapOpts);
      try {
        await client.connect();
        await client.logout();
        ...
        const rawMsg = connErr?.message || String(connErr);
        const serverResp2 = connErr?.response ?? connErr?.serverResponse ?? connErr?.responseText ?? "";
        const fullMsg2 = serverResp2 ? `${rawMsg} | server: ${serverResp2}` : rawMsg;
        console.error(`[EmailAutomação TESTAR] ERRO: ${fullMsg2}`);
        res.status(400).json({ success: false, error: traduzirErroIMAP(fullMsg2), rawError: fullMsg2, detalhe: rawLog.join('\n') });
      }
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // POST sincronizar conta manualmente
  app.post('/api/email-automacao/contas/:id/sincronizar', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const id = parseInt(req.params.id);
      const result = await sincronizarConta(id, user.unidade || 'geral');
      res.json({ success: true, ...result });
    } catch (e: any) {
      console.error('[EmailAutomação] Erro na sincronização RAW:', e.message);
      // Retorna 200 com flag de erro para não travar o frontend
      res.json({ success: false, capturados: 0, erros: 1, erro: traduzirErroIMAP(e.message), rawError: e.message });
    }
  });

  // POST sincronizar todas as contas ativas da unidade
  app.post('/api/email-automacao/sincronizar-todas', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const contas = await db.select().from(emailContasT)
        .where(and(eq(emailContasT.unidade, user.unidade || 'geral'), eq(emailContasT.ativo, true)));
      let totalCapturados = 0, totalErros = 0;
      const errosContas: { email: string; erro: string; rawError?: string; bloqueada?: boolean }[] = [];
      for (const conta of contas) {
        if (isContaBloqueada(conta.id)) {
          console.warn(`[EmailAutomação] Pulando conta ${conta.email} — backoff ativo`);
          errosContas.push({ email: conta.email, erro: 'Bloqueio temporário ativo — aguarde alguns minutos para a conta ser desbloqueada automaticamente.', bloqueada: true });
          continue;
        }
        try {
          const r = await sincronizarConta(conta.id, user.unidade || 'geral');
          totalCapturados += r.capturados;
          totalErros += r.erros;
        } catch (e: any) {
          console.error(`[EmailAutomação] Erro conta ${conta.email} RAW:`, e.message);
          errosContas.push({ email: conta.email, erro: e.message, rawError: e.message });
        }
      }
      res.json({ success: errosContas.length === 0, capturados: totalCapturados, erros: totalErros, contas: contas.length, errosContas });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  function traduzirErroIMAP(msg: string): string {
    if (!msg) return 'Erro desconhecido';
    const m = msg.toLowerCase();
    // Bloqueio temporário / rate-limit — ANTES de checar credenciais
    if (m.includes('[unavailable]') || m.includes('temporary authentication failure') || m.includes('temporarily') || m.includes('too many') || m.includes('rate limit') || m.includes('login rate') || m.includes('try again')) return 'Bloqueio temporário no servidor — muitas tentativas em sequência. Aguarde 15–30 minutos e tente novamente. Considere acessar o Webmail para desbloquear a conta.';
    // Autenticação — verificar ANTES de SSL para não engolir erros de senha
    if (m.includes('invalid credentials') || m.includes('authentication failed') || m.includes('username and password') || m.includes('login failed') || m.includes('incorrect password') || m.includes('[authenticationfailed]') || (m.includes('auth') && m.includes('fail')) || m.includes('command failed') || m.includes('credenciais incorretas')) return 'Credenciais incorretas — verifique o usuário (e-mail completo) e a senha da conta IMAP.';
    // Rede / DNS
    if (m.includes('econnrefused') || m.includes('enotfound') || m.includes('getaddrinfo') || m.includes('enetunreach')) return 'Servidor IMAP não encontrado ou porta bloqueada. Verifique o host e a porta.';
    // SSL apenas para erros de certificado reais (não o mero string "tls")
    if (m.includes('self signed') || m.includes('unable to verify') || m.includes('cert') && (m.includes('expired') || m.includes('invalid') || m.includes('chain') || m.includes('authority')) || m.includes('ssl routines') || m.includes('ssl23')) return 'Erro de certificado SSL no servidor. Contacte o suporte do provedor.';
    // Timeout
    if (m.includes('timeout') || m.includes('etimedout') || m.includes('connection timeout')) return 'Tempo esgotado ao conectar. Servidor lento ou porta bloqueada.';
    // IMAP desativado
    if (m.includes('imap access') || m.includes('imap disabled')) return 'Acesso IMAP desativado na conta. Ative o IMAP nas configurações do provedor.';
    // Fallback: exibe a mensagem real (truncada) para diagnóstico
    return msg.slice(0, 300);
  }

  // GET listar e-mails capturados (com filtros)
  app.get('/api/email-automacao/emails', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const { categoria, prioridade, nivelRisco, validacao, vinculo, empreendimento, busca, limit: lim, offset: off } = req.query;
      const limite = Math.min(parseInt(lim as string) || 50, 200);
      const offset = parseInt(off as string) || 0;

      let query = db.select().from(emailsCapturadosT).where(eq(emailsCapturadosT.unidade, user.unidade || 'geral'));

      const emails = await db.select().from(emailsCapturadosT)
        .where(and(
          eq(emailsCapturadosT.unidade, user.unidade || 'geral'),
          categoria ? eq(emailsCapturadosT.categoriaPrincipal, categoria as string) : undefined,
          prioridade ? eq(emailsCapturadosT.prioridade, prioridade as string) : undefined,
          nivelRisco ? eq(emailsCapturadosT.nivelRisco, nivelRisco as string) : undefined,
          validacao === 'sim' ? eq(emailsCapturadosT.precisaValidacao, true) : undefined,
          vinculo === 'pendente' ? eq(emailsCapturadosT.vinculoPendente, true) : undefined,
          empreendimento ? eq(emailsCapturadosT.empreendimentoId, parseInt(empreendimento as string)) : undefined,
          busca ? sql`(${emailsCapturadosT.assunto} ILIKE ${'%' + busca + '%'} OR ${emailsCapturadosT.resumo} ILIKE ${'%' + busca + '%'} OR ${emailsCapturadosT.remetente} ILIKE ${'%' + busca + '%'})` : undefined,
        ))
        .orderBy(desc(emailsCapturadosT.criadoEm))
  ...
  // GET detalhe de um e-mail
  app.get('/api/email-automacao/emails/:id', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const id = parseInt(req.params.id);
      const [email] = await db.select().from(emailsCapturadosT)
        .where(and(eq(emailsCapturadosT.id, id), eq(emailsCapturadosT.unidade, user.unidade || 'geral')));
      if (!email) return res.status(404).json({ error: 'E-mail não encontrado' });
      res.json(email);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // PATCH validar/atualizar e-mail capturado
  app.patch('/api/email-automacao/emails/:id', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const id = parseInt(req.params.id);
      const { empreendimentoId, empreendimentoNome, statusDemanda, prioridade, responsavelSugerido, validacaoObservacoes, precisaValidacao, vinculoPendente } = req.body;
      const update: any = { atualizadoEm: new Date() };
      if (empreendimentoId !== undefined) update.empreendimentoId = empreendimentoId;
      if (empreendimentoNome !== undefined) update.empreendimentoNome = empreendimentoNome;
      if (statusDemanda !== undefined) update.statusDemanda = statusDemanda;
      if (prioridade !== undefined) update.prioridade = prioridade;
      if (responsavelSugerido !== undefined) update.responsavelSugerido = responsavelSugerido;
      if (validacaoObservacoes !== undefined) update.validacaoObservacoes = validacaoObservacoes;
      if (precisaValidacao !== undefined) update.precisaValidacao = precisaValidacao;
      if (vinculoPendente !== undefined) update.vinculoPendente = vinculoPendente;
      update.validadoPor = user.id;
      update.validadoEm = new Date();
      const [updated] = await db.update(emailsCapturadosT).set(update)
        .where(and(eq(emailsCapturadosT.id, id), eq(emailsCapturadosT.unidade, user.unidade || 'geral'))).returning();
      if (!updated) return res.status(404).json({ error: 'Não encontrado' });
      res.json(updated);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // GET relatório executivo
  app.get('/api/email-automacao/relatorio', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const relatorio = await gerarRelatorioExecutivo(user.unidade || 'geral');
      res.json(relatorio);
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // GET stats do dashboard
  app.get('/api/email-automacao/stats', requireAuth, async (req: any, res) => {
    try {
      const user = await storage.getUser(req.session.userId!);
      if (!user) return res.status(401).json({ error: 'Unauthorized' });
      const unidade = user.unidade || 'geral';
      const [total, validacao, vinculo, alta, risco, demandas_criadas] = await Promise.all([
        db.select({ c: sql<number>`count(*)` }).from(emailsCapturadosT).where(eq(emailsCapturadosT.unidade, unidade)),
        db.select({ c: sql<number>`count(*)` }).from(emailsCapturadosT).where(and(eq(emailsCapturadosT.unidade, unidade), eq(emailsCapturadosT.precisaValidacao, true))),
        db.select({ c: sql<number>`count(*)` }).from(emailsCapturadosT).where(and(eq(emailsCapturadosT.unidade, unidade), eq(emailsCapturadosT.vinculoPendente, true))),
        db.select({ c: sql<number>`count(*)` }).from(emailsCapturadosT).where(and(eq(emailsCapturadosT.unidade, unidade), eq(emailsCapturadosT.prioridade, 'alta'))),
        db.select({ c: sql<number>`count(*)` }).from(emailsCapturadosT).where(and(eq(emailsCapturadosT.unidade, unidade), eq(emailsCapturadosT.nivelRisco, 'alto'))),
        db.select({ c: sql<number>`count(*)` }).from(emailsCapturadosT).where(and(eq(emailsCapturadosT.unidade, unidade), sql`${emailsCapturadosT.demandaCriadaId} IS NOT NULL`)),
      ]);
      const porCategoria = await db.select({
        categoria: emailsCapturadosT.categoriaPrincipal,
        total: sql<number>`count(*)`,
      }).from(emailsCapturadosT).where(eq(emailsCapturadosT.unidade, unidade))
        .groupBy(emailsCapturadosT.categoriaPrincipal).orderBy(sql`count(*) desc`).limit(10);

      res.json({
        total: Number(total[0]?.c || 0),
        precisamValidacao: Number(validacao[0]?.c || 0),
        vinculoPendente: Number(vinculo[0]?.c || 0),
        altaPrioridade: Number(alta[0]?.c || 0),
        altoRisco: Number(risco[0]?.c || 0),
        demandasCriadas: Number(demandas_criadas[0]?.c || 0),
        porCategoria,
      });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  // Cron diário às 6h para sincronizar todas as contas ativas
  const emailCron = await import("node-cron");
  emailCron.schedule("0 6 * * *", async () => {
    console.log("[EmailAutomação] Iniciando sincronização diária...");
    try {
      const todasContas = await db.select().from(emailContasT).where(eq(emailContasT.ativo, true));
      for (const conta of todasContas) {
        if (isContaBloqueada(conta.id)) {
          console.warn(`[EmailAutomação] Cron: pulando conta ${conta.email} — backoff ativo`);
          continue;
        }
        try {
          const r = await sincronizarConta(conta.id, conta.unidade);
          console.log(`[EmailAutomação] Conta ${conta.email}: ${r.capturados} novos e-mails`);
        } catch (e: any) {
          console.error(`[EmailAutomação] Erro conta ${conta.email}:`, e.message);
        }
      }
    } catch (e: any) {
      console.error("[EmailAutomação] Erro no cron:", e.message);
    }
  }, { timezone: "America/Sao_Paulo" });
  console.log("[EmailAutomação] Cron diário agendado para 06:00 BRT");

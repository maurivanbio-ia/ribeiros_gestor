...
export function registerLicencasRoutes(app: Express, { storage, requireAuth }: LicencasRoutesContext) {
  ...
  // Replicar condicionantes de uma licença para outras licenças
  app.post("/api/licencas/:licencaId/condicionantes/replicar", requireAuth, async (req, res) => {
    try {
      const licencaId = parseInt(req.params.licencaId);
      const { targetLicencaIds, condicionanteIds } = req.body as {
        targetLicencaIds: number[];
        condicionanteIds?: number[];
      };

      if (!Array.isArray(targetLicencaIds) || targetLicencaIds.length === 0) {
        return res.status(400).json({ message: "Selecione ao menos uma licença de destino" });
      }

      // Busca condicionantes da licença origem
      let sourceConds = await db.select().from(condicionantes).where(eq(condicionantes.licencaId, licencaId));

      // Filtra pelos selecionados se especificado
      if (condicionanteIds && condicionanteIds.length > 0) {
        sourceConds = sourceConds.filter(c => condicionanteIds.includes(c.id));
      }

      if (sourceConds.length === 0) {
        return res.status(400).json({ message: "Nenhuma condicionante encontrada para replicar" });
      }

      let totalCriadas = 0;
      const erros: string[] = [];

      for (const targetId of targetLicencaIds) {
        if (targetId === licencaId) continue;
        try {
          for (const cond of sourceConds) {
            const { id, licencaId: _lid, criadoEm, atualizadoEm,
              solicitadoEncerramento: _se, dataEncerramento: _de, arquivoEncerramento: _ae,
              progresso: _prog, status: _st, ...rest } = cond as any;
            await storage.createCondicionante({
              ...rest,
              licencaId: targetId,
              status: "pendente",
              progresso: 0,
          ...
          erros.push(`Licença ${targetId}: erro ao criar condicionantes`);
        }
      }
      ...
      res.json({
        ...
        licencas: targetLicencaIds.length,
        erros: erros.length > 0 ? erros : undefined,
      });
    } catch (error) {
      console.error("Replicar condicionantes error:", error);
      res.status(500).json({ message: "Erro ao replicar condicionantes" });
    }
  });
  ...
  // Solicitação de Encerramento de Condicionante
  app.post("/api/condicionantes/:id/encerramento", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      const dataEncerramento = req.body?.dataEncerramento || null;

      let arquivoEncerramento: string | null = null;
      if (file) {
        const allowed = ['application/pdf', 'image/jpeg', 'image/jpg', 'image/png'];
        if (!allowed.includes(file.mimetype)) {
          return res.status(400).json({ message: "Tipo de arquivo não permitido. Use PDF, JPG ou PNG." });
        }
        ...
        const objectPath = `${privateDir}/encerramento/${objectId}.${ext}`;
        ...
        arquivoEncerramento = `object:encerramento/${objectId}.${ext}`;
      }

      const updated = await storage.updateCondicionante(id, {
        solicitadoEncerramento: true,
        ...(dataEncerramento ? { dataEncerramento } : {}),
        ...(arquivoEncerramento ? { arquivoEncerramento } : {}),
      } as any);
      websocketService.broadcastInvalidate('condicionantes');
      res.json(updated);
    } catch (error) {
      console.error("Encerramento condicionante error:", error);
      res.status(500).json({ message: "Erro ao registrar solicitação de encerramento" });
    }
  });

  app.delete("/api/condicionantes/:id/encerramento", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const updated = await storage.updateCondicionante(id, {
        solicitadoEncerramento: false,
        dataEncerramento: null,
        arquivoEncerramento: null,
      } as any);
      websocketService.broadcastInvalidate('condicionantes');
      res.json(updated);
    } catch (error) {
      console.error("Remove encerramento error:", error);
      res.status(500).json({ message: "Erro ao remover solicitação de encerramento" });
    }
  });

  // Servir arquivo de encerramento (object storage)
  app.get("/api/condicionantes/:id/encerramento/arquivo", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const { condicionantes: condTable } = await import('@shared/schema');
      const { eq } = await import('drizzle-orm');
      const rows = await db.select().from(condTable).where(eq(condTable.id, id)).limit(1);
      const cond = rows[0];
      if (!cond || !(cond as any).arquivoEncerramento) return res.status(404).json({ message: "Arquivo não encontrado" });
      const path: string = (cond as any).arquivoEncerramento;
      if (!path.startsWith('object:')) return res.status(404).json({ message: "Arquivo não acessível" });
      const { objectStorageClient, ObjectStorageService } = await import("../replit_integrations/object_storage/objectStorage");
      const objStorage = new ObjectStorageService();
      const privateDir = objStorage.getPrivateObjectDir();
      if (!privateDir) throw new Error("PRIVATE_OBJECT_DIR not set");
      const relativePath = path.replace('object:', '');
      const fullPath = `${privateDir}/${relativePath}`;
      const parts = fullPath.split("/").filter((p: string) => p.length > 0);
      const bucketName = parts[0];
      const objectName = parts.slice(1).join("/");
      const bucket = objectStorageClient.bucket(bucketName);
      const file = bucket.file(objectName);
      const [buffer] = await file.download();
      const ext = relativePath.split('.').pop() || 'pdf';
      const mimeMap: Record<string, string> = { pdf: 'application/pdf', jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png' };
      res.setHeader('Content-Type', mimeMap[ext] || 'application/octet-stream');
      res.setHeader('Content-Disposition', `inline; filename="encerramento.${ext}"`);
      res.send(buffer);
    } catch (error) {
      console.error("Download encerramento error:", error);
      res.status(500).json({ message: "Erro ao baixar arquivo" });
    }
  });
  ...
  // Server-side image upload — stores logo as base64 data URL (works in all environments)
  app.post("/api/upload/image/server", requireAuth, async (req, res) => {
    try {
      ...
      if (!allowed.includes(file.mimetype)) {
        return res.status(400).json({ message: "Tipo não permitido. Use JPG, PNG, WebP ou GIF." });
      }
      ...
      res.json({ filePath: dataUrl });
    } catch (error: any) {
      console.error("Image upload error:", error);
      res.status(500).json({ message: "Erro ao fazer upload da imagem: " + error.message });
    }
  });
  ...
  app.get("/files/:filePath(*)", requireAuth, async (req, res) => {
    try {
      const { ObjectStorageService, ObjectNotFoundError } = await import("../services/objectStorage");
      const objectStorageService = new ObjectStorageService();
      ...
      const { ObjectNotFoundError } = await import("../services/objectStorage");
